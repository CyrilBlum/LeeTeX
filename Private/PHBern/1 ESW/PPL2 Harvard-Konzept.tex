...
\lhead[ \leftmark ]{PHBern, PPD2}
\rhead[Informatik]{Cyril Blum, \today, Bern}

\part*{PPD2, Merkblatt zum Harvard-Konzept\\\normalsize
Severin Henzi, Simon Kronenberg, Malte Scheurer, Cyril Blum}

\section*{Das Harvard-Konzept}
Im Buch \enquote{Getting to Yes} (Deutsch: \enquote{Das Harvard-Konzept} \cite{harvardKonzeptDE}) beginnen die Autoren \textcite{Fisher1981GettingTY} mit der Frage, welches die beste Art sei, wie Menschen mit ihren unterschiedlichen Arten umgehen können und der Feststellung, dass der Alltag von Familien, Paaren, Angestellten, Vorgesetzten, Konsumenten, Verkäufern und anderen Menschen, die in gemeinsamer Beziehung stehen, von der Herausforderung geprägt ist, in vielen Bereichen Einigungen erzielen zu können (\enquote{getting to yes}), ohne sich in Streitigkeiten zu verwickeln. Dabei wird häufig einer von zwei Ansätzen gewählt, um Einigungen zu erzielen: der \enquote{harte} Ansatz, bei dem jede Verhandlung zum Willens- und Überlebenskampf wird, oder der \enquote{weiche} Ansatz, bei dem eine Person nachgibt und Zugeständnisse macht, um einen Kompromiss zu erzielen. Die Autoren plädieren jedoch für einen anderen Ansatz, das sogenannte sachbezogene Verhandeln (\enquote{\textit{principled negotiation}}), welches sie im Rahmen des Harvard Negotiation Projects erarbeitet haben \cite{hnp}. Der grundlegende Befund dieses Ansatzes ist, dass Verhandlungen nicht als Präferenzen-basiertes \enquote{Feilschen} auszuführen seien, sondern vielmehr sachorientiert und basierend auf möglichst fairen Standards, welche zum grösstmöglichen gemeinsamen Nutzen führen. Aufgrund der Trennung vom Sachverhalt zu den Menschen lässt sich dieser Ansatz als \enquote{hart} in der Sache aber \enquote{weich} im Umgang mit Menschen bezeichnen. 
...
\end{enumerate*} Darauf aufbauend entwickeln die Autoren vier Kriterien, um sachorientiertes Verhandeln zu definieren (siehe \cref{table:gty})

\begin{table}[H]
\centering
\begin{tabular}{p{4cm}|p{4cm}||>{\columncolor[gray]{.9}}p{6cm}}
\multicolumn{2}{c||}{\textbf{Problemorientierte Ansätze}, \enquote{Feilschen}} & \textbf{Lösungs-Orientierter Ansatz} \\\midrule
...
%\enquote{gut} und nicht \enquote{sehr gut} war. Können Sie mir sagen, wie ich mich verbessern kann?
...
Lernapplikation: \\
\small \texttt{\href{https://cblum.ch/quiz-harvard}{cblum.ch/quiz-harvard}}
\end{minipage}
\begin{minipage}[c]{.22\textwidth}
\qrcode[height=1.3cm]{https://cblum.ch/quiz-harvard}
\end{minipage}
\vline\hfill
\begin{minipage}[c]{.25\textwidth}
Video: \\
\small \texttt{\href{https://cblum.ch/ppl2}{cblum.ch/ppl2}}
\end{minipage}
\begin{minipage}[c]{.22\textwidth}
\qrcode[height=1.3cm]{https://cblum.ch/ppl2}
\end{minipage}
\end{myExBox}
