...
\newcommand{\maketitlepage}[2]{
	...
	\begin{titlepage}
		...
		\begin{center}
			% logo
			\iftoggle{includelogo}{\begin{figure}[H]\centering\includegraphics[width=\logosize]{\thelogo}
				\end{figure}}{}
			\thesubject\\
			...
			% Author block logic:
			\iftoggle{oneauthor}{
				% Case 3: Only one main author (did 90%+)
				% Case: Only one main author
				\foreach \author/\mail in \mainauthors {
					\href{mailto:\mail}{\color{black}{\author}}\\[0.5cm]
				}
			}{
				\iftoggle{twomainauthors}{
					% Case 2: Two main authors (contribution ~50/50)
					% Print authors separated by commas (works with pgffor v3.1+)
					\newcounter{mainauthorcount}
					\setcounter{mainauthorcount}{0}
					\foreach \a/\m in \mainauthors {
						\stepcounter{mainauthorcount}
					}
					\foreach [count=\i] \author/\mail in \mainauthors {
						\href{mailto:\mail}{\color{black}{\author}}%
						\ifnum\i<\value{mainauthorcount},\fi
					}
				}{
					% Case 1: One main author + major contributors (main did >70%)
					\newcount\majorcontribcount
					\majorcontribcount=0
					\foreach \contrib/\mail in \majorcontributors {
						\advance\majorcontribcount by 1
					}
					\begin{center}
						\begin{tabular}{cc}
							\textbf{Hauptautor}                                                                  & \textbf{Ko-Autor} \\
							\foreach \author/\mail in \mainauthors {\href{mailto:\mail}{\color{black}{\author}}} &
							\begin{tabular}[t]{c}
								\newcount\currentcontrib
								\currentcontrib=0
								\foreach \contrib/\mail in \majorcontributors {\advance\currentcontrib by 1
									\href{mailto:\mail}{\color{black}{\contrib}}%
									\ifnum\currentcontrib<\majorcontribcount,\fi}
							\end{tabular}
						\end{tabular}\\[0.5cm]
					\end{center}
				}
