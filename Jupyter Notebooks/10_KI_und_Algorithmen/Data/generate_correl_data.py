...
def generiere_korrelationsdaten(
    szenario: str, n: int = 100, seed: int = 42
) -> pd.DataFrame:
    np.random.seed(seed)
    ...
    if szenario == "polizei_vs_kriminalitaet":
        ...
        y = np.clip(
            np.random.normal(loc=x, scale=1, size=n), 0, None
        )  # mehr Streifen → mehr Meldungen, keine negativen Straftaten
        ...
        y = 1 / (1 + np.exp(-0.2 * (x - 10))) + np.random.normal(
            0, 0.05, n
        ...
        y = np.clip(
            0.01 * (x - 60) ** 2 + np.random.normal(0, 1, n), 0, 100
        )  # Risiko steigt mit Abweichung von Ideal
        ...
        y = np.clip(
            8 - 0.5 * x + np.random.normal(0, 1, n), 0, 10
        )  # mehr Beiträge → weniger Zufriedenheit
        ...
        y = np.clip(
            3 + 0.05 * x + np.random.normal(0, 0.5, n), 1, 6
        )  # längere Haare → schlechtere Note
        ...
        y[outlier_indices] = np.random.uniform(
            1, 6, num_outliers
        )  # zufällige Werte innerhalb des Notenbereichs
        ...
        y = np.clip(
            6 - 0.5 * (8 - x) + np.random.normal(0, 0.3, n), 1, 6
        )  # mehr Schlaf → bessere Note
...
if __name__ == "__main__":
    ...
    for r in correlations:
        ...
        plt.savefig(
            f"Grundlagen_Info/10_KI_und_Algorithmen/Figures/correlation_{r}.pdf",
            bbox_inches="tight",
            pad_inches=0,
            transparent=True,
    ...
    generiere_korrelationsdaten("berufserfahrung_vs_einladung", n=20).to_csv(
        "Grundlagen_Info/10_KI_und_Algorithmen/Data/berufserfahrung_vs_einladung.csv",
        index=False,
    )
    generiere_korrelationsdaten("socialmedia_vs_zufriedenheit", n=20).to_csv(
        "Grundlagen_Info/10_KI_und_Algorithmen/Data/socialmedia_vs_zufriedenheit.csv",
        index=False,
    )
    generiere_korrelationsdaten("haarlaenge_vs_note", n=14, seed=1).to_csv(
        "Grundlagen_Info/10_KI_und_Algorithmen/Data/haarlaenge_vs_note.csv", index=False
    )
    generiere_korrelationsdaten("schlafdauer_vs_note", n=25, seed=1).to_csv(
        "Grundlagen_Info/10_KI_und_Algorithmen/Data/schlafdauer_vs_note.csv", index=False
    )
