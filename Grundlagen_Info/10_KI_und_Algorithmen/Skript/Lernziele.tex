...
\section{Regression}
\begin{todolist}
    \item Ich verstehe die Grundidee der \textbf{linearen Regression} und die Methode der kleinsten Quadrate.
    \item Ich kann Regressionskoeffizienten interpretieren und vorhersagen.
    \item Ich kann \textbf{MAE} und \textbf{RMSE} berechnen und verstehe, warum RMSE grosse Fehler stärker bestraft.
    \item Ich erkenne, dass lineare Regression nur lineare Zusammenhänge abbildet und Erweiterungen (z.\,B. polynomiale Regression) für nichtlineare Daten nötig sind.
    \item Ich verstehe den wichtigen Unterschied: Regression modelliert historische Muster, sagt aber nicht automatisch die Zukunft voraus.
    \item Ich erkenne, wie historische Verzerrungen in Trainingsdaten perpetuiert werden, wenn Regression für Vorhersagen genutzt wird.
\end{todolist}
