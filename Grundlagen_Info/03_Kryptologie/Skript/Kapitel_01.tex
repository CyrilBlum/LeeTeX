...
\begin{myremark}[\emph{Security through Obscurity}]\label{myremark:security_through_obscurity}
...
\begin{myexercise}
	...
	\end{center}
	wurde mit einer Tabelle mit 5 Zeilen und 8 Spalten erzeugt. Wie lautet der Klartext? Tipp: Nehmen Sie Ihr bevorzugtes Tabellenkalkulationsprogramm zuhilfe oder verwenden Sie \href{https://cryptbreaker.marcwidmer.xyz/solve}{diesen Link} mit \emph{Analysewerkzeug} $\rightarrow$ \emph{Tabelle}.
	\begin{myanswer}
...
\begin{myexercise}
	...
	\begin{center}
		\texttt{SRESIENITEIINHFNCDIROHAEADTSRGESIDE}
	\end{center}
	...
	\begin{myanswer}
		\begin{table}[H]
			\centering
			% *{6}{C|} bedeutet: 6-mal den Typ 'C' mit Trennlinie
			% 6 Spalten vom Typ C (mit Linien) und die letzte Spalte ohne Linie
			\begin{tabular}{*{6}{C|}C}
				S & E & I & N & O & D & E \\ \hline
				R & N & I & C & H & T & S \\ \hline
				E & I & N & D & A & S & I \\ \hline
		...
		\end{table}
		\begin{enumerate}
			\item Wir müssen $35$ Buchstaben auf ein Rechteck verteilen. Damit kommen alle Teiler von $35$, also 1, 5, 7 und 35, als mögliche Schlüssel in Frage.
			...
			\item Der Klartext lautet:
			      \begin{center}
				      {\small
					      \texttt{SEIN ODER NICHT SEIN DAS IST HIER DIE FRAGE}}
			      \end{center}
		\end{enumerate}
	\end{myanswer}
\end{myexercise}
...
\begin{myexercise}\label{ex:zweiertausch}
	...
	\begin{myanswer}
		...
		\end{figure}
		Beachten Sie, dass die Buchstaben des Klartexts im Kryptotext erhalten bleiben, aber ihre Reihenfolge geändert wird.
		Die Schlüssel für diese Kryptosystem sind die Anweisungen, wie die Buchstaben vertauscht werden, z.\,B. \enquote{Zweiertausch} oder \enquote{Dreiertausch}.
	\end{myanswer}
\end{myexercise}
...
\begin{myexercise}\label{myexercise:vorbetrachung_haeufigkeitsanalyse}
	...
	Entschlüsseln Sie den Kryptotext, ohne alle Schlüssel auszuprobieren, wenn Sie wissen, dass der häufigste Buchstabe im Klartext \texttt{E} ist. Verwenden Sie das \href{https://www.101computing.net/cipher-wheel.html}{interaktive Werkzeug}, um die Entschlüsselung durchzuführen.
	\begin{myanswer}
...
\begin{mydefinition}[Kryptosystem]\label{mydefinition:kryptosystem}
	...
	\end{align*}
	Schlüsselmenge bei Caesar: $\lrc{0, 1, \dots, 25}$ beziehungsweise $\lrc{\text{Ord}(\texttt{A}), \text{Ord}(\texttt{B}), \dots, \text{Ord}(\texttt{Z})}$.
\end{mydefinition}
...
\section{Kryptoanalyse von Caesar}\label{sec:kryptoanalyse_caesar}
...
Die durchschnittlichen Häufigkeiten der häufigsten Buchstaben, Bi- und Tri-Grammen in deutschen Texten ist in \cref{tab:alle-haeufigkeiten} angegeben.
...
\begin{table}[H]
	...
	\end{subtable}
	\caption{Relative Häufigkeiten der Buchstaben, Bigramme und Trigramme in der deutschen Sprache.}
	\label{tab:alle-haeufigkeiten}
\end{table}

\begin{myexercise}\label{ex:caesar}
	Können Sie mit Hilfe einer Häufigkeitsanalyse den Klartext finden? Verwenden Sie \cref{tab:alle-haeufigkeiten}, um den Schlüssel zu erraten und den Text zu entschlüsseln.

	Kryptotext:

	\texttt{WMILEFIRIWKIWGLEJJXHMIWIRXIBXDYOREGOIR}

	Verwenden Sie das \href{https://www.101computing.net/cipher-wheel.html}{interaktive Werkzeug}, um die Entschlüsselung durchzuführen.
...
Im Allgemeinen könnten wir aber auch jede beliebige \emph{Permutation} (Eins-zu-Eins-Zuordnung) von Buchstaben zu erlauben. Eine solche Permutation ist in \cref{fig:monoalph-subst} dargestellt. Wir nennen dieses Kryptosystem \textbf{allgemeine monoalphabetische Substitution}.
\begin{figure}[H]
	\centering
...
\begin{myexercise}\label{myexercise:monoalph-subst-keys}
	...
	\begin{myanswer}
		...
		\par
		Angenommen wir hätten einen Computer, der 1 Milliarde (also $10^9$) Schlüssel pro Sekunde ausprobieren könnte, dann würden wir immer noch ungefähr $1.28\cdot 10^{10}$ Jahre benötigen, um alle möglichen Schlüssel auszuprobieren.
		Zum Vergleich: Das Alter des Universums wird auf etwa $1.38\cdot 10^{10}$ Jahre geschätzt.
	\end{myanswer}
\end{myexercise}

In \cref{myexercise:monoalph-subst-keys} haben Sie gesehen, dass es eine sehr grosse Anzahl solcher Zuordnungen gibt.
Obschon die Schlüsselmenge bei der allgemeinen monoalphabetischen Substitution viel grösser ist als bei Caesar, kann dieser Verschlüsselungsalgorithmus ebenfalls (wie Caesar) sehr einfach mit Hilfe einer Häufigkeitsanalyse geknackt werden.
Das genauere Vorgehen wird in \cref{myexample:monoalph-subst-freq} illustriert.
...
\begin{myremark}[Grosse Schlüsselmenge notwendig, aber nicht hinreichend für Sicherheit]
	Obwohl gemäss \cref{myexercise:monoalph-subst-keys} die monoalphabetische Substitution eine sehr grosse Schlüsselmenge besitzt, ist sie dennoch überhaupt nicht sicher, da sie leicht durch Häufigkeitsanalyse geknackt werden kann.
	Gleichzeitig kann ein Kryptosystem mit einer kleinen Schlüsselmenge nie sicher sein, da es einfach ist, alle möglichen Schlüssel auszuprobieren.
