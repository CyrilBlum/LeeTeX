\chapter{Begleitdokument für Lehrpersonen}
%\begin{itemize}
...
Beim untenstehenden Themenüberblick wurde insbesondere darauf geachtet, die Reihenfolge möglichst sinnvoll zu gestalten, so dass die Kenntnisse schrittweise erweitert und aufeinander aufgebaut werden können. Zudem wurde darauf geachtet, stets ein möglichst ausgeglichenes Verhältnis zwischen \enquote{theoretisch-mathematischen'' und ``grafisch-interaktiven} Aufgaben zu bewahren (da letztere die SuS erfahrungsgemäss stärker motivieren).
...
\begin{myExBox}
	...
	\begin{myExBox}[title=Mögliche Schwierigkeiten \& geeignete Massnahmen]
		\begin{enumerate}
			...
			\item Technische Herausforderungen beim Ausführen von Code: Gerade in der ersten Lektion brauchen einige SuS noch Unterstützung beim Ausführen und Debuggen von Code (\enquote{wo ist der Run-Knopf?}). Mögliche Lösungsansätze könnten sein, genügend Zeit für die Übungen einzuplanen (siehe Punkt 2), bzw. den Einstieg nach Möglichkeit in Halbklassen zu machen.
		\end{enumerate}
	\end{myExBox}
\end{myExBox}
\newpage

\section{Datentypen und Syntax (Variablen \& Zahlen)}
\begin{myExBox}
	\tcbsubtitle{Thema}
	...
	\begin{todolist}
		...
		\item Die SuS können den Modulo-Operator (\texttt{\%}) korrekt verwenden, beispielsweise um alle geraden Zahlen in einer Schleife auszugeben  (siehe DL 1 für einfache Schleifen).
	...
	\begin{myExBox}[title=Mögliche Schwierigkeiten \& geeignete Massnahmen]
		\begin{enumerate}
			\item Die SuS könnten verwirrt sein, weshalb mit dem Gleichzeichen nun etwas anderes gemeint ist als in der Mathematik (Zuweisung statt Gleichheit). Dies könnte durch klare grafische Darstellungen gelöst werden, etwa wie folgt: $  \tikzmarknode{b}{\text{\texttt{a}}} =\tikzmarknode{a}{\color{red}\underbrace{\color{black}\text{\texttt{10+3}}}}
			      ...
			      Die Erklärung dazu könnte lauten: \enquote{Wir werten \lstinline|10+3 = 13| aus und speichern den Wert \lstinline|13| in der Variable \lstinline|a|}.
...
\section{Datentypen und Syntax (Text \& Debugging)}
\begin{myExBox}
	\tcbsubtitle{Thema}
...
\section{Verzweigungen mit \texorpdfstring{\lstinline|if...elif...else|}{if...elif...else}}
\begin{myExBox}
	\tcbsubtitle{Thema}
...
\end{lstlisting}
	...
	\begin{minipage}[H]{.55\textwidth}
		\begin{figure}[H]
			...
			\begin{tikzpicture}[node distance=2.5cm, scale=.5, transform shape, on grid, ->]
				...
				\node (printif) [code, text width=7cm, right=7cm of dec1] {\lstinline|print("Vielen Dank! :)")|};
				\node (dec2) [decision, below of=dec1] {\lstinline|note > 60|};
				\node (printelif) [code, text width=7cm, right=7cm of dec2] {\lstinline|print("Wie kann ich mich verbessern?")|};
				\node (printelse) [code, text width=6cm, below of=dec2] {\lstinline|print("Geben Sie mir eine Chance!")|};
	...
	\begin{myExBox}[title=Mögliche Schwierigkeiten \& geeignete Massnahmen]
		\begin{enumerate}
			...
			\item Die Negation einer Aussage ist nicht dasselbe wie deren Gegenteil. Dies sollte anhand von alltagsnahen Beispielen (z.B. der Aussage \enquote{der Schrank ist voll}) aufgezeigt werden.
		\end{enumerate}
	\end{myExBox}
\end{myExBox}
\newpage

\section{Verzweigungen mit \texorpdfstring{\lstinline|while|}{while}}
\begin{myExBox}
	\tcbsubtitle{Thema}
	In dieser Doppellektion soll die \lstinline|while|-Schleife eingeführt werden, die gewissermassen die bisher gewonnenen Kenntnisse zu Schleifen sowie konditionaler Logik kombiniert. Dabei wird das Augenmerk wieder vermehrt auf \enquote{unterhaltsame'', grafische Übungen gelegt, wie etwa dem Zeichnen von Spiralen mit \lstinline|turtle| und mit dem \lstinline|while|-Befehl. Als gegenüber zur \enquote{kopfgesteuerten} \lstinline|while|-Schleife wird der Befehl \lstinline|break| eingeführt, sozusagen als ``fussgesteuerte} Schleife. Die Visualisierung von \lstinline|while|-Schleifen als Flussdiagramm hilft, den Code besser zu verstehen.
...
\section{Befehle / Funktionen ohne Parameter, ohne \texorpdfstring{\lstinline|return|}{return}}
\begin{myExBox}
	\tcbsubtitle{Thema}
	Diese Doppellektion soll als Einführung in die modulare Programmierung dienen. Dabei soll klargestellt werden, dass wir für das Schreiben eines komplexen Programms zunächst kleinere \enquote{Bausteine} benötigen, die zum Lösen von Teilproblemen nötig sind. Die verschiedenen Programme, die sie bisher geschrieben haben, sollen somit für den zukünftigen Gebrauch als Befehle festgehalten werden. Für die Einführung verwenden wir weder Parameter noch das \lstinline|return|-statement.
	...
	Zu Beginn das Konzept der Schleifen in Erinnerung rufen (als Automatisierung eines sich wiederholenden Vorgangs), Übertragung dieses Konzeptes auf den Aufbau des Codes (\enquote{Bündelung von vorprogrammierten Befehlen durch die Nutzung des \lstinline|def|-Statements, um Wiederholungen zu vermeiden}.) Anschliessend sollen die SuS bereits verwendete Befehlsfolgen von \lstinline|turtle|-Befehlen aus den vorherigen Lektionen in einen neuen, eigenen Befehl zusammenfassen und aufrufen.
...
\section{Befehle / Funktionen mit Parameter, ohne \texorpdfstring{\lstinline|return|}{return}}
\begin{myExBox}
	\tcbsubtitle{Thema}
...
\section{Koordinaten \& Zufallszahlen}
\begin{myExBox}
	\tcbsubtitle{Thema}
...
\begin{myExBox}
	...
	\begin{myExBox}[title=Mögliche Schwierigkeiten \& geeignete Massnahmen]
		\begin{enumerate}
			\item SuS könnten zunächst die Vorstellung haben, dass eine Liste mit dem Index \enquote{1} beginnt und somit versuchen, dass erste Element einer Liste mit \lstinline|mylist[1]| abzufragen. Eine graphische Darstellung, wobei die Indizes direkt unter der Liste angegeben sind, kann hier weiterhelfen.
