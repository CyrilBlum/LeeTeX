...
Sie kenne bereits die Funktionen \lstinline|print| sowie \lstinline|t.forward| und haben diese auch schon mehrmals selbst verwendet. In diesem Kapitel werden Sie lernen, wie Sie eigene Funktionen in Python definieren und schliesslich sinnvoll verwenden können. Funktionen helfen enorm dabei, Programme leserlicher, kürzer, effizienter und wartbarer zu machen. Funktionen sind deshalb aus modernen Programmen kaum wegzudenken sind.
...
Definitionen von Funktionen können auch mehrere (oder sogar gar keine) Parameter haben. Dies ist besonders nützlich, wenn Sie eine Funktion schreiben wollen, die mehrere Werte benötigt, um ihre Aufgabe zu erfüllen. Zum Beispiel könnte eine Funktion, die ein Vieleck zeichnet, sowohl die Anzahl der Ecken als auch die Farbe des Vielecks benötigen.
...
\begin{myoverview}[Nomenklatur bei Python-Funktionen]\label{myoverview:NomenklaturFunktion}
...
\begin{myexercise}
	...
	\begin{itemize}
		\item \lstinline|def summiere(x1, x2)|: Berechnet die Summe der zwei Parameter \lstinline|x1| und \lstinline|x2| und gibt das Resultat mittels \lstinline|print| auf der Konsole aus.
		\item \lstinline|def summe_quadrate(x, y)|: Berechnet die Summe \(x^2 + y^2\) und gibt diese mittels \lstinline{print} auf der Konsole aus.
	\end{itemize}
...
\begin{myexercise}\label{ex:dreieck-return}
	Gegeben seien die Höhe \texttt{h} und die Grundseite \texttt{b} eines Dreiecks. Schreiben Sie eine Funktion \lstinline|flaeche_dreieck(h, b)|, welche den Flächeninhalt dieses Dreiecks berechnet und mit \lstinline|return| zurückgibt.
...
\begin{myexercise}\label{ex:quadrat-return}
	Geben sei eine ganze Zahl \(n\). Schreiben Sie eine Funktion \lstinline|square(n)|, welche das Quadrat von \(n\) berechnet und mit \lstinline|return| zurückgibt.
...
Schreiben Sie eine Funktion \lstinline|max_kaufpreis(E, b)|, welche für gegebenes Eigenkapital $E$ und gegebenes Brutto-Jahreseinkommen $b$ den maximal tragbaren Kaufpreis berechnet und printet.
...
\end{align*}
Beachten Sie, dass Fall $k \leq 2E$ die Beziehung $k < E$ nicht ausschliesst. Das Auflösen der Ungleichung
\begin{align*}
	k \le \frac{100b + 15E}{18} \stackrel{!}{=} E
\end{align*}
zeigt, dass dies genau dann auftritt, wenn $b < 0.003E$ ist. In diesem Fall wäre aber $h = k - E < 0$ und die Hypothek wäre somit negativ. Eine negative Hypothek wäre dann eher als Guthaben zu verstehen und natürlich macht die Tragbarkeitssberechnung für ein Guthaben keinen Sinn. In diesem Fall ist der maximale Kaufpreis $E$ (wir bezhalen einfach alles einzig mit Eigenkapital und nehmen gar keine Hypothek auf). Die direkte Konsequenz davon ist, dass sich der maximal tragbare Kaufpreis mit steigendem Lohn $b$ nicht erhöht, solange noch $b \leq 0.003E$ ist.
...
\end{align*}
	...
	Um dem Spezialfall $k - E < 0$ (negative Hypothek) auszuschliessen, müssen wir noch das Maximum zwischen $k$ und $E$ nehmen.

	Die finale Python-Funktion ist schliesslich gegeben durch
	...
	\lstinline{MAX(MIN(MIN((100 * A2 + 15 * A1) / 18, 150 * A2 / 37 + 105 * A1 / 74), 5 * A1), A1)}
	\end{myanswer}
