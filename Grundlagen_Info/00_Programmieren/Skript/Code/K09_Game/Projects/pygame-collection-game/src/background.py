...
class Background:
    ...
    def draw(self, screen):
        ...
        # Draw the sea
        pg.draw.rect(screen, self.sea_color, (0, self.screen_height // 2, self.screen_width, self.screen_height // 2))

    def update(self):
        # Optional: Update the sun position for animation
        if self.sun_position[1] < self.screen_height // 2 + 100:
            self.sun_position = (self.sun_position[0], self.sun_position[1] + 1)  # Move the sun down
