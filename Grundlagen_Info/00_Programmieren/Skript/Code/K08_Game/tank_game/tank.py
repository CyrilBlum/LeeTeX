...
class Tank:
    def __init__(self, x, color, dir, SCREENW, SCREENH):
        ...
        self.angle = (
            random.random() * 2 - 1
        )  # radians upward (-1 to +1), for the barrel
    ...
    def hitbox(self):
        return pg.Rect(
            self.x, self.y - self.width // 2, self.width, self.height + self.width // 2
        )
    ...
    def draw(self, surf):
        ...
        pg.draw.rect(
            self.surface,
            (200, 0, 0),
            (self.x, self.y + self.height * 0.7, bar_w, bar_h),
        )
        pg.draw.rect(
            self.surface,
            (0, 200, 0),
            (self.x, self.y + self.height * 0.7, bar_w * life_ratio, bar_h),
        )
