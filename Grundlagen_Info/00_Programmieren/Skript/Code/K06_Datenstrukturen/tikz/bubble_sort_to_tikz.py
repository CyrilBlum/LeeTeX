...
def generate_tikz_figure(data, idx1, idx2, step, step_prog, exchanged, range=[]):
    ...
    for i, val in enumerate(data):
        box_color = (
            "LimeGreen"
            if i in [idx1, idx2] and exchanged
            else "Lavender" if i in [idx1, idx2] and not exchanged else "CornflowerBlue"
        )
        border_color = (
            "ForestGreen"
            if i in [idx1, idx2] and exchanged
            else "OrangeRed" if i in [idx1, idx2] and not exchanged else "Black"
        )
    ...
    caption = (
        f"Schritt {step_prog+1}"
        if not exchanged
        else f"Schritt {step_prog+1} (ausgetauscht!)"
    )
    tikz_code += f"\\end{{tikzpicture}}\\caption*{{{caption}}}\n"

    # filename = f"step_{step}{'_exchanged' if exchanged else ''}.tex"
    filename = os.path.join(
        "./Grundlagen_Info/00_Programmieren/Skript/Code/K06_Datenstrukturen/tikz/bubble_sort_steps/",
        f"step_{step+1}{'' if exchanged else ''}.tex",
    )
        ...
        for j in range(0, n - 1 - i):
            filenames.append(
                generate_tikz_figure(
                    arr, j, j + 1, step, step_prog, False, range=[0, n - i - 1]
                )
            )
            step += 1
            ...
            if arr[j] > arr[j + 1]:
                ...
                filenames.append(
                    generate_tikz_figure(
                        arr, j, j + 1, step, step_prog, True, range=[0, n - i - 1]
                    )
                )
                step += 1
...
def generate_tikz_binary_search_to_tex(array, target):
    ...
    while left <= right:
        ...
        filename = os.path.join(
            "./Grundlagen_Info/00_Programmieren/Skript/Code/K06_Datenstrukturen/tikz/binary_search_steps/",
            f"binary_search_step_{step}.tex",
        )
        # Binary search logic
        if array[mid] == target:
