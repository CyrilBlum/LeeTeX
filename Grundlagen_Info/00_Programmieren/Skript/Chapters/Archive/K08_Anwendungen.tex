...
\begin{myexercise}\label{ex:bmr-simple}
	...
	\end{enumerate}
	\begin{myanswer} %
		\lstinputlisting[language=Python,caption=\texttt{grundumsatz.py},label=listing:grundumsatz]{Grundlagen_Info/00_Programmieren/Skript/Code/K08_Anwendungen/bmr_functions/grundumsatz.py}
	\end{myanswer}
\end{myexercise}
...
\begin{myexercise}\label{ex:bmr}
	...
	\begin{myanswer} %
		\lstinputlisting[language=Python,caption=\texttt{bmr.py},label=listing:bmr]{Grundlagen_Info/00_Programmieren/Skript/Code/K08_Anwendungen/bmr_functions/bmr.py}
	\end{myanswer}
\end{myexercise}
...
\begin{myexercise}\label{ex:leistungsumsatz}
	...
	\begin{myanswer} %
		\lstinputlisting[language=Python,caption=\texttt{leistungsumsatz.py},label=listing:leistungsumsatz]{Grundlagen_Info/00_Programmieren/Skript/Code/K08_Anwendungen/bmr_functions/leistungsumsatz.py}
	\end{myanswer}
...
\begin{myexercise}
	...
	\begin{myanswer} %
		...
		\lstinputlisting[language=Python,caption=\texttt{total.py},label=listing:total]{Grundlagen_Info/00_Programmieren/Skript/Code/K08_Anwendungen/bmr_functions/total.py}
	\end{myanswer}
\end{myexercise}
...
\begin{enumerate}
    ...
    \item Installieren Sie \texttt{pillow}, \texttt{matplotlib} sowie \texttt{numpy} mit pip:\\
	\lstinline|pip install numpy matplotlib pillow|
	\item Erstellen Sie auf Ihrem eigenen Rechner einen neuen Ordner \emoji{file-folder} mit dem Namen \texttt{Bilder\_Bearbeiten}.
	...
	\item Die Datei \texttt{dateien.zip} ist komprimiert und muss zuerst entpackt werden. Entpacken geht ganz einfach so:
	      ...
	      \end{description}
	\item Öffnen Sie das Python-File \texttt{bilder\_template.py} in \texttt{VS Code}.
...
\begin{myexercise}[\textbf{Vertikale Streifen}]
	Schreiben Sie ein Python-Funktion \lstinline|vertical_stripes(filename)|, welche ein Graustufenbild mit dem Dateinamen \lstinline|filename| als Argument erhält. Die Funktion soll \enquote{vertikale Streifen} in gleichmässigen Abständen durch das Bild legen.
	% Ein Beispiel dieser Transformation ist in \cref{fig:streifen} gegeben. Hier haben wir vertikale Streifen mit einer Breite von 2 Pixeln gewählt. Der Abstand (Anfang zu Anfang) der Streifen ist 7 Pixel (also 5 unveränderte Pixel zwischen Ende eines Streifens und dem Anfang des nächsten Streifens).
	% \begin{figure}[H]
	% 	\centering
