...
\begin{myexercise}\label{ex:arithm-ausdruecke}
	...
	\begin{table}[H]
		...
		\begin{tabular}{c|c}
			...
			$17 - 3 \cdot 8$    & \texttt{res1}     \\\midrule
			$(5-2) \cdot 4 + 1$ & \texttt{res2}     \\\midrule
...
\begin{myexercise}\label{ex:austausch}
	...
	\begin{lstlisting}[numbers=left]
        ...
        y = x
\end{lstlisting}
	Führen Sie das Programm aus und geben Sie die Werte von \lstinline|x| und \lstinline|y| auf der Konsole aus. Funktioniert das Programm wie erwartet? Weshalb (nicht)?
	\begin{myanswer}
		\lstinline|x| und \lstinline|y| haben nun beide den Wert \lstinline|11|. Auf Zeile 4 wird der \textit{Wert} von \lstinline|y| in \lstinline|x| gespeichert. Ab diesem Moment haben sowohl die Variablen \lstinline|x| wie \lstinline|y| denselben Wert \lstinline|11|.
	\end{myanswer}
\end{myexercise}
...
In Python kann man nicht nur arithmetische Operationen mit Zahlen durchführen, sondern auch mit Text arbeiten. In der Informatik verstehen wir unter einem \textit{Text} jede endliche Folge von Symbolen der Tastatur. Eine solche Folge von Symbolen bezeichnet man auch als \textit{Zeichenkette}\index{Zeichenkette} (Englisch: \textit{string}\index{string!Zeichenkette}). Beispiele von Strings haben wir bereits in \cref{myexample:prints} gesehen. Wir können Strings, ebenso wie Zahlen, in Variablen speichern.
...
\begin{mychallenge}
	Schreiben Sie ein Programm, welches die Form einer Banane auf der Konsole ausgibt, indem Sie die Stringoperationen \lstinline|*| und \lstinline|+| verwenden.
	\begin{myanswer}
...
\section{Datentypen}
Bisher haben wir zwei Arten, oder \textit{Typen} von Variablen gesehen: Zahlen und Text (Strings).
...
\begin{myexample} Führen Sie folgenden Code in VS Code aus und beobachten Sie das Resultat in der Konsole:
\begin{lstlisting}[numbers=none]
n = 20
m = 30
prod = n * m
...
Eine etwas elegantere Art, Variablen in einem print mit Text zu verbinden ist, die Python-Schreibweise \lstinline|print(f"...")| zu verwenden.
...
\begin{myexercise}
	Schreiben Sie ein Python-Programm, welches in einer Schleife die Quotienten von 2/3, 3/3, 4/3, etc. bis 11/3 berechnet und auf der Konsole ausgibt, z.B.:\lstinline|"4 durch 3 ist 1.333"|. Verwenden Sie dazu die \lstinline|f"|-Schreibweise und formatieren Sie die Quotienten so, dass immer 3 Nachkommastellen angezeigt werden.
	\begin{myanswer}
		\begin{lstlisting}
n = 2
for _ in range(10):
...
\begin{mychallenge}\label{ex:sToY}
	...
	Das Programm soll den folgenden Text ausgeben, wobei alle Zahlen (ausser 123456789) im Programm berechnet werden: \enquote{123456789 Sekunden sind 3 Jahre, 333 Tage, 21 Stunden, 33 Minuten, 9 Sekunden!}
...
Syntax-Fehler (Englisch: \textit{syntax error}) sind in Programmiersprache vergleichbar mit Rechtschreibfehlern in Aufsätzen: Es handelt sich um Schreibweisen, die nicht erlaubt sind. Beispielsweise würde folgender Code eine Fehlermeldung geben:
...
Laufzeitfehler (Englisch: \textit{runtime error}) werden erst bei der Ausführung des Programms erkannt. Dabei stimmt die grammatikalische Struktur (Syntax) des Codes zwar, der Code ergibt aber keinen Sinn: Beispielsweise wird versucht, auf Variablen zuzugreifen, welche es gar nicht gibt. Fehler können beispielsweise aufgrund von Gross- und Kleinschreibung auftreten:
...
\begin{myexercise}\label{myexercise:austausch-debug}
	...
	\begin{myanswer}
		Dies ist ein Beispiel für einen semantischen Fehler (Fehler in der Logik).
	\end{myanswer}
\end{myexercise}
...
\begin{mychallenge}\label{mychallenge:fibonacci_spirale}
	...
	Zeichnen Sie danach mit der Turtle eine Spirale, bestehend aus 8 Viertelkreisen, welche als Radius\footnote{Natürlich zeichnen wir hier regelmässige Polygone und keine Kreise.} jeweils das \textbf{Zehnfache} der zuletzt berechneten Fibonacci-Zahl $1, 2, 3, 5, 8, 13, \ldots$ haben.
