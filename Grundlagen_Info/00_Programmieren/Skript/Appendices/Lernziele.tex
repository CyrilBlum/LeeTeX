...
\begin{todolist}
    ...
    \item Ich kann \enquote{Kreise} als Vielecke mit grosser Eckenzahl zeichnen.
    ...
    \item Ich setze die Operationen \lstinline|+| und \lstinline|*| ein, um komplizierte Texte (Zeichenketten) zu erzeugen.
    \item Ich kenne die Operationen auf Zahlen, die in \cref{myoverview:arithm-op} aufgelistet sind, und kann diese auch einsetzen.
...
\begin{todolist}
    ...
    \item Ich kann die logischen Operatoren \lstinline|and|, \lstinline|or| sowie \lstinline|not| einsetzen, um boolesche Ausdrücke zu kombinieren.
...
\begin{todolist}
    ...
    \begin{todolist}
        \item Ich kann \textit{bubble sort} in Python programmieren und verstehe den Algorithmus im Detail.
        \item Ich kann die binäre Suche in Python programmieren und verstehe den Algorithmus im Detail.
    \end{todolist}
    ...
    \begin{todolist}
        ...
        \item Ich kann die Länge eines Dictionaries, also die Anzahl der Schlüssel-Wert-Paare, mit dem Befehl \lstinline|len(dictionary)| bestimmen.
        \item Ich kann ein Schlüssel-Wert-Paar aus einem Dictionary entfernen.
        \item Ich verstehe, dass Dictionaries ungeordnet sind und die Reihenfolge der Elemente keine Rolle spielt.
    \end{todolist}
    \item \textbf{Mengen (Sets)}
    \begin{todolist}
        \item Ich kann Mengen in Python erstellen und verwenden.
        \item Ich kann die grundlegenden Operationen auf Mengen durchführen, wie z.B. Vereinigung, Schnittmenge und Differenz.
        \item Ich kann die grundlegenden Operationen kombinieren, um komplexere Mengenoperationen durchzuführen.
        \item Ich kann typische Anwendungsfälle für Mengen nennen, wie z.B. das Entfernen von Duplikaten aus einer Liste oder das Überprüfen von Mitgliedschaften.
    \end{todolist}
    \item \textbf{Tupel}
    \begin{todolist}
        \item Ich kann Tupel in Python erstellen und auf ihre Elemente zugreifen.
        \item Ich kann den Hauptunterschied zwischen Tupeln und Listen erklären, nämlich die Unveränderlichkeit (Immutability) von Tupeln.
        \item Ich kann die wesentlichen Eigenschaften von Tupeln (geordnet, heterogen) beschreiben.
        \item Ich kann typische Anwendungsfälle für Tupel nennen, wie z.B. die Rückgabe mehrerer Werte aus einer Funktion oder die Verwendung als Schlüssel in einem Dictionary.
    \end{todolist}
\end{todolist}
