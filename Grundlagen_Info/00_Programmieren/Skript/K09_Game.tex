...
Die Bewertung Ihres Projekts erfolgt anhand der folgenden Kriterien:
\begin{itemize}
    \item \textbf{Funktionalität (30\%):} Das Spiel läuft fehlerfrei und erfüllt die grundlegenden Anforderungen (Steuerung, Kollisionen, Punktesystem).
    \item \textbf{Code-Qualität (30\%):} Der Quellcode ist gut strukturiert, lesbar und verwendet die geforderten Python-Konzepte (Variablen, Funktionen, Klassen, Datenstrukturen etc.).
    ...
    \item \textbf{Dokumentation (20\%):} Die Berichte sind klar, gut strukturiert und reflektieren den Entwicklungsprozess.
\end{itemize}

Die maximale Punktzahl beträgt 100 Punkte. 

\subsection{Individuelle Besprechung des Spiels}

Im Anschluss an Ihre Abgabe findet eine kurze, individuelle Besprechung Ihres Spiels (ca. 15 Minuten pro Person) mit der Lehrperson statt. Dabei werden Ihnen Fragen zu Ihrem Spiel gestellt, um Ihr Verständnis und Ihre Reflexion zu überprüfen. Die Besprechung resultiert in einer Verständnis-Prozentzahl von 0\%-100\% Punkten, welche mit Ihrer Endnote multiplizipert wird.

Ihre Endnote wird wie folgt berechnet:
\[\text{Endnote} = \Bigg(\text{Projekt-Punkte} \times \left(\frac{\text{Verständnis-Prozentzahl}}{100}\right)\Bigg)+1*5\]

\begin{myattention}
Es wird von Ihnen erwartet, dass Sie sich aktiv an der Entwicklung des Spiels beteiligen und die geforderten Python-Konzepte verstehen und anwenden können. ChatGPT oder andere KI-Tools dürfen Sie verwenden, solange Sie Ihren gesamten Code verstehen und erklären können. Wie Sie im obigen Benotungsschema erkennen können, kann Ihre Note durch mangelndes Verständnis stark beeinträchtigt werden (bis zur Note 1).
\end{myattention}
