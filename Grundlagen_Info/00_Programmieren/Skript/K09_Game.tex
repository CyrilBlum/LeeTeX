...
Wenn Ihr Ordner beispielsweise \texttt{Informatik/} heisst und das Bild unter \texttt{Informatik/Game/icon.png} gespeichert ist, dann verwenden Sie \lstinline|pg.image.load("Game/icon.png")|.
...
\begin{mychallenge}[Highscore anzeigen]
...
\begin{enumerate}
    \item Rechtecke für Paddles und Ball
    \item Mittellinie zeichnen
    \item Tastatur-Eingaben für Paddle-Bewegung (\texttt{W/S} und \texttt{UP/DOWN})
    \item Kleines Rechteck für den Ball
    \item Ball zufällig von der Mitte aus starten lassen
    \item Kollisionserkennung für Ball und Paddles
...
if ball.colliderect(left_paddle) and ball_vel.x < 0:
    ...
    ball_vel.x *= -1               # x-Richtung umkehren (Abprall)
\end{lstlisting}
...
\end{lstlisting}
    ...
    \item \textbf{Zentrierten Text zeichnen:} \lstinline|get_rect(center=(...))| ist oft einfacher als Koordinaten zu raten.
\begin{lstlisting}
score_surf = font.render("0 : 0", True, (255, 255, 255))
...
def reset_ball(direction):  # direction: -1 nach links, +1 nach rechts
    ...
    ball_vel.x = direction * ball_speed
    ball_vel.y = random.choice((-1, 1)) * ball_speed
\end{lstlisting}
