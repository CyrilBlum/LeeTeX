...
Sie leiten die Entwicklung des Tram-Fahrplans in der Stadt Zürich. Die aktuelle Glatttalbahn (Tram 12) fährt von Stettbach, bzw. Oerlikon, bis zum Flughafen (siehe folgende Abbildungen). Ignorieren Sie die blaue Linie, die bei ``Glattpark'' nach rechts abbiegt.
...
Sie haben sich folgende Stationen und die Fahrzeit vom Anfangsort (Sternen Oerlikon) zu jeder folgenden Station bis und mit ``Zürich Flughafen Fracht'' in Sekunden notiert:
...
Entfernen Sie die Station ``Flughafen Fracht'' aus der Liste \lstinline|stationen| und fügen Sie die neuen Stationen (Obstgartenstrasse) sowie die Reisezeiten (erfundene Werte) in Ihren Listen \lstinline|stationen| und \lstinline|dauer| ein.
...
\begin{itemize}
\item Falls der Startbahnhof nicht Teil der Liste von Bahnhöfen (\lstinline|stationen|) ist, soll zurückgegeben werden ``Kann Start-Bahnhof nicht in Stationen finden''.
