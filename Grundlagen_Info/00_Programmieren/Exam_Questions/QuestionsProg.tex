...
\lstset{style=python, basicstyle=\scriptsize\ttfamily,escapechar=!,xleftmargin=1.5em,
  frame=single,
...
\begin{parts}
	\part[2] Haben Sie es geschafft, alle Aufgaben zu lösen? Berechnen Sie die Antwort in Python, indem Sie eine \lstinline|repeat|-Schleife verwenden und reichen Sie Ihren Code ein.
	\begin{solution}
...
zeit = 0
i=0
for _ in range(len(zeit_aufgaben)):
...
\begin{parts}
	\part[2] Berechnen Sie mit einer \lstinline|repeat|-Schleife, wie viel Geld insgesamt benötigt würde, um alle Bäume unter Schutz zu stellen. Reicht das Geld, um alle Bäume unter Schutz zu stellen?
	\begin{solution}
		Es würden 1'218'079\$ benötigt, um alle Bäume unter Schutz zu stellen. Da wir nun eine Million Dollar besitzen, reicht das Geld also nicht, um alle Bäume zu schützen (siehe Python-Lösung von Teil 2).
	\end{solution}
	...
	\begin{solution}
		Man kann 97.9\% des Waldes unter Schutz stellen, indem man die günstigeren Bäume zuerst \enquote{kauft}.
		\begin{lstlisting}
...
for _ in range(len(baeume_kosten)):
...
Sie entwickeln eine Dating-App, die sich an wissenschaftlichen Prinzipien orientiert. Sie funktioniert folgendermassen: Statt wie bei Tinder zu swipen, geben Sie jedem Kandidaten eine Note zwischen 1 und 10. Die Person gibt Ihnen ebenfalls eine Note zwischen 1 und 10. Sobald 10 Personen sich bewertet haben, werden Sie mit der passendsten Person \enquote{gematched} und Sie können sich schreiben. Um die passendste Person zu finden, wird Folgendes betrachtet:
\begin{itemize}
	\item Die 10 Noten, die Sie abgegeben haben (\lstinline|noten_von_mir = [...]|)
	\item Die 10 Noten, die andere Personen für Sie abgegeben haben (\lstinline|noten_von_anderen=[...]|)
	\item Der Score, um zu bestimmen, ob eine Person ein Match sein könnte:\\
	 \lstinline|match=note_von_mir + note_von_person - abs(note_von_mir - note_von_person)|
\end{itemize}
...
\end{lstlisting}
		Diese Daten bedeuten beispielsweise, dass Johannes Ihnen eine 2 gegeben hat, währenddem Sie ihm eine 3 gegeben haben. Der Matching-Score für Johannes wäre also $3+2-\underbrace{\abs{3-2}}_{1}=4$. Verwenden Sie in Ihrem Code eine \lstinline|repeat|-Schleife und eine Index-Variable. Reichen Sie Ihren Code ein.
	\begin{solution}
...
for _ in range(len(noten_von_mir)):
    ich = noten_von_mir[i]
...
\end{lstlisting}
	\end{solution}
	\part[2] Wie Sie vermutlich wissen, sind Frauen selektiver auf Dating-Plattformen als Männer. Diesen Effekt wollen wir korrigieren, indem wir zuerst die Mittelwerte aller erhaltenen Noten sowie aller gegebenen Noten berechnen. Danach berechnen wir die Differenz beider Mittelwerte als \lstinline|differenz = schnitt_von_anderen - schnitt_von_mir|. Danach addieren wir diese Differenz zu jeder abgegeben Note, so dass am Ende der Durchschnitt der gegebenen und erhaltenen Noten derselbe ist. Wer ist nun der beste Match?
	\begin{solution}
...
Wie sieht Ihr Inventar am Ende des Jahres aus? Verwenden Sie eine Schleife, um an jedem Monatsende Gegenstände zu kaufen, sowie den Befehl \lstinline|.append|, um Gegenstände, die Sie sich leisten können, Ihrem Inventar hinzuzufügen. Können Sie sich innerhalb eines Jahres alle Gegenstände kaufen?
    ...
    for geld in geld_monat:
        ...
        while kann_kaufen and len(wunsch_objekte) > 0:
            kann_kaufen = False
            i = 0
for _ in range(len(wunsch_objekte)):
...
Berechnen Sie die Black-Friday-Preise für alle Elemente im Laden. Der rabattierte Preis wird folgendermassen berechnet:\\
	\lstinline|preis_blackfriday = preis_normal * (1 - rabatt_prozent / 100)|
	\begin{solution}
    \begin{lstlisting}
    preise_normal = [39.95, 65.95, 66.95, 76.95, 9.95, 10.95, 13.95]
    rabatte_blackfriday_prozent = [30, 40, 30, 35, 20, 15, 35]
    ...
    i = 0
for _ in range(len(preise_normal)):
...
Gegeben sei eine Liste \lstinline|A| von ganzen Zahlen der Länge $n$ und eine angestrebte Zahl \mbox{\lstinline|target|.} Schreiben Sie eine Python-Funktion \lstinline|target_sum(A,target)|, die zwei Zahlen \lstinline|a| und \lstinline|b| in \lstinline|A| ermittelt, welche sich zu \lstinline|target| addieren. Die Python-Funktion soll die Indizes von \lstinline|a| und \lstinline|b| zurück geben. Es kann davon ausgegangen werden, dass jede Eingabe genau eine solche Lösung zulässt (es existiert eine Lösung und sie ist eindeutig). Jedes Element der Liste darf nur einmal in der Summe verwendet werden: Das heisst \lstinline|[i,j]| mit \lstinline|target == A[i] + A[j]| für \lstinline|i| $\neq$ \lstinline|j| wäre eine gültige Lösung des Problems, aber \lstinline|[i,i]| mit \lstinline|target == A[i] + A[i]| nicht.
\begin{lstlisting}[numbers=none,framexleftmargin=0em,xleftmargin=0em]
Beispiel 1
...
\begin{solution}
Wir betrachten jedes Element in der Liste und schauen, ob es ein weiteres Element gibt, sodass die Summe der Zwei Elemente dem \lstinline|target| entspricht.
\begin{lstlisting}
...
Wir wollen mit \lstinline|turtle| eine Vieleck-Spirale zeichnen, die sich \textit{n} Umdrehungen macht. Die Anzahl Ecken (\lstinline|n_ecken|) , die Länge des ersten Strichs (\lstinline|laenge|) , die Vergrösserung (\lstinline|vergroesserung|) sowie die Anzahl Umdrehungen (\lstinline|n_spiralen|) sollen frei wählbar sein. Für \lstinline|ecken=5| und \lstinline|n_spiralen=3| soll folgendes Bild entstehen:

\begin{figure}[H]
	\centering
	\includegraphics[width=.3\textwidth]{Grundlagen_Info/00_Programmieren/Exam_Questions/Figures/spirale53}
\end{figure}
...
\begin{itemize}
	\item Falls \lstinline|n_ecken| 1 oder 2 ist, soll eine Fehlermeldung ausgegeben werden und nichts gezeichnet werden.
...
def vieleck_spirale(n_ecken, laenge, vergroesserung, n_spiralen, inc_liste=None):
    ...
    for _ in range(n_spiralen):
        i = 0
        for _ in range(n_ecken):
            t.forward(laenge)
...
Gegeben sei eine Liste \lstinline|A| von ganzen Zahlen mit mindestens drei Elementen. Schreiben Sie eine
TigerJython-Funktion \lstinline|def dreierfenster(A)|, die stets jeweils Gruppen (\enquote{Dreierfenster}) von drei
benachbarten Elementen von \lstinline|A| summiert und diese Summen in einer Liste \lstinline|dreiersummen| abspeichert.
Diese Liste \lstinline|dreiersummen| soll schliesslich durch einen \lstinline|print| ausgegeben werden. Für die gegebene
Liste \lstinline|A = [3, 4, 1, 6, -2]| zum Beispiel, existieren genau die folgenden drei Dreierfenster:
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
...
\clearpage
\question[2]{\textbf{\emoji{tram} Tram-Fahrplan}\\}
...
Sie haben sich folgende Stationen und die Fahrzeit vom Anfangsort (Sternen Oerlikon) zu jeder folgenden Station bis und mit \enquote{Zürich Flughafen Fracht} in Sekunden notiert:
...
Nehmen Sie einfachheitshalber an, dass alle Strecken immer in der Richtung Oerlikon $\rightarrow$ Flughafen zurückgelegt zurücklegt (Berechnungen müssen Sie nur in dieser Richtung machen).

(1 Punkt) Schreiben Sie eine Definition, um die Fahrtzeit von einer beliebigen Station zu einer anderen zu berechnen. 

(1 Punkt) Die Definition soll die Reisezeit in Minuten (ohne Dezimalstellen) von einer beliebigen Station zu einer anderen an das Hauptprogramm zurückgeben. Geben Sie das Resultat \textit{im Hauptprogramm} ()und nicht in der Definition) aus!

(1 Punkt) Die Glattal-Tramlinie soll erweitert werden um die folgenden Stationen:
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{Grundlagen_Info/00_Programmieren/Figures/GTB_Karte_mit_Legende_Web_v02}
\end{figure}

Entfernen Sie die Station \enquote{Flughafen Fracht} aus der Liste \lstinline|stationen| und fügen Sie die neuen Stationen (Obstgartenstrasse) sowie die Reisezeiten (erfundene Werte) in Ihren Listen \lstinline|stationen| und \lstinline|dauer| ein.
...
\begin{solution}
\begin{lstlisting}
def berechne_dauer(station_start, station_ende, stationen, l_dauer):
    # berechne Indizes der Start- und Endstationen
    i_start = l_stationen.index(station_start, 0, len(l_stationen))
    i_end = l_stationen.index(station_ende, 0, len(l_stationen))
    
    # berechne Dauer zwischen Start und Ende
    dauer_gesamt = l_dauer[i_end] - l_dauer[i_start]
    # konvertiere Resultat in Minuten (Ganzzahl)
    dauer_gesamt_min = dauer_gesamt//60
    
    return(dauer_gesamt_min)

l_stationen = ["Sternen", "Messe", "Leutschenbach", "Oerlikerhus", "Glattpark", "Lindbergh", "Glattbrugg", "Bäuler", "Unterriet", "Kloten", "Flughafen", "Flughafen Fracht"]
l_dauer = [0, 60, 122, 185, 241, 363, 543, 603, 666, 729, 786, 845]

# Aufgabe a und b
dauer_reise_min = berechne_dauer("Glattpark", "Lindbergh", l_stationen, l_dauer)
print(dauer_reise_min)

# Aufgabe c
l_stationen.pop()
l_stationen += ["Obstgarten","Stadthaus","Mühle", "Steinacker", "Oberfeld"]
l_dauer += [919,1011,1102,1209,1308,1400]
\end{lstlisting}
\end{solution}

\question[2]{\emoji{train} Zugreise}
Schreiben Sie eine Definition \lstinline|reise_zu_bahnhof(start_bahnhof, stationen, anzahl_stopps)|, welche einen Startbahnhof (Text), eine Liste von Bahnhöfen und die Anzahl Stopps als Parameter nimmt und den neuen Bahnhof zurückgibt.
\begin{itemize}
\item Falls der Startbahnhof nicht Teil der Liste von Bahnhöfen (\lstinline|stationen|) ist, soll zurückgegeben werden \enquote{Kann Start-Bahnhof nicht in Stationen finden}.
\item Falls der Zielbahnhof weiter ist als möglich soll ebenfalls eine Fehlermeldung ausgegeben werden
\item Andernfalls soll die Definition den neuen Bahnhof ausgeben wo man ankommt nach \lstinline|anzahl_stopps|.
\end{itemize}
\begin{solution}
\begin{lstlisting}
def reise_zu_bahnhof(start_bahnhof, stationen, anzahl_stopps):
    if start_bahnhof not in stationen:
        return "Kann Start-Bahnhof nicht in Stationen finden"
    
    # teste, ob man überhaupt noch anzahl_stopps vorwärts fahren kann
    position_start = stationen.index(start_bahnhof, 0, len(stationen))
    if (position_start + anzahl_stopps) > len(stationen) - 1:
        return "Kann nicht so weit (vorwärts) reisen"

    # teste, ob man überhaupt noch anzahl_stopps vorwärts fahren kann
    if (position_start + anzahl_stopps) < 0:
        return "Kann nicht so weit (rückwärts) reisen"

    return stationen[position_start+anzahl_stopps]


stationen = ["Bern", "Zürich", "Zürich Oerlikon", "Zürich Flughafen", "Winterthur", "St. Gallen"]
neue_station = reise_zu_bahnhof("Zürich", stationen, -1)
print(neue_station)
\end{lstlisting}
content
\end{solution}
