...
\begin{frame}[fragile]{Methoden}
	\begin{lstlisting}
meine_liste = [1, 2, 3]
(*@\only<1>{meine\_liste}\only<2->{\textcolor{ForestGreen}{meine\_\tikzmark{arrowFrom}liste}}@*).append((*@\only<3->{ \tikzmark{boxStart}\textcolor{ForestGreen}{\textit{meine}\tikzmark{arrowTo}\textit{\_liste},}\tikzmark{boxEnd} }@*)4)
...
print(meine_liste)  # Ausgabe: [1, 2, 3, 4]	
	...
	\begin{tikzpicture}[overlay,remember picture]
		% Wert übergeben
		\only<3->{\draw[ForestGreen,-stealth, thick] (arrowFrom.south) to[bend right] node[align=left, pos=.5, below, fill=ForestGreen,fill opacity=.5, text opacity=1, text=black,rounded corners] {als Parameter übergeben} (arrowTo.south);}
		\only<3->{\draw[ForestGreen,thick,rounded corners, fill=ForestGreen,fill opacity=.5] (boxStart.south west) rectangle (boxEnd.north east);}
	\end{tikzpicture}
	\begin{itemize}
		\item<4-> Methoden beziehen sich auf das Objekt, auf dem sie aufgerufen werden (hier: \lstinline|meine_liste|)
		\item<5-> Methoden können Werte verändern, die in diesem Objekt gespeichert sind, ohne Gleichzeichen
		\item<6-> Erster Parameter einer Methode ist immer das Objekt, auf dem die Methode aufgerufen wird (hier: \lstinline|meine_liste|)
		\item<7-> Weitere Parameter können zusätzlich übergeben werden (hier: \lstinline|4|)
	\end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Auftrag}{Skript, Abschnitt 6.1.3}
	\begin{itemize}
		\item \readbox{Definition 6.2 \& 6.3, Beispiel 6.9}
