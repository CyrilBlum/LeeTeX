...
\begin{itemize}
    ...
    \item \textbf{Grosse Sprachmodelle}: ChatGPT, Claude, Gemini führen Konversationen
...
\begin{itemize}
    ...
    \item Das Netz ist in \textbf{Schichten} aufgebaut:
          Eingabeschicht $\rightarrow$ versteckte Schichten $\rightarrow$ Ausgabeschicht.
...
\begin{myanswer}
\begin{enumerate}
    \item Person A: Ausgabe $= 0.8 \cdot 1 + 0.3 \cdot 0 = 0.8$ $\rightarrow$ Ja
    \item Person B: Ausgabe $= 0.8 \cdot 0 + 0.3 \cdot 1 = 0.3$ $\rightarrow$ Nein
    \item Person C: Ausgabe $= 0.8 \cdot 0 + 0.3 \cdot 0 = 0$ $\rightarrow$ Nein
\end{enumerate}
Wenn man $w_2$ auf 0.6 erhöht, würde Person B eine Ausgabe von $0.6$ erhalten, was immer noch unter der Schwelle von $0.5$ liegt, also würde das Neuron weiterhin \enquote{Nein} sagen. Allerdings wäre die Entscheidung für Person B jetzt näher an der Schwelle, was bedeutet, dass die Vorliebe für Sport einen stärkeren Einfluss auf die Kaufentscheidung hätte.
\end{myanswer}
\end{myexercise}
...
\begin{mydefinition}
Das \textbf{Kontextfenster} (context window) bestimmt, wie viel Text (gemessen in Tokens) ein LLM auf
...
das entspricht etwa 1500 Buchseiten oder 30'000 Zeilen Code (\href{https://gemini.google/overview/long-context/}{Link: Gemini}).
\end{mydefinition}
...
\begin{mychallenge}[Erkunden Sie einen Tokenizer]
