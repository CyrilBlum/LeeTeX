...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[auto]
    ...
    \node[relationship] (has) at (7,0) {has};
...
\begin{itemize}
  \item \textbf{Eins-zu-Eins (1:1)}: Dies wird normalerweise angezeigt, indem eine \enquote{1} in der Nähe beider Entitäten platziert wird, die durch eine Beziehung verbunden sind. Es bedeutet, dass eine Instanz einer Entität mit genau einer Instanz einer anderen Entität assoziiert ist.
  \item \textbf{Eins-zu-Viele (1:n)}: Dies wird dargestellt, indem eine \enquote{1} in der Nähe der Entität auf der 'eins'-Seite der Beziehung und ein \enquote{n} oder \enquote{m} (für \enquote{\textit{many}}=viele) in der Nähe der Entität auf der \enquote{viele}-Seite der Beziehung platziert wird. Es zeigt an, dass eine Instanz der ersten Entität mit null, einer oder mehreren Instanzen der zweiten Entität assoziiert sein kann, aber eine Instanz der zweiten Entität nur mit einer Instanz der ersten Entität assoziiert sein kann.
  \item \textbf{Viele-zu-Viele (m:n)}: Dies wird gezeigt, indem ein \enquote{m} oder \enquote{n} in der Nähe beider Entitäten platziert wird. Es zeigt an, dass Instanzen der ersten Entität mit null, einer oder mehreren Instanzen der zweiten Entität assoziiert sein können und umgekehrt.
\end{itemize}
...
\begin{myexercise}
  ...
  Die Schule hat entschieden, ein Entity-Relationship-Modell (ERM) zu entwerfen, um die Beziehungen zwischen diesen Entitäten klar darzustellen (siehe \cref{fig:erm-school}).
  ...
  \begin{myanswer}
    ...
    Statt \texttt{0:1} sollte also \texttt{m} stehen zwischen \enquote{unterrichtet} und \enquote{Klasse}.
    Zudem sollten \texttt{m} und \texttt{1} zwischen \textit{Klasse} und \textit{Schüler} getauscht werden.
  \end{myanswer}
\end{myexercise}

\begin{myexercise}
  Ein Krankenhaus möchte seine Datenbankstruktur neu organisieren, um Informationen über Ärzte, Patienten und Behandlungen effizienter zu verwalten. Das Krankenhaus hat sich entschieden, ein Entity-Relationship-Modell (ERM) zu entwerfen, um die Beziehungen zwischen diesen Entitäten klar darzustellen (siehe \cref{fig:erm-hospital}).
  \begin{figure}[H]
    \centering
    \begin{tikzpicture}
      % Entitäten
...
\begin{table}[H]
  ...
  \begin{tabular}{a|l|l}
    ...
    ...                         & ...      & ...
  \end{tabular}
  \caption{Beispiel-Tabelle: \enquote{Angestellte}}
...
In diesem Beispiel hat die Tabelle \enquote{Angestellte} nicht nur einen Primärschlüssel \texttt{mID}, sondern auch einen Fremdschlüssel \texttt{aID} (hervorgehoben mit dem Symbol \faKey), der auf den Primärschlüssel mit demselben Name der Tabelle \enquote{Abteilungen} verweist. Angenommen wir möchten nun (mit SQL, nicht mit manuellem Nachschauen) herausfinden, in welcher Abteilung die Person \enquote{Kaufmann} arbeitet, wie lassen sich nun die Informationen aus beiden Tabellen kombinieren?



\subsection{\texorpdfstring{Tabellen verbinden mit oder ohne \lstinline|JOIN|}{Tabellen verbinden mit oder ohne JOIN}}

\begin{myexample}
Eine erste Möglichkeit, \cref{table:angestellte2} mit \cref{table:abteilungen} zu verbinden, besteht darin, einfach beide Tabellen \enquote{aufzurufen}:
\begin{lstlisting}
SELECT *
...
Die Tatsache, dass unsere Abfrage mehrere Kolonnen \texttt{aID} zurückgibt, mag etwas unschön sein. Dies könnte umgangen werden, indem wir ein \lstinline|JOIN ... USING|-Konstrukt verwenden.
...
\begin{mycaution}
  Die Klammer um \texttt{aID} muss gesetzt werden, damit der \lstinline|USING|-Befehl funktioniert.
\end{mycaution}
...
\begin{myremark}
  Nach einer Tabellenverbindung können Sie alles machen, was Sie sonst auch tun: filtern mit \lstinline|WHERE|, gruppieren mit \lstinline|GROUP BY|, summieren mit \lstinline|SUM| etc.
\end{myremark}
...
\section{\texorpdfstring{\lstinline|JOIN|-Typen}{JOIN-Typen}}
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[b]{0.45\textwidth}
    ...
    \begin{tikzpicture}
      ...
      \end{scope}
    \end{tikzpicture}
    \caption{\lstinline|RIGHT OUTER JOIN|}
  \end{subfigure}
  \hfill
  % FULL OUTER JOIN
  \begin{subfigure}[b]{0.45\textwidth}
    ...
    \begin{tikzpicture}
      ...
      \fill[gray] (1.5,0) circle (1.25cm);
    \end{tikzpicture}
    \caption{\lstinline|FULL OUTER JOIN|}
  \end{subfigure}
...
\subsection{\texorpdfstring{\lstinline|INNER JOIN| (=\lstinline|JOIN|)}{INNER JOIN (=JOIN)}}
...
\end{lstlisting}
Hier verbinden wir zwei Tabellen, indem wir zwei Kolonnen \texttt{kolonne\_id1} und \texttt{kolonne\_id2}, die derselben Information entsprechen (beispielsweise die user-ID), miteinander abgleichen. Beim \lstinline|INNER JOIN| werden nur die Zeilen aus \texttt{tabelle1} retourniert, für die es einen entsprechenden Wert in \texttt{kolonne\_id2} in \texttt{tabelle2} gibt. Alle Werte in \texttt{tabelle1} die sich mit keinem Wert in \texttt{tabelle2} überschneiden, sowie umgekehrt, tauchen nicht im Resultat auf (siehe \cref{fig:venn-join}).
...
Wie Sie sehen, kann es hilfreich sein, im \lstinline|SELECT|-Ausdruck explizit die Tabelle zu nennen, von welcher eine bestimmte Kolonne stammt.
\end{myexample}

Auch alle anderen SQL-Befehle wie beispielsweise \lstinline|GROUP BY| können nach einem \lstinline|JOIN|-Befehl verwendet werden.
...
\begin{mycaution}
  Wie Sie in \cref{myexample:join-group} sehen, können Kolonnen jederzeit mit dem Befehl \lstinline|AS "Neuer Name"| umbenannt werden. Allerdings gilt es zu beachten, dass Abstände im Namen eher ungünstig sind: Falls der Name später wiederverwendet wird, wie etwa in einem \lstinline|ORDER BY|-Ausdruck, müssen sogenannte \textit{backticks} (\lstinline|`|) vor und nach dem Namen gesetzt werden.
\end{mycaution}
...
\subsection{\texorpdfstring{\lstinline|LEFT JOIN| (=\lstinline|LEFT OUTER JOIN|)}{LEFT JOIN (=LEFT OUTER JOIN)}}
Ein \lstinline|LEFT JOIN|, oder \lstinline|LEFT OUTER JOIN| unterscheidet sich von einem \lstinline|INNER JOIN| dadurch, dass \textbf{alle} Einträge der ersten Tabelle (nach dem \lstinline|SELECT|-Befehl) in der neuen Tabelle erscheinen. Werte aus der zweiten Tabelle, die nicht in der ersten sind, werden jedoch nicht angezeigt (siehe \cref{fig:venn-left-join}).
...
\begin{myexercise}
  Diejenigen BenutzerInnen, die noch keine Fotos hochgeladen haben, sollen per Email dazu aufgefordert werden, Fotos hochzuladen. Finden Sie alle users, die noch keine Fotos hochgeladen haben. Geben Sie deren Name und Email-Adresse aus. Verwenden Sie statt \lstinline|WHERE| den Befehl \lstinline|HAVING|.
  \begin{myanswer}
    \begin{lstlisting}
...
\subsection{\texorpdfstring{\lstinline|RIGHT JOIN| (=\lstinline|RIGHT OUTER JOIN|)}{RIGHT JOIN (=RIGHT OUTER JOIN)}}
Ein \lstinline|RIGHT JOIN| funktioniert analog zu einem \lstinline|LEFT JOIN| (siehe \cref{fig:venn-right-join}).
...
\subsection{\texorpdfstring{\lstinline|FULL JOIN| (=\lstinline|FULL OUTER JOIN|)}{FULL JOIN (=FULL OUTER JOIN)}}
Bei einem \lstinline|FULL (OUTER) JOIN| werden alle Werte aus der ersten und der zweiten Tabelle angezeigt (siehe \cref{fig:venn-outer-join}).
...
\end{figure}
In InstaHub funktioniert der \lstinline|FULL (OUTER) JOIN| nicht, weshalb es an dieser Stelle keine Übungen hierzu gibt.
