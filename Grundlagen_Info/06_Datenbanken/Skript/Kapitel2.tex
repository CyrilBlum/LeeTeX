...
Das ``\%''-Zeichen ist eine sogenannte \textit{wildcard} und dient hier als Platzhalter, der Folgendes bedeutet: ``es \textit{kann} noch weiterer Text an dieser Stelle stehen, \textit{muss} aber nicht'' (siehe \cref{table:wildcards}).
...
\begin{myexercise}
  Verändern Sie den SQL-Befehl so, dass die Werbung auch für Fotos mit dem Hashtag ``\texttt{\#Wasser}'' angezeigt werden. Testen Sie danach, ob es geklappt hat, indem Sie Fotos mit diesem Hashtag suchen. Sie können auf Ihrer Instahub-Webseite überprüfen, ob es geklappt hat:
