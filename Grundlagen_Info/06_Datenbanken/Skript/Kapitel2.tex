...
Fügen wir zu einer \gls{SQL}-Abfrage den Befehl \enquote{\lstinline|WHERE conditions|} hinzu, können wir das erhaltene Resultat nach gewissen Bedingungen (\texttt{conditions}) filtern.
...
\begin{table}[H]
  ...
  \begin{tabular}{c|l}
    ...
    \lstinline|IS NULL|                & Wert existiert nicht (ist leer)                        \\
  \end{tabular}
\end{table}
...
Wie Sie anhand der \cref{myexample:in,myexample:between} sehen, spielt es in SQL eine Rolle, ob  wir nach Zahlen oder nach
  ...
  \lstinline|"Berlin"| in \cref{myexample:in}). Bei Zahlen dürfen Sie keine Anführungszeichen schreiben (siehe z.B. die Zahl
...
\end{myexercise}
\begin{myexercise}
  Geben Sie jede Benutzerrolle nur einmal aus. \texttt{dba} bedeutet \enquote{Database Administrator}, also Datenbank-AdministratorIn.
  \begin{myanswer}
    \begin{lstlisting}
...
Das \enquote{\%}-Zeichen ist eine sogenannte \textit{wildcard} und dient hier als Platzhalter, der Folgendes bedeutet: \enquote{es \textit{kann} noch weiterer Text an dieser Stelle stehen, \textit{muss} aber nicht} (siehe \cref{table:wildcards}).
...
\begin{myexample}
Mit folgendem Befehl erhalten wir alle Städte, die mit \enquote{H} beginnen und ein \enquote{m} an dritter Stelle haben:
...
\url{https://sql-tutorial.de/home/uebungen.php?lektion=1} (keine Abgabe notwendig / möglich)
    \item Geben Sie danach auf Moodle die \enquote{Übungen 1 (Einfache Selektion)} ab.
  \end{itemize}
\end{myexercise}
...
\begin{myexample}
Folgender Code berechnet die durchschnittliche Körpergrösse aller Personen, die Felix heissen:
...
\lstinline|LENGTH| gibt die Anzahl Zeichen eines Texts zurück.
...
\begin{myexercise}
  \begin{itemize}
    \item Lösen Sie Aufgaben 1--6 unter diesem folgendem \href{https://sql-tutorial.de/home/uebungen.php?lektion=2}{Link} (keine Abgabe notwendig / möglich)
    \item Geben Sie danach auf Moodle die \enquote{Übungen 2 (Aggregatsfunktionen)} ab.
  \end{itemize}
\end{myexercise}
...
\begin{mychallenge}
  Geben Sie die Stadtnamen aus, wo die meisten Mitglieder mit einem \enquote{b} im Namen wohnen (nur die ersten drei Zeilen).

  \textbf{Tipps:}
  \begin{itemize}
    \item Berechnen Sie zuerst mit einem \lstinline|GROUP BY|-Befehl die Anzahl Einwohner pro Stadt
    \item Beschränken Sie danach Ihre Abfrage mit dem \lstinline|WHERE|-Befehl auf Benutzer, die ein \enquote{b} im Namen haben
...
\begin{myexercise}
  \begin{itemize}
    \item Lösen Sie Aufgaben 1--5 unter diesem \href{https://sql-tutorial.de/home/uebungen.php?lektion=4}{Link}.
    \item Geben Sie danach auf Moodle \enquote{Übungen 3 (\lstinline|GROUP BY|)} ab.
  \end{itemize}
\end{myexercise}
...
\begin{myexercise}
  Berechnen Sie eine neue Kolonne, die den Text \enquote{langer Name} enthält, falls ein Name länger als 20 Zeichen lang ist, und ansonsten \enquote{kurzer Name}. BenutzerInnen mit langem Namen sollen zuoberst stehen. Zeigen Sie nur die ersten 5 Zeilen.
  \begin{myanswer}
    \begin{lstlisting}
SELECT name, LENGTH(name),
CASE
...
\begin{table}[H]
  ...
  \begin{tabular}{r|l}
    ...
    \hline
    \commandbox|browser_family|   & Browser (Firefox, Chrome, Safari...)                                       \\
    \hline
    ...
    \hline
    \commandbox|created_at|       & Zeitpunkt, als das Foto angesehen wurde                          \\
    \hline
...
\clearpage
Auf InstaHub können Werbeanzeigen mit \gls{SQL} personalisiert werden. Dafür geht man zunächst auf die Werbungs-Seite (siehe \cref{fig:screenshot-ads1})
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\linewidth]{Grundlagen_Info/06_Datenbanken/Figures/screenshot-ads1}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\linewidth]{Grundlagen_Info/06_Datenbanken/Figures/screenshot-ads2}
  \caption{Beispielkampagne}
  \label{fig:screenshot-ads2}
\end{figure}

Als erstes sehen wir den (frei wählbaren) Kampagnen-Namen \texttt{bergalm}. Wir sehen auch dass die Werbung vom Typ \enquote{banner} ist, sie wird also bei Fotos angezeigt. Nebst weiteren Informationen sehen wir insbesondere zuunterst die \enquote{SQL-Query}, also die SQL-Abfrage, mit der eine Werbung personalisiert wird.
...
Wir sehen zuerst, dass die Kolonne \texttt{description} der Tabelle \texttt{photos}, also die Kolonne, die Hashtags speichert, auf Texte wie \enquote{Natur}, \enquote{Landschaft} und \enquote{Berg} abgesucht wird. Falls so ein Text vorhanden ist in den Hashtags des Fotos, wird der Wert \texttt{true} zurückgegeben, und somit wird die Werbung potentiell angezeigt, andernfalls wird der Wert \texttt{false} zurückgegeben. Auf der letzten Zeile sehen wir eine Variable \lstinline|$photo|, mit welcher jedes Foto der gesamten Webseite nach den genannten Kriterien abgesucht werden kann.

\begin{myexercise}
  Verändern Sie den SQL-Befehl so, dass die Werbung auch für Fotos mit dem Hashtag \enquote{\texttt{\#Wasser}} angezeigt werden. Testen Sie danach, ob es geklappt hat, indem Sie Fotos mit diesem Hashtag suchen. Sie können auf Ihrer Instahub-Webseite überprüfen, ob es geklappt hat:

  \url{[ihrefarbe].instahub.org/p/1517}
...
\begin{mychallenge}
  Erstellen Sie eine neue Werbekampagne (siehe \cref{fig:screenshot-ads1}).
\end{mychallenge}
...
\begin{mychallenge}
  ...
  \begin{itemize}
    ...
    \item \textcolor{ForestGreen}{BenutzerInnen}: Gehen Sie auf die von der Administratorin erstellte Webseite (\url{https://namedesnetzwerks.instahub.org}, wobei \enquote{\texttt{namedesnetzwerks}} ersetzt werden muss) und erstellen Sie ein neues Konto für sich.
    \item \textcolor{RoyalBlue}{AdministratorIn}: Die neuen Accounts der \textcolor{ForestGreen}{BenutzerInnen} muss dann von Ihnen als \textcolor{RoyalBlue}{AdministratorIn} aktiviert werden, indem Sie den User suchen, auf \enquote{Bearbeiten} und dann ganz unten auf \enquote{Account ist freigeschaltet} klicken.
    ...
    \item Erstellen Sie nun personalisierte Werbungen für Ihre KlassenkameradInnen (siehe \cref{fig:screenshot-ads1}).
  \end{itemize}
\end{mychallenge}
