...
Mit SQL kann man Daten nicht nur abfragen, sondern auch erstellen, verändern und löschen. Im Folgenden schauen wir uns die dafür nötigen Befehle an.
...
\begin{itemize}
  ...
  \item \lstinline|TINYINT(1)| speichert den Wert 0 oder 1, wobei 0 typischerweise \enquote{falsch} und 1 \enquote{wahr} bedeuten.
  \item \lstinline|DATETIME| ist ein Datum mit einer Uhrzeit
  \item \lstinline|VARCHAR(laenge)| bezeichnet eine Zeichenkette, d.h. ein Text, der mit maximal \texttt{laenge} vielen Bytes oder Zeichen kodiert wird.
  \item \lstinline|TEXT| bezeichnet eine Zeichenkette mit maximaler Länge von 65'535 bytes.
  \item \lstinline|ENUM('wert1', 'wert2',...)| bezeichnet eine Auflistung (engl. \textit{enumeration}) möglicher Werte. Beispielsweise können nur \enquote{male} und \enquote{female} im Feld \texttt{gender} eingetragen werden.
\end{itemize}

Zudem können folgende Eigeschaften für Kolonnen angegeben werden:
...
\begin{itemize}
  ...
  \item \lstinline|PRIMARY KEY|: Primärschlüssel (siehe \cref{table:table-cia}).
  \item \lstinline|FOREIGN KEY (kolonneID) REFERENCES tabelle2(kolonneID2)|: Fremdschlüssel (siehe \cref{table:table-cia}).
...
\end{mycaution}
	...
	\begin{mycaution}
Auf InstaHub kann immer nur ein Befehl pro Mal ausgeführt werden, das \enquote{;}-Zeichen funktioniert also leider nicht. Stattdessen müssen Sie in InstaHub jeden Befehl einzeln eingeben.
	\end{mycaution}
...
\end{myexample}
\begin{mycaution}
Beim Umbenennen einer Kolonne können Datenbankschemen (z.B. Fremdschlüssel-Referenzen) zerstört werden, z.B. falls ein Primärschlüssel umbenannt wird, welcher von einer anderen Tabelle per Fremdschlüssel referenziert wird (siehe \cref{subsec:create-table}).
\end{mycaution}

\subsection{Tabellen löschen: \texorpdfstring{\lstinline|DROP TABLE|}{DROP TABLE}}
Mit dem Befehl \lstinline|DROP TABLE tabellenname| kann man eine Tabelle komplett löschen.
...
\begin{myexercise}
  Erstellen Sie auf InstaHub eine Tabelle \texttt{users2} mit denselben Eigenschaften wie \texttt{users} (siehe Beispiel \ref{ex:sql-create})
  \begin{myanswer}
    \begin{lstlisting}
CREATE TABLE users2 (
  `id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
...
\begin{myexercise}\label{aufgabe:evalPoly}
  ...
  Ersetzen Sie die Stadt \enquote{Berlin} überall durch \enquote{Bern}
  \begin{myanswer}
    \begin{lstlisting}
UPDATE users
SET city="Bern"
