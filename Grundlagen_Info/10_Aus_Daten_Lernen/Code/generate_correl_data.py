...
if __name__ == "__main__":
    ...
    for r in correlations:
        ...
        plt.savefig(
            f"Grundlagen_Info/10_Aus_Daten_Lernen/Figures/correlation_{r}.pdf",
            bbox_inches="tight",
            pad_inches=0,
            transparent=True,
    ...
    generiere_korrelationsdaten("berufserfahrung_vs_einladung", n=20).to_csv(
        "Grundlagen_Info/10_Aus_Daten_Lernen/Data/berufserfahrung_vs_einladung.csv",
        index=False,
    )
    generiere_korrelationsdaten("socialmedia_vs_zufriedenheit", n=20).to_csv(
        "Grundlagen_Info/10_Aus_Daten_Lernen/Data/socialmedia_vs_zufriedenheit.csv",
        index=False,
    )
    generiere_korrelationsdaten("haarlaenge_vs_note", n=14, seed=1).to_csv(
        "Grundlagen_Info/10_Aus_Daten_Lernen/Data/haarlaenge_vs_note.csv", index=False
    )
    generiere_korrelationsdaten("schlafdauer_vs_note", n=25, seed=1).to_csv(
        "Grundlagen_Info/10_Aus_Daten_Lernen/Data/schlafdauer_vs_note.csv", index=False
    )
