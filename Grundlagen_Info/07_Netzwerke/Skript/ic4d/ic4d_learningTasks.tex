...
client.close()
    ...
    \begin{figure}[H]
        \begin{tikzpicture}[
            ...
            % Nodes
            \node[client] (client) at (0, 0) {Client\\\texttt{localhost:54321}};
            \node[server] (server) at (10, 0) {Server\\\texttt{localhost:12345}};

            % Arrows with labels
            \draw[arrow, loop, min distance=2cm, in=120, out=60] (server.north) to node[above, yshift=8pt, code] {bind(), listen(), accept()} (server.north);
            \draw[arrow] (client) to[bend left=20] node[above, yshift=3pt, code] {connect()} (server);
            \draw[arrow] (client) -- node[below, code] {send("Hallo Server...")} (server);
            \draw[arrow, dashed] (server) -- node[above, yshift=3pt, code, xshift=10pt] {recv()} (client);
            \draw[arrow, dashed, bend left=20] (server.south) to node[below, yshift=-8pt, code, xshift=-10pt] {send("Hallo Client...")} (client.south);
...
\begin{myexercise}
    Erweitern Sie nun Ihr Programm, indem der Client eine Nachricht schickt, und der Server antwortet mit \enquote{Nachricht erhalten!}. Sie können hierfür den Befehl \lstinline|send()| verwenden, um die Antwort zu senden, und \lstinline|recv()|, um die Antwort im Client zu empfangen und anzuzeigen.
\end{myexercise}
