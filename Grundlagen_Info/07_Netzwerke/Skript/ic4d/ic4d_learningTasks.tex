...
client.close()
     \end{lstlisting}
        ...
        \end{figure}
 Ein Client kann auch innerhalb eines Programms mehrere Sockets öffnen, um mit verschiedenen Servern oder Diensten gleichzeitig zu kommunizieren. Jeder Socket repräsentiert eine separate Verbindung. Im folgenden Beispiel sehen Sie, wie ein Client zwei Sockets öffnet, um bidirektional zu kommunizieren:
        \begin{figure}[H]
            \begin{tikzpicture}[
                    base/.style={rectangle, draw, minimum width=2.5cm, minimum height=1cm, text=white, rounded corners, text width=2.8cm, align=center, inner sep=8pt},
                    client/.style={base, fill=ForestGreen},
                    socket/.style={circle, draw, fill=yellow!70, minimum size=0.6cm}
                ]

                \node[client] (client) at (5, 0) {Client};
                \node[socket] (socket1) at (3.5, 0) {};
                \node[socket] (socket2) at (6.5, 0) {};

                \draw[->] (socket1) to[bend left=60] (socket2);
                \draw[->] (socket2) to[bend left=60] (socket1);
