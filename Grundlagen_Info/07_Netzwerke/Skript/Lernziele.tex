\chapter{Lernziele: Rechnernetze}
\begin{todolist}
\item Ich kann den Unterschied zwischen Internet und Rechnernetz erklären und Beispiele für verschiedene Netzwerke nennen.
\item Ich kann die Funktionsweise von \gls{IP}, \gls{IP}v4 und \gls{IP}v6 erklären und die Adressräume vergleichen.
\item Ich kann die Erreichbarkeit eines Hosts mit \commandbox*|ping| prüfen und typische Diagnoseschritte durchführen.
...
\item Ich kann die grundlegenden Konzepte von \gls{TCP} beschreiben und dessen Einsatz im Kontext von Netzwerkprotokollen verstehen.
...
\item Ich kann grundlegende \gls{HTML}-Strukturen erstellen und verstehen, wie \gls{HTML}-Seiten über \gls{HTTP} übertragen werden.
...
\item Ich kann die Rolle eines \gls{DNS}-Servers im Netzwerk erklären und wie \gls{DNS}-Anfragen bearbeitet und beantwortet werden.
...
\item Ich kann eine Chat-Verbindung im \gls{LAN} einrichten und notwendige Einstellungen (Ports) berücksichtigen.
 \item Ich kann die Funktionsweise eines Relay-Servers erklären und einfache Nachrichtenformate definieren.
 \item Ich kann mit Netzwerkanalyse-Tools (z.B. Wireshark) gängige Protokolle in einem Abruf identifizieren (\gls{DNS}, \gls{TCP}, \gls{HTTP}).
