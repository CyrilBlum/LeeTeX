...
% TODO Abschnitt erweitern, Bilder einfügen, Quellenangaben, etc.
...
\begin{figure}[H]
	...
	\caption{Ausbreitung des \gls{ARPANET} von 1969 bis 1977 (\href{http://www.fibel.org/linux/lfo-0.6.0/node457.html}{Quelle})}
\end{figure}

\subsubsection{Die Entwicklung von Protokollen: \acrshort{TCP}/\acrshort{IP}}
...
Eine \gls{URL} (Uniform Resource Locator) ist eine eindeutige Adresse, die auf eine Ressource (meiste eine Datei) im Internet verweist. Sie besteht aus mehreren Teilen, darunter das Protokoll (\texttt{http://} oder \texttt{https://}), der Domain-Name (\texttt{www.beispielseite.ch}) und der Pfad zur Datei (\texttt{/unterseite.html}).
...
\begin{figure}[H]
	...
	\caption{Sir Tim Berners-Lee, der Erfinder des World Wide Web, 2014 (\href{https://de.wikipedia.org/wiki/Tim_Berners-Lee}{Quelle})}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{Grundlagen_Info/07_Netzwerke/Figures/internet_use_by_income_class.pdf}
	\caption{Entwicklung der weltweiten Internetnutzung nach Einkommensklasse (\href{https://data.worldbank.org/indicator/IT.NET.USER.ZS}{Quelle})}
	\label{fig:internet_use_by_income_class}
\end{figure}

Wie auf \cref{fig:internet_use_by_income_class} zu sehen ist, hat sich die Internetnutzung in den letzten Jahren stark erhöht. Insbesondere in einkommensstarken Ländern ist die Internetnutzung nahezu flächendeckend, während in einkommensschwachen Ländern noch grosse Unterschiede bestehen.
