...
\chapter{Methodische Anleitung zur Durchführung der Prüfungsaufgaben}

\begin{mainBox}[title=Methodische Anleitung zur Durchführung der Prüfungsaufgaben]

    \small

    \textbf{\large 1. Theorieaufgaben (IP-Adresse, NAT, Client-Server)}
    \begin{itemize}[topsep=2pt, itemsep=2pt]
        \item \textbf{Schritt 1:} Konzepte wiederholen $\rightarrow$ Mindmap oder Tabelle mit Definitionen anfertigen
        \item \textbf{Schritt 2:} Analogie aus dem Alltag finden (Hausadresse $\Leftrightarrow$ IP, Apartment-Nummer $\Leftrightarrow$ Port)
        \item \textbf{Schritt 3:} Schichtenmodell verknüpfen $\rightarrow$ Auf welcher Schicht passiert das? (Transport/Netzwerk-Schicht)
        \item \textbf{Schritt 4:} Präzise Fachsprache prüfen $\rightarrow$ Fehler? Lücken?
    \end{itemize}

    \textit{Meine Analogie:} \underline{\hspace{9cm}}

    \textbf{\large 2. Debugging-Aufgaben (Fehleranalyse, Firewall)}
    \begin{itemize}[topsep=2pt, itemsep=2pt]
        \item \textbf{Schritt 1:} Fehlertyp einordnen (Software vs. Infrastruktur)
        \item \textbf{Schritt 2:} Ursachen systematisch sammeln: 3+ Varianten mit Begründung
        \item \textbf{Schritt 3:} Zu jeder Ursache eine testbare Lösung formulieren (z.B. Befehl, Einstellung)
        \item \textbf{Schritt 4:} Lösungsweg nachvollziehen: Würde das das Problem beheben?
    \end{itemize}

    \textit{Häufigster Fehler und Lösung:} \underline{\hspace{7cm}}

    \textbf{\large 3. Wissensaufgaben (Lückentext, Zuordnung, Netzwerkbefehle)}
    \begin{itemize}[topsep=2pt, itemsep=2pt]
        \item \textbf{Schritt 1:} Relevante Kapitel (Schichtenmodell, Netzzugriff, Befehle) durcharbeiten
        \item \textbf{Schritt 2:} Fachbegriffe korrekt verwenden (z.B. \enquote{Routing-Tabelle}, nicht \enquote{Weitergabe})
        \item \textbf{Schritt 3:} Lücke/Zuordnung im Kontext prüfen $\rightarrow$ Passt die Antwort grammatisch?
        \item \textbf{Schritt 4:} Verknüpfung zu Learning Tasks herstellen: Wo haben wir das gelernt?
    \end{itemize}

    \textit{Mein wichtigster Fachbegriff:} \underline{\hspace{8cm}}

    \textbf{\large 4. Programmieraufgaben (Chat-Client-Erweiterung)}
    \begin{itemize}[topsep=2pt, itemsep=2pt]
        \item \textbf{Schritt 1:} Anforderungen verstehen $\rightarrow$ Was ist gefordert? (mehrere Clients, Threading, Relay?)
        \item \textbf{Schritt 2:} Architektur skizzieren $\rightarrow$ Server: Wie verwalte ich mehrere Verbindungen?
        \item \textbf{Schritt 3:} Code strukturiert schreiben $\rightarrow$ Funktionen: \texttt{connect}, \texttt{listen}, \texttt{forward}, \texttt{handle\_error}
        \item \textbf{Schritt 4:} Zwischentests (\textit{unit tests}) durchführen $\rightarrow$ Funktioniert Mehrfachverbindung? Namensregistrierung?
        \item \textbf{Schritt 5:} Testen mit 2--3 Clients $\rightarrow$ Error Handling implementiert? Sockets geschlossen?
    \end{itemize}

    \textit{Meine Architektur-Skizze:} \\
    \vspace{5cm}
    \newpage
    \textbf{\large 5. Projektarbeit (Debugging + Relay-Server)}
    \begin{itemize}[topsep=2pt, itemsep=2pt]
        \item \textbf{Phase 1 -- Analyse:} Fehlerlogbuch anlegen $\rightarrow$ Symptom, vermutete Ursache, Test, Lösung
        \item \textbf{Phase 2 -- Umsetzung:} Rollen verteilen (Server-Lead, Client-Lead, Test-Lead) $\rightarrow$ täglich synchronisieren
        \item \textbf{Phase 3 -- Dokumentation:} Fehler-Bericht, Architektur-Diagramm, Test-Protokoll
        \item \textbf{Phase 4 -- Präsentation:} Live-Demo (mehrere Clients chatten), Architektur erklären
    \end{itemize}

    \textit{Unsere Rollenverteilung:} 
    
    Person A: \underline{\hspace{2.5cm}}, Person B: \underline{\hspace{2.5cm}}, Person C: \underline{\hspace{2.5cm}}

    \textbf{\large 6. Reflexions- \& Analyse-Aufgaben (Wireshark, Datenschutz)}
    \begin{itemize}[topsep=2pt, itemsep=2pt]
        \item \textbf{Schritt 1:} Protokoll-Chronologie verstehen $\rightarrow$ Welcher Schritt passiert zuerst? (\gls{DNS} $\rightarrow$ \gls{TCP} $\rightarrow$ \gls{HTTP})
        \item \textbf{Schritt 2:} Daten-Datenschutz-Risiken benennen $\rightarrow$ Meta-Daten, Zugriff, Monopol-Stellung
        \item \textbf{Schritt 3:} Alternativen recherchieren $\rightarrow$ \gls{P2P}, Verschlüsselung (z.B. Signal)
        \item \textbf{Schritt 4:} Konkrete Massnahmen formulieren $\rightarrow$ Datenminimierung, Logging-Strategie
    \end{itemize}

    \textit{Mein wichtigster Datenschutz-Gedanke:} \underline{\hspace{7cm}}
