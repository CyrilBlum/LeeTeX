...
\begin{myexercise}
	Schauen Sie sich das Datenaustausch-Fenster von \cref{ex:filius-echo-server} genauer an. Erklären Sie, was auf jeder Zeile passiert.
	...
	\begin{myanswer}
		\begin{enumerate}
			\item \textbf{Zeile 1 (ARP)}: Der Client (192.168.0.10) sendet eine ARP-Anfrage (\textit{Broadcast}) an alle Geräte im Netzwerk, um die MAC-Adresse des Servers (192.168.0.11) zu finden.
			\item \textbf{Zeile 2 (ARP)}: Der Server (192.168.0.11) antwortet mit seiner MAC-Adresse (86:77:1C:6A:51:D7) direkt an den Client (\textit{Unicast}).
			\item \textbf{Zeile 3 (TCP)}: Der Client startet den Drei-Wege-Handschlag mit einem SYN-Paket (Sequenznummer: 2.000.000), um eine TCP-Verbindung zum Server aufzubauen.
			\item \textbf{Zeile 4 (TCP)}: Der Server antwortet mit SYN+ACK (eigene Sequenznummer: 1.000.000, Bestätigung: 2.000.001), um die Verbindung zu bestätigen und seine eigene Sequenznummer mitzuteilen.
			\item \textbf{Zeile 5 (TCP)}: Der Client sendet ein ACK-Paket (Sequenznummer: 2.000.001, Bestätigung: 1.000.001), um den Drei-Wege-Handschlag abzuschliessen. Die TCP-Verbindung ist nun etabliert.
			\item \textbf{Zeile 6 (Anwendung)}: Der Client sendet die Nachricht \enquote{Hallo!} über die etablierte TCP-Verbindung an den Echo-Server.
			\item \textbf{Zeile 7 (TCP)}: Der Server bestätigt den Empfang der Nachricht mit einem ACK-Paket (Sequenznummer: 1.000.001, Bestätigung: 2.000.008). Die Bestätigungsnummer ist um 8 erhöht worden, da eine Nachricht mit 8 Bytes empfangen wurde.
			\item \textbf{Zeile 8 (Anwendung)}: Der Echo-Server sendet die Nachricht \enquote{Hallo!} unverändert zurück an den Client.
			\item \textbf{Zeile 9 (TCP)}: Der Client bestätigt den Empfang der Echo-Nachricht mit einem ACK-Paket (Sequenznummer: 2.000.008, Bestätigung: 1.000.008).
		\end{enumerate}
	\end{myanswer}
\end{myexercise}
