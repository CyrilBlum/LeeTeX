...
Bevor wir uns näher mit den Dimensionen der Anwendungsschicht sowie der darunterliegenden Schichten befassen, sollten wir zuerst einige häufig auftauchende Begriffe klären, von denen im Kontext von Rechner-Netzen häufig die Rede ist.
...
Damit eine \gls{IP}-Adresse in eine \gls{MAC}-Adresse umgewandelt werden kann, wird ein weiteres Protokoll benötigt, das \gls{ARP}. Dieses wird jedes Mal ausgeführt, wenn ein Computer eine Nachricht an eine bestimmte \gls{IP}-Adresse senden will, aber die dazugehörige \gls{MAC}-Adresse nicht kennt. In diesem Fall sendet der Computer eine Anfrage an alle Computer im selben Subnetz (Broadcast), um die \gls{MAC}-Adresse des gewünschten Empfängers zu erfahren. Der Computer mit der gesuchten \gls{IP}-Adresse antwortet dann mit seiner \gls{MAC}-Adresse (Unicast) und der sendende Computer kann nun die Nachricht an den Empfänger weiterleiten.
