...
Das Netzwerk aus \cref{example:subnet} könnte dezimal angegeben werden wie folgt: \texttt{159.233.1.0/255.255.255.0}, wobei der erste Teil die \gls{IP}-Adresse eines Hosts im Subnetz ist und der zweite Teil die Subnetzmaske angibt. 
...
\begin{myexercise}[\acrshort{CIDR}-Notation]
	Geben Sie die folgenden Subnetzmasken in \gls{CIDR}-Notation an und notieren Sie die möglichen Adressebereiche für beide Beispielnetze, also die kleinste und grösste mögliche \gls{IP}-Adresse im jeweiligen Subnetz. Notieren Sie zudem, wie viele Hosts in jedem Subnetz Platz haben.
	\begin{itemize}
		\item \textbf{Netz 1}: \texttt{192.168.1.0} mit Netzmaske \texttt{255.255.255.192}
		\item \textbf{Netz 2}: \texttt{10.0.0.0} mit Netzmaske \texttt{255.128.0.0}
	\end{itemize}
	...
	\begin{table}[H]
			\centering
			\begin{tabular}{l||m{2.5cm}|m{1.75cm}|m{2.5cm}|m{2.5cm}}
								&  \multirow{2}{2cm}{\textbf{\gls{CIDR}-\newline Schreibweise}}&\multirow{2}{2cm}{\textbf{Anzahl Hosts}} & \multicolumn{2}{l}{\textbf{Adressbereich}} \\\cmidrule{4-5}
								& & & \textbf{Start} & \textbf{Ende} \\
				\midrule\midrule
				\texttt{Netz 1}  & \fldT & \fldT& \fldT           & \fldT         \\
				\midrule
				\texttt{Netz 2} & \fldT & \fldT     & \fldT&\fldT       \\
			\end{tabular}
	...
	\begin{myanswer}
		\begin{table}[H]
			\centering
			\begin{tabular}{l||l|m{1.75cm}|l|l}
								&  \multirow{2}{2cm}{\textbf{\gls{CIDR}-\newline Schreibweise}}&\multirow{2}{1.75cm}{\textbf{Anzahl Hosts}} & \multicolumn{2}{l}{\textbf{Adressbereich}} \\\cmidrule{4-5}
																& & & \textbf{Start} & \textbf{Ende} \\
												\midrule\midrule
												\texttt{Netz 1}  & \texttt{192.168.1.0/26} & $2^{32-26} = 2^{6} = 64$& \texttt{192.168.1.0}           & \texttt{192.168.1.63}         \\
												\midrule
												\texttt{Netz 2} & \texttt{10.0.0.0/9} & $2^{32-9} = 2^{23} = 8\,388\,608$     & \texttt{10.0.0.0}              & \texttt{10.127.255.255}       \\
			\end{tabular}
			\caption{Subnetzmasken in CIDR-Notation mit Anzahl Hosts und Adressbereichen}
		\end{table}
	\end{myanswer}
