...
def plot_internet_use_by_income_class():
    ...
    meta = meta.rename(
        columns={"Country Code": "Country Code", "IncomeGroup": "IncomeGroup"}
    )
    ...
    merged = df_long.merge(
        meta, left_on="Country Code", right_on="Country Code", how="left"
    )
    ...
    merged["IncomeGroup"] = (
        merged["IncomeGroup"].map(income_class_de).fillna(merged["IncomeGroup"])
    )
    ...
    merged["IncomeGroup"] = pd.Categorical(
        merged["IncomeGroup"], categories=einkommen_order, ordered=True
    )

    # Group by year and income group, calculate mean internet use
    grouped = (
        merged.groupby(["Year", "IncomeGroup"], observed=True)["InternetUse"]
        .mean()
        .reset_index()
    )
    ...
    plt.ylim(0, 100)
    plt.savefig(
        "Grundlagen_Info/07_Netzwerke/Figures/internet_use_by_income_class.pdf",
        format="pdf",
        bbox_inches="tight",
    )
...
def plot_world_map_internet_use(year=None):
    ...
    if year is None:
        ...
        df["latest_year"] = df[value_vars].apply(
            lambda row: max(
                [y for y in value_vars if pd.notnull(row[y]) and row[y] != ""],
                default=None,
            ),
            axis=1,
        )
        df["InternetUse"] = df.apply(
            lambda row: row[row["latest_year"]] if row["latest_year"] else None, axis=1
        )
    ...
    # Load world geometry
    world = gpd.read_file(
        "Grundlagen_Info/07_Netzwerke/Skript/Code/Files/ne_110m_admin_0_countries/ne_110m_admin_0_countries.shp"
    )
    # Remove Antarctica
    ...
    merged["InternetUse_binned"] = pd.cut(
        merged["InternetUse"], bins=bins, include_lowest=True
    )
    ...
    plt.savefig(
        f"Grundlagen_Info/07_Netzwerke/Figures/internet_use_map_{max_year:.0f}.pdf",
        format="pdf",
        bbox_inches="tight",
        pad_inches=0,
    )
