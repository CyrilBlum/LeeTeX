...
def plot_internet_use_by_income_class():
    ...
    """
    # File paths
    internet_use_file = 'Grundlagen_Info/07_Netzwerke/Code/Files/API_IT.NET.USER.ZS_DS2_en_csv_v2_81221.csv'
    metadata_file = 'Grundlagen_Info/07_Netzwerke/Code/Files/Metadata_Country_API_IT.NET.USER.ZS_DS2_en_csv_v2_81221.csv'
...
def plot_world_map_internet_use(year=None):
    ...
    # Some country codes may not match, so merge on ISO_A3 (3-letter code)
    ...
    merged.plot(column='InternetUse', ax=ax, legend=True, cax=cax,
                legend_kwds={'label': '% Zugang zum Internet', 'orientation': 'vertical'},
                missing_kwds={"color": "lightgrey", "label": "Keine Daten"},
                cmap='YlGnBu', edgecolor='black', linewidth=0.2)
    ax.set_title(f'Zugang zum Internet weltweit ({df_map["Year"].iloc[0] if year else "letztes Jahr mit Daten"})')
    ax.set_axis_off()
    plt.tight_layout()
    plt.show()
