...
\begin{figure}
    \centering
    \includegraphics[width=1\textwidth]{Logos/FreeFlower.pdf.pdf}
...
\begin{itemize}[<+->]
    ...
    \item Seit dem Jahre 1946 gehört die Stadt zu Russland und wurde in \textit{Kaliningrad} umbenannt.
\end{itemize}
\end{frame}
...
\begin{block}{Die zentrale Frage:}
    Gibt es einen Weg durch Königsberg, bei dem man alle sieben Brücken \textbf{genau einmal} überquert?
\end{block}
\pause
\begin{itemize}[<+->]
    \item Falls ja: Ist auch ein \textbf{Rundweg} möglich (Startpunkt $=$ Endpunkt)?
    \item Wichtig: Man darf Stadtteile beliebig oft betreten, aber jede Brücke nur einmal nutzen!
\end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Formulierung des Brückenproblem}
\begin{mydefinition}[Königsberger Brückenproblem, in der Sprache von Graphen]
...
\begin{center}
    \aufgabebox{Lösen Sie die beiden Aufgaben 1.1 und 1.2 im Skript.}
\end{center}
\end{frame}
...
\begin{frame}[fragile]{2. Aufgabenblock (10 Minuten, ohne Besprechung)}
    \begin{itemize}[<+->]
        \item Studieren Sie die Definition der Eulerwege und Eulerkreise im Skript.
        \item \aufgabebox{Lösen Sie die drei Aufgaben 1.3, 1.4 und 1.5 im Skript.}
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{3. Aufgabenblock}
    \begin{itemize}[<+->]
        \item \aufgabebox{Lösen Sie die beiden Aufgaben 1.6 und 1.7 im Skript.}
        \item  Falls Sie schon fertig sind: Wenden Sie sich den Aufgaben 1.8 und 1.9 zu.
    \end{itemize}
\end{frame}
