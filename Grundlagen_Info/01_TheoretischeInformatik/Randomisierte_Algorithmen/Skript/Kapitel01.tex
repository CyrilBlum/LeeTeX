...
Bislang haben wir uns hauptsächlich mit deterministischen Algorithmen beschäftigt. Diese arbeiteten auf gleichen Eingaben immer genau gleich und durchliefen bei ihren Berechnungen immer die gleichen Schritte. Der nächste Rechenschritt des Programms war dabei immer (bereits von Anfang an) eindeutig festgelegt.
...
Man kann sich auch vorstellen, dass der randomisierte Algorithmus an mindestens einer Stelle eine Münze wirft und seine Berechnung / Entscheidung (zum Beispiel bei einem \lstinline|if|) abhängig vom Ausgang dieses zufälligen Münzwurfs macht.
...
\begin{myexercise}
	\begin{aenum}
		...
		\item Schreibe eine Python-Funktion \lstinline|def division_mit_rest(a, b)|, welche für gegebene positive ganze Zahlen $a$ und $b$, die Zahlen $q$ und $r$ berechnet und in einer Liste der Form \lstinline|[q, r]| zurückgibt.
	\end{aenum}
	\begin{myanswer}
		\begin{aenum}
			\item \begin{align*}
...
\begin{mychallenge}
	Schreibe eine Python-Funktion \lstinline|def Nummer(x)|, welche $\text{Nummer}(x)$ für einen gegebenen binären String (binäres Wort) $x$ berechnet.
	\begin{myanswer}
...
\begin{myexercise}\label{myexercise:sieb}
	Schreiben Sie eine Python-Funktion \lstinline|def sieb(n)|, welche den Algorithmus von Eratosthenes implementiert und alle Primzahlen $\leq n$ ausgibt. Vervollständigen Sie dazu die folgende Vorlage:
...
\end{lstlisting}
	\begin{myanswer}
		\lstinputlisting[language=Python,caption=\texttt{sieb.py},label=listing:sieb]{Grundlagen_Info/01_TheoretischeInformatik/Randomisierte_Algorithmen/Skript/Code/sieb.py}
	\end{myanswer}
\end{myexercise}

Mithilfe des Siebs von Eratosthenes, ist es nun ganz einfach, eine Funktion zu definieren, welche $\text{Prim}(n)$ berechnet. Dazu ändern wir den Befehl \lstinline|print(primzahlen)| in unserer Funktion \lstinline|sieb| ab zu \lstinline|return primzahlen| und können dann schreiben:
...
\begin{myexercise}[Implementation von Verbesserung 1]\label{myexercise:implementVerbesserung1}
	...
	\begin{myanswer}
		\lstinputlisting[language=Python,caption=\texttt{sieb\_verbesserung\_1.py},label=listing:sieb_verbesserung_1]{Grundlagen_Info/01_TheoretischeInformatik/Randomisierte_Algorithmen/Skript/Code/sieb_verbesserung_1.py}
	\end{myanswer}
\end{myexercise}
...
\begin{mychallenge}[Implementation von Verbesserung 2]\label{myexercise:implementVerbesserung2}
	...
	\begin{myanswer}
		\lstinputlisting[language=Python,caption=\texttt{sieb\_verbesserungen\_1\_und\_2.py},label=listing:sieb_verbesserungen_1_und_2]{Grundlagen_Info/01_TheoretischeInformatik/Randomisierte_Algorithmen/Skript/Code/sieb_verbesserungen_1_und_2.py}
	\end{myanswer}
\end{mychallenge}
...
\begin{myexercise}
	\begin{aenum}
		\item Bestimme $\text{Prim}(n)$ für $n = 10'000'000$ (zehn Millionen) mithilfe der Funktion \lstinline|prim| (aus \cref{myexercise:implementVerbesserung1} oder \cref{myexercise:implementVerbesserung2}) und vergleiche den Wert mit der Schätzung $n/\ln{n}$.
		...
		\item Schreibe eine Python-Funktion \lstinline|def prim_approx(n)|, welche $\text{Prim}(n)$ mithilfe der Abschätzung, welche von dem Primzahlsatz \ref{eq:primzahlsatz} geliefert wird, approximiert.

		\item Verwende die sehr schnelle Funktion \lstinline|prim_approx| anstelle von \lstinline|prim|, um den Wertebereich des Plots von $8\cdot 10^7$ auf $10^{10}$ zu erhöhen.
	\end{aenum}
	\begin{myanswer}
		\begin{aenum}
...
\begin{mychallenge}
	Schreibe eine Python-Funktion \lstinline|def Bin(n)|, welche $\text{Bin}(n)$ für eine natürliche Zahl $n$ berechnet.
	\begin{myanswer}
		\begin{lstlisting}[language=Python,numbers=none]
def Bin(n):
    if n == 0: return "0"
...
\begin{align*}
	A := \ceil{\log_2\lr{p + 1}} + \ceil{\log_2\lr{s + 1}}.
\end{align*}
Nun gilt aber
\begin{align*}
	\lr{s < p < n^2} \Rightarrow \lr{s + 1 < p + 1 \leq n^2} \Rightarrow \lr{\ceil{\log_2\lr{s + 1}} \leq \ceil{\log_2\lr{p + 1}} \leq \ceil{\log_2\lr{n^2}}},
\end{align*}
wobei wir verwendet haben, dass $\log$ und die Aufrundeklammer monoton wachsende Funktionen sind. Somit gilt nun für den Kommunikationsaufwand $A$:
\begin{align*}
	A = \ceil{\log_2\lr{p + 1}} + \ceil{\log_2\lr{s + 1}} \leq \ceil{\log_2\lr{n^2}} + \ceil{\log_2\lr{n^2}} = \\
	= 2\ceil{\log_2\lr{n^2}} = 2\ceil{2\log_2\lr{n}} \leq 4\ceil{\log_2\lr{n}},
\end{align*}
wobei wir verwendet haben, dass
\begin{align*}
	\ceil{mx} \leq m\ceil{x}
\end{align*}
für alle natürlichen Zahlen $m$ und reellen Zahlen $x>0$. Zusammengefasst gilt also:
\begin{align*}
	A \leq 4\ceil{\log_2\lr{n}}.
\end{align*}
...
\begin{myexercise}
	...
	\begin{myanswer}
		\begin{align*}
			A \leq                             &                           \\
			4\ceil{\log_2\lr{n}}               & =                         \\
			4\ceil{\log_2\lr{10^{16}}}         & \leq                      \\
			4\cdot 16\cdot\ceil{\log_2\lr{10}} & = 4\cdot 16\cdot 4 = 256.
		\end{align*}
	\end{myanswer}
\end{myexercise}
...
\begin{aenum}
	...
	\begin{align*}
		\frac{n-1}{\text{Prim}(n^2)}\approx \frac{n-1}{n^2/\ln(n^2)} < \frac{n}{n^2/\ln(n^2)} = \frac{\ln(n^2)}{n},
	\end{align*}
	wobei wir die Approximation $\text{Prim}(n)\approx n/\ln(n)$ verwendet haben\footnote{siehe \cref{mytheorem:primzahlsatz}, \cref{myexcursion:finiteN}}. Die Fehlerwahrscheinlichkeit des Kommunikationsprotokolls ist also höchstens $\ln(n^2)/n$, was für $n=10^{16}$ höchstens $0.736827\cdot 10^{-14}$ ist.
\end{aenum}
...
\begin{myexercise}\label{myexercise:protokoll}
	Verwende Deine Python-Funktionen \lstinline|sieb|, \lstinline|Bin| und \lstinline|Nummer| um das Kommunikationsprotokoll in Python zu implementieren.
	\begin{myanswer}
...
\iftoggle{exerciseonly}{}{%
	...
	\begin{myanswer}[myexercise:protokoll]
		Die Funktionen \lstinline|sieb|, \lstinline|Nummer| und \lstinline|Bin| haben wir schon geschrieben.
		Diese können wir nun hier verwenden.
		Die Funktion \lstinline|is_equal| wird nicht benötigt, kann aber verwendet werden um (deterministisch) zu prüfen, ob zwei Datensätze identisch sind oder nicht.
		\lstinputlisting[language=Python,caption=\texttt{protokoll.py},label=listing:protokoll]{Grundlagen_Info/01_TheoretischeInformatik/Randomisierte_Algorithmen/Skript/Code/protokoll.py}
	\end{myanswer}
