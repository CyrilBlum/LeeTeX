...
\begin{questions}
	...
	\question[2] Im Unterricht haben wir die Python \lstinline|sieb| wie in \cref{listing:sieb} implementiert.
	Schreiben Sie eine Python-Funktion \lstinline|def Prim(n)|, welche \lstinline|sieb| verwendet um die Funktion $\text{Prim}(n)$ für eine gegebene natürliche Zahl $n$ zu berechnen.
...
\end{lstlisting}
	...
	Ein Kommunikationsprotokoll hat eine Fehlerwahrscheinlichkeit von $r\in\io{0}{1}$. Wie oft muss das Protokoll \underline{mindestens} wiederholt werden, bis die gesamte Fehlerwahrscheinlichkeit (nach wiederholter Durchführung des Protokolls) kleiner oder gleich $w$ ist. Dabei gilt $w\in\io{0}{1}$.
	\begin{solution}
		Wir suchen die kleinste natürliche Zahl (Anzahl) $n$, sodass
		\begin{align*}
			r^n               & \leq w\iff                  \\
			\lr{e^{\ln(r)}}^n & \leq e^{\ln(w)}\iff         \\
			e^{\ln(r)n}       & \leq e^{\ln(w)} \iff        \\
			\ln(r)n           & \leq w \iff                 \\
			n                 & \geq \frac{\ln(w)}{\ln(r)},
		\end{align*}
		wobei Sie beachten müssen, dass wegen $r\in\io{0}{1}$ der Ausdruck $\ln(r)$ eine negative Zahl ist. Multiplizieren einer Ungleichung mit einer negativen Zahl führt zur \enquote{Umkehrung} der $\leq\leftrightarrow \geq$ Relation.
		Und somit ($n$ ist eine natürliche Zahl):
		\begin{align*}
