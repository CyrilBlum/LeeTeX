...
\end{lstlisting}
	...
	\begin{solution}
		Wir suchen die kleinste natürliche Zahl $n$, sodass:
		\begin{align*}
			r^n         & \leq w \quad \Big| \, \ln(\dots) \iff \tag{Logarithmieren} \\
			\ln(r^n)    & \leq \ln(w) \iff \\
			n \cdot \ln(r) & \leq \ln(w)
		\end{align*}
		Da $r \in \io{0}{1}$ gilt, ist $\ln(r)$ negativ ($\ln(r) < 0$). Bei der Division durch eine negative Zahl dreht sich das Relationszeichen um:
		\begin{align*}
			n & \geq \frac{\ln(w)}{\ln(r)}
		\end{align*}
		Da $n$ eine natürliche Zahl sein muss, folgt durch Aufrunden:
		\begin{align*}
