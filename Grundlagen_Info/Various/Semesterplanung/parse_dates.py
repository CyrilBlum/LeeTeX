...
df["Startdatum"] = pd.to_datetime(
    df["Startdatum"], format="%d.%m.%Y", errors="coerce"
).dt.normalize()
df["Enddatum"] = pd.to_datetime(
    df["Enddatum"], format="%d.%m.%Y", errors="coerce"
).dt.normalize()
...
df["Wochentag_Mo"] = df["Startdatum"] - pd.to_timedelta(
    df["Startdatum"].dt.weekday, unit="D"
)
...
grouped = (
    expanded_df.groupby("Wochentag_Mo")
    .apply(
        lambda g: ", ".join(
            [
