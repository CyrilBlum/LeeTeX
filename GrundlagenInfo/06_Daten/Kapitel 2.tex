...
Falls nach einem \lstinline[style=sql]|GROUP BY| die Resultate noch weiter eingegrenzt werden sollen, muss statt \lstinline[style=sql]|WHERE| der Befehl \lstinline[style=sql]|HAVING| verwendet werden. Der Befehl \lstinline[style=sql]|WHERE| wird verwendet, um die ursprünglichen Daten \textit{vor} einem \lstinline[style=sql]|GROUP BY| zu filtern, der Befehl \lstinline[style=sql]|HAVING| wird verwendet, um die Resultate \textit{nach} einem \lstinline[style=sql]|GROUP BY| zu filtern.
...
\begin{mysubmission}
  \begin{itemize}
    ...
    \item Geben Sie danach auf Moodle ``Übungen 3 (\lstinline[style=sql]|GROUP BY|)'' ab.
  \end{itemize}
\end{mysubmission}
...
\begin{mycaution}
  Für die nachfolgenden Teile müssen Sie Adblockers in Ihrem Browser deaktivieren (oder zumindest eine Ausnahme für InstaHub erstellen).
\end{mycaution}
