...
\tiny\textbf{Ich} (``das Selbst, dessen man sich bewusst ist und mit dem man sich von der Umwelt unterscheidet'') \textbf{ging} (gehen, ``schrittweises Sichfortbewegen auf den Füßen in aufrechter Haltung'') \textbf{nach} (``drückt aus, dass jemand jemandem, einer Sache folgt, nachgeht'') \textbf{draussen} (``außerhalb eines Raumes, Gebäudes'')...
...
\begin{frame}{Weitere Beispiele}{Supermarkt}
  \begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{GrundlagenInfo/06_Daten/Figures/er_nordwind_komplett}
...
3         & Aniseed Syrup                   & 10.00       & 13           & 2           & Gewürze       & Süße und saure Soßen, Gewürze            \\\hline
4         & Chef Anton's Cajun Seasoning    & 22.00       & 53           & 2           & Gewürze       & Süße und saure Soßen, Gewürze            \\\hline
5         & Chef Anton's Gumbo Mix          & 21.35       & 0            & 2           & Gewürze       & Süße und saure Soßen, Gewürze            \\\hline
6         & Grandma's Boysenberry Spread    & 25.00       & 120          & 2           & Gewürze       & Süße und saure Soßen, Gewürze            \\\hline
…         & …                               & …           & …            & …           & …             & …                                       
\end{tabular}
\end{table}
\end{frame}
...
2           & Gewürze          & Süße und saure Soßen, Gewürze            \\\hline
3           & Süßwaren         & Desserts und Süßigkeiten                 \\\hline
...
\begin{center}
\only<1>{
\textbf{Frage}: Was könnte ein Fremdschlüssel \faKey{} sein? \\
...
\begin{frame}[fragile]{Mehrere Tabellen miteinander verbinden}{``Naive'' Tabellenverbindung}
...
\begin{table}[H]
\begin{tabular}{l|l|a}
\emoji{key} \underline{mID} & Name   & \underline{aID} \faKey{} \\\midrule
1  & Müller  & 31  \\
2  & Schmidt & 32  \\
3  & Kaufmann  & 32\\
... & ... & ... 
...
\begin{frame}[fragile]{Mehrere Tabellen miteinander verbinden}{Verwendung von \lstinline[style=sql]|WHERE|}
\begin{lstlisting}[style=sql,basicstyle=\ttfamily\footnotesize]
SELECT *
FROM Angestellte, Abteilung
...
\begin{frame}[fragile]{Mehrere Tabellen miteinander verbinden}{Nur eine Kolonne \texttt{aID}: Verwendung von \lstinline[style=sql]|JOIN|}
\begin{lstlisting}[style=sql,basicstyle=\ttfamily\footnotesize]
SELECT *
FROM Angestellte JOIN Abteilung
ON Angestellte.aID = Abteilung.aID
\end{lstlisting}
\vspace{-.3cm}
\footnotesize
\begin{columns}
...
\begin{frame}[fragile]{Mehrere Tabellen miteinander verbinden}{Verwendung von \lstinline[style=sql]|JOIN|}
\begin{lstlisting}[style=sql,basicstyle=\ttfamily\footnotesize]
SELECT *
FROM Angestellte JOIN Abteilung
USING (aID)
...
\begin{frame}[fragile]{Auftrag}
  Skript (Moodle)
  \begin{itemize}
    \item \aufgabebox{Aufgabe(n) 2.41 (externer Link)}
    \item \challengebox{Aufgaben im Skript, Kapitel 2.4 (InstaHub)}
    \item \challengebox{Abgabe auf Moodle (InstaHub, ``\lstinline[style=sql]|JOIN|'')}
  \end{itemize}
\end{frame}
