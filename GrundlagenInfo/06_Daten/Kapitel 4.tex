...
\begin{itemize}
  ...
  \item \lstinline[style=sql]|PRIMARY KEY|: Primärschlüssel (s. \cref{table:table-cia}).
  \item \lstinline[style=sql]|FOREIGN KEY (kolonneID) REFERENCES anderetabelle(kolonneID)|: Fremdschlüssel (s. \cref{table:table-cia}).
  \item \lstinline[style=sql]|UNIQUE|: Darf keine doppelten Werte enthalten
\end{itemize}

\subsection{Tabellen verändern: \texorpdfstring{\lstinline[style=sql]|ALTER table|}{ALTER table}}
Mit dem Befehl \lstinline[style=sql]|ALTER table| können existierende Tabellen verändert werden.
