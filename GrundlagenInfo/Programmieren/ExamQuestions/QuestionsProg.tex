...
\begin{parts}
	\part[2] Haben Sie es geschafft, alle Aufgaben zu lösen? Berechnen Sie die Antwort in Python, indem Sie eine \pythoninline{repeat}-Schleife verwenden und reichen Sie Ihren Code ein.
	\begin{solution}
...
\begin{itemize}
	\item Die 10 Noten, die Sie abgegeben haben (\pythoninline{noten_von_mir=[...]})
	\item Die 10 Noten, die die anderen Personen für Sie abgegeben haben (\pythoninline{noten_von_anderen=[...]})
	\item Der Score, um zu bestimmen, ob jemand ein Match sein könnte:\\
	 \pythoninline{match=note_von_mir+note_von_person - abs(note_von_mir-note_von_person)}
\end{itemize}
...
\end{lstlisting}
	\end{solution}
	\part[2] Wie Sie vermutlich wissen, sind Frauen selektiver auf Dating-Plattformen als Männer. Diesen Effekt wollen wir korrigieren, indem wir zuerst den Mittelwert aller erhaltenen Noten sowie aller gegebenen Noten berechnen. Danach berechnen wir die Differenz beider Schnitte als \pythoninline{differenz = abs(schnitt_von_anderen - schnitt_von_mir)} und addieren die Differenz zu jeder erhaltenen Note. Wer ist nun der beste Match?
	\begin{solution}
...
Ihre Strategie ist folgende: Sie schauen sich jeden Monat Ihre gewünschten Gegenstände an. Falls Sie genügend Geld haben, kaufen Sie den Gegenstand und fügen ihn Ihrem Inventar hinzu. Ebenfalls entfernen Sie den Gegenstand von Ihrer Wunschliste mit dem Befehl \pythoninline{liste.remove(gegenstand)}.

\begin{parts}
	\part[2] Wie sieht Ihr Inventar am Ende des Jahres aus? Verwenden Sie eine Schleife, um an jedem Monatsende Gegenstände zu kaufen, sowie den Befehl \pythoninline{.append}, um Gegenstände, die Sie sich leisten können, Ihrem Inventar hinzuzufügen. Können Sie sich innerhalb eines Jahres alle Gegenstände kaufen?
	\begin{solution}
...
\begin{parts}
	...
	\pythoninline{preis_bf = preis_original*(1-rabatt_prozent/100)}
	\begin{solution}
		\begin{lstlisting}[style=TigerJython]
preise_normal = [39.95,65.95,66.95,76.95,9.95,10.95,13.95]
rabatte_blackfriday_prozent = [30,40,30,35,20,15,35]
...
Der Mikrocontroller einer R2-D2 Roboter Einheit wurde beschädigt. Der Roboter kann deshalb nur noch einzelne, geradlinige \enquote{Schritte} nach links (l), rechts (r), oben (o) oder unten (u) (um jeweils eine Längeneinheit) durchführen. Die Bewegungsanweisung erhält der Roboter in Form des Strings \pythoninline{bewegung}. Der String \pythoninline{bewegung = 'lollru'} würde zum Beispiel dem Bewegungsablauf links, oben, links, links, rechts und unten (um jeweils eine Längeneinheit) entsprechen. Die R2-D2-Einheit bewegt sich auf dem zweidimensionalen Koordinatensystem und startet im Ursprung $(0,0)$.

\noindent
Schreiben Sie eine Python-Funktion \pythoninline{def r2d2(bewegung)}, welche für eine gegebene beliebige Be\-we\-gungs\-an\-wei\-sung \pythoninline{bewegung} entscheidet, ob sich der Roboter nach den Bewegungen wieder im Ursprung befindet (\pythoninline{return True}) oder nicht (\pythoninline{return False}). Hinweis: Auf die einzelnen Buchstaben eines Strings kann ebenso zugegriffen werden wie auf Listenelemente. Falls zum Beispiel \pythoninline{wort = "Hallo"}, dann ist \pythoninline{wort[0] == "H", wort[1], == "a"} und so weiter. Wie auch bei Listen, kann mit dem Befehl \pythoninline{len(wort)} die Länge (Anzahl Buchstaben) eines Worts bestimmt werden.
