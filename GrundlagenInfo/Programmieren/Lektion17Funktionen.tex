\begin{frame}
    ...
    Lektion 17 \emoji{cook}: \textbf{Funktionen und \tjinline{return}}
\end{frame}
...
(*@\only<1>{gericht}\only<2>{\colorbox{CornflowerBlue}{gericht}}@*) = gericht((*@ \emoji{bread}, \emoji{poultry-leg}, \emoji{cheese} @*))
print(gericht) # (*@ \emoji{hamburger} @*)

# Beispiel 2 (funktioniert nicht! Nur zur Illustration)
(*@\only<1>{gericht}\only<2>{\colorbox{CornflowerBlue}{gericht}}@*) = gericht((*@ \emoji{tropical-fish}, \emoji{cooked-rice}, \emoji{salt} @*))
print(gericht) # (*@ \emoji{sushi} @*)
...
\begin{frame}[fragile]{Modularität durch Funktionen}{Beispiel mit Turtle \emoji{turtle}}
\begin{minipage}{.65\textwidth}
\begin{lstlisting}[style=TigerJython, basicstyle=\ttfamily\scriptsize, escapeinside={(*@}{@*)}]
from gturtle import *
makeTurtle()
hideTurtle()
...
def vieleck_muster(umfang, ecken):
    while umfang>50:
        ...
        umfang-=10
        rt(10)
        (*@\only<2>{\colorbox{CornflowerBlue}{gesamtdistanz+=umfang}}@*)
    (*@\only<2>{\colorbox{CornflowerBlue}{return gesamtdistanz}}@*)
    
(*@\only<2>{\colorbox{CornflowerBlue}{gesamtdistanz = }}@*)vieleck_muster(500,35)
(*@\only<2>{\colorbox{CornflowerBlue}{print(gesamtdistanz)}}@*)
\end{lstlisting}
...
\begin{figure}[H]
	\centering
    \includegraphics[keepaspectratio, width=.9\textwidth]{GrundlagenInfo/Programmieren/Figures/spirale.png}
\end{figure}
...
% lower door coordinate
 ...
 % Draw doors
   \fill[Apricot] (llD\x#1) -- ++(0,0.5,0) -- ++(0.2,0,0)node[pos=0.5, below](in#1\x){\textcolor{black}{\tiny\texttt{x\x}\normalsize}} -- ++(0,-0.5,0);
}

% coordinate for result door
\coordinate (llDOut#1) at ({#2+1},0,0.2);
\fill[ForestGreen] (llDOut#1) -- ++(0,0.5,0) -- ++(0,0,0.6)node[pos=0.8,rotate=45, below, align=center](out#1){\textcolor{black}{\tiny\texttt{out#1}\normalsize}} -- ++(0,-0.5,0);

	% Draw a 2D rectangle around the cube
	  \node[fit=(E#1) (C#1), inner sep=4pt, red, dotted, draw, line width=1pt](dottedRect#1) {};
	  % annotate the two-d rect
	  \node[above=5pt of dottedRect#1.north west, anchor=west, align=left]{\textcolor{red}{\tiny\texttt{def a#1(\foreach\x in {1,...,#3}{x\x,}...)}\normalsize}};
...
\node(input1) at (0.3,0,2){3};
\node(input2) at (0.7,0,2){12};
...
\only<4->{
\draw[thick,->] (out1) -- ++(0,-1.5,0) -- ++(1.55,0,0)node[below]{\lstinline[language=Python,style=TigerJython,basicstyle=\ttfamily\tiny]{return out1}} -- ++(1.55,0,0) -- (in31);}
...
\begin{frame}{Aufgaben}
	...
	\begin{itemize}
	    \item Buch, Aufgabe 7.2 $\rightarrow$ Abgabe
	    \item Buch, Aufgabe 7.4 $\rightarrow$ Abgabe
	    \item Moodle, Abschnitt ``Extra-Aufgaben''
	    \begin{itemize}
	    \item Aufgabe 7 ``R2-D2'' $\rightarrow$ Abgabe
	    \item Aufgabe 8 ``Summanden'' $\rightarrow$ Abgabe
	    \end{itemize}
	    \item \emoji{trophy} Challenge: 7.4, 7.5
	    \begin{itemize}
