...
\begin{lstlisting}[style=TigerJython, basicstyle=\ttfamily\tiny, escapeinside={(*@}{@*)}]
from gturtle import *
makeTurtle()
hideTurtle()
...
def vieleck_muster(umfang, ecken(*@\only<3->{\colorbox{CornflowerBlue}{, gesamtdistanz}}@*)):
    while umfang>50:
        vieleck(umfang, ecken)
        (*@\only<4->{\colorbox{CornflowerBlue}{gesamtdistanz+=umfang}}@*)
        umfang-=10
        rt(10)
    
    (*@\only<5->{\colorbox{CornflowerBlue}{return gesamtdistanz}}@*)
    
(*@\only<6->{\colorbox{CornflowerBlue}{gesamtdistanz = }}@*)vieleck_muster(500,35(*@\only<3->{\colorbox{CornflowerBlue}{, gesamtdistanz}}@*))
(*@\only<7->{\colorbox{CornflowerBlue}{print(gesamtdistanz)}}@*)
\end{lstlisting}
% Clean code
%from gturtle import * 
%makeTurtle() 
...
% lower door coordinate
 ...
 % Draw doors
   \fill[Apricot] (llD\x#1) -- ++(0,0.5,0) -- ++(0.2,0,0)node[pos=0.5, below](in#1\x){\textcolor{blue}{\tiny\texttt{x\x}\normalsize}} -- ++(0,-0.5,0);
}
