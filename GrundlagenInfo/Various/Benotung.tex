...
Bitte beachten Sie, dass wir Ihnen die Noten aller Leistungskontrollen (schriftliche Prüfungsnoten, mündliche Note) bereits auf 1/10 gerundet bekanntgeben werden  (z.B. 4.9 oder 5.3). Wir werden dann stets mit diesen (auf Zehntel) gerundeten Werten arbeiten. 

\begin{mycaution}
Wer zu einem festgesetzten Probentermin nicht erscheint, informiert die zuständige Lehrkraft \textbf{vor} dem Zeitpunkt der Probe per Mail. Er / sie schreibt eine Nachprobe (NP) an einem fachschaftlich bestimmten Termin gegen Ende des jeweils laufenden Semesters. Bei dieser NP handelt es sich um eine \textbf{Semesterprobe}. Es erfolgt keine spezielle Einladung, da der Termin jeweils zu Beginn des Semesters kommuniziert wird. Es liegt im Ermessen der Lehrperson, ob in begründeten Fällen (z.B. amtliches Aufgebot, länger
dauernde Krankheit, o.ä.) eine Ausnahme von dieser Regelung gewährt werden kann.
\end{mycaution}
...
\[N_e=\text{runden\_auf\_halbe\_note}\Big(N_s+B_m+B_f\Big).\]

Der Schnitt wird abgerundet auf 6, sollte er aufgrund der beiden Boni die Note 6 übersteigen.
