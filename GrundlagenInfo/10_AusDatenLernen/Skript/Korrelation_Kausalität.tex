...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        \label{fig:socialmediaVsZufriedenheit}
    \end{subfigure}
...
Der Korrelationskoeffizient wird typischerweise als Zahl $r$ angegeben, welche sich zwischen -1 (perfekte negative Korrelation) und 1 (perfekte positive Korrelation) bewegt (s. \cref{fig:korr-r--1} und \cref{fig:korr-r-1}). Ein Wert von 0 bedeutet, dass es keinen linearen Zusammenhang zwischen den beiden Variablen gibt (s. \cref{fig:korr-r-0}). Dies schliesst jedoch nicht aus, dass es einen anderen, nicht-linearen Zusammenhang gibt (s. \cref{fig:korr-r-oth0}).
...
\begin{figure}[H]
    ...
    \foreach \corr in {-1, -0.8, -0.4, 0, 0.4, 0.8, 1} {\begin{subfigure}[t]{0.16\pagewidth}
                ...
                \label{fig:korr-r-\corr}
            \end{subfigure}
        }
    \begin{subfigure}[t]{0.16\pagewidth}
        \centering
        \includegraphics[width=\linewidth]{"GrundlagenInfo/10_AusDatenLernen/Figures/correl_ex_15"}
        \caption{r$\simeq$\textbf{0}}
        \label{fig:korr-r-oth0}
    \end{subfigure}
...
Der erste Schritt zur Berechnung des Korrelationskoeffizienten ist die Berechnung der Mittelwerte aller Variablen, $\overline{x}$ und $\overline{y}$ (s. \cref{fig:Korrelationen-Ursprung}). Der Mittelwert ist der arithmetische Durchschnittswert aller Werte einer Variablen.

In einem zweiten Schritt berechnen wir die Abweichung der ursprünglichen Werte von ihrem Mittelwert (s. \cref{fig:Korrelationen-Abweichung-Ursprung}).  Die Abweichung eines Wertes \(x_i\) von seinem Mittelwert \(\overline{x}\) wird als \( x_i' \) bezeichnet.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        \label{fig:Korrelationen-Ursprung}
    \end{subfigure}
    ~
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        ...
        \label{fig:Korrelationen-Abweichung-Ursprung}
    \end{subfigure}
...
\begin{equation}
    \begin{aligned}
        r      & = \frac{Z}{\sqrt{\Big((x_1')^2 + (x_2')^2 + \dots + (x_n')^2\Big) \cdot \Big((y_1')^2 + (y_2')^2 + \dots + (y_n')^2\Big)}} \\
         & = \frac{Z}{\sqrt{\Big(\sum_{i=1}^n (x_i')^2\Big) \cdot \Big(\sum_{i=1}^n (y_i')^2\Big)}}
    \end{aligned}
\end{equation}
