...
\begin{frame}{Anwendungen von Markov-Ketten}{Wortgenerierung mit ChatGPT}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[scale=0.7, transform shape, ->, >=stealth', node distance=4cm, thick,
            ...
            (wie) edge node[right] {0.6} (geht)
            (wie) edge node[right] {0.4} (dir)
    ...
    \begin{itemize}[<+->]
        \item Lektion 1
              \begin{itemize}[<+->]
                  ...
                  \item[#1] Ich kann eine einfache Matrixmultiplikation durchführen.
              \end{itemize}
        \item Lektionen 2 \& 3
              \begin{itemize}[<+->]
...
\begin{frame}{Visualisierung von Markov-Ketten}
    ...
    \begin{columns}
        ...
        \only<5->{
            \begin{myexercise}
                \small
                Jedes Mal, wenn Sie Instagram (\textbf{I}) benutzen, benutzen Sie als nächste App zu \textbf{80\%} nochmals Instagram, und auf TikTok (\textbf{T}) mit \textbf{20\%}. Wenn Sie zuerst TikTok benutzen, wechseln Sie zu \textbf{30\%} zu Instagram und bleiben zu \textbf{70\%} auf TikTok. Erstellen Sie den \textbf{gerichteten Graphen} für die Zustände \textbf{I} und \textbf{T}.
            \end{myexercise}
        }
    \end{columns}
\end{frame}
...
\begin{frame}{Gerichteter Graph: Social-Media-App-Nutzung}
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[node distance=4cm, auto, thick]
            ...
            \path[->]
            (I) edge[bend left] node[above] {0.2} (T)
            (I) edge[loop above] node {0.8} (I)
            (T) edge[bend left] node[below] {0.3} (I)
            (T) edge[loop above] node {0.7} (T);
        \end{tikzpicture}
...
\begin{frame}{Gerichtete Graphen}{Übung: Thailändischer Tourist}
    \begin{myexercise}
        ...
        \end{itemize}
        Erstellen Sie den gerichteten Graphen für die Zustände \textbf{F}, \textbf{S} und \textbf{P} (\red{\textbf{danach nicht ausradieren!}}).
    \end{myexercise}
\end{frame}
...
\begin{frame}{Die Übergangsmatrix}
    ...
    \end{itemize}
    \only<4->{
        \begin{myexercise}[Übergangsmatrix: Thailändischer Tourist]
            Erstellen Sie die Übergangsmatrix für den gerichteten Graphen des thailändischen Touristen (\textbf{F}, \textbf{S}, \textbf{P})
            \only<5->{
                \[
                    P =
                    \begin{pmatrix}
...
\begin{frame}{Wettervorhersage für übermorgen}
    ...
    \only<3->{
        \begin{myexercise}[Instagram oder TikTok nach 2 Schritten?]
            Wie gross ist die Wahrscheinlichkeit, dass der Schüler nach zwei Nutzungen bei Instagram landet, wenn er mit TikTok startet? 
...
\begin{frame}{Übung: TikTok / Instagram nach 2 Schritten?}
    \begin{itemize}
        ...
        \item Nach einem Schritt:
              \[
                  \pi_1 = \pi_0 \cdot P = \begin{pmatrix} 0 & 1 \end{pmatrix} \cdot \begin{pmatrix} 0.8 & 0.2 \\ 0.3 & 0.7 \end{pmatrix} = \begin{pmatrix} 0.3 & 0.7 \end{pmatrix}
              \]
        \item Nach zwei Schritten:
              \[
                  \pi_2 = \pi_1 \cdot P = \begin{pmatrix} 0.3 & 0.7 \end{pmatrix} \cdot \begin{pmatrix} 0.8 & 0.2 \\ 0.3 & 0.7 \end{pmatrix} = \begin{pmatrix} 0.45 & 0.55 \end{pmatrix}
              \]
        \item Die Wahrscheinlichkeit, nach zwei Nutzungen bei Instagram zu landen, beträgt \textbf{45\%}.
    \end{itemize}
\end{frame}
