...
\subsection{Primzahlsatz und Prim}
...
Schreiben Sie eine Python-Funktion \pythoninline{def sieb(n)}, welche den Alogrithmus von Eratosthenes implementiert und alle Primzahlen $\leq n$ ausgibt. Vervollständigen Sie dazu die folgende Vorlage:
...
\begin{mychallenge}[Verbesserung 2]\label{myexercise:implementVerbesserung2}
...
% \begin{myexercise}
% \begin{aenum}
% \item Bestimme $\text{Prim}(n)$ für $n = 10000000$ (zehn Millionen) mit Hilfe der Funktion \pythoninline{prim_fast} und vergleiche den Wert der Schätzung $n/\ln{n}$ aus dem Primzahlensatz. Wie gut ist die Schätzung?
%
...
%
% \item Verwende die sehr schnelle Funktion $\pythoninline{prim_approx}$ anstelle von $\pythoninline{prim_fast}$, um den Wertebereich des Plots von $8\cdot 10^7$ auf $10^{10}$ zu erhöhen.
% \end{aenum}
% \end{myexercise}
%
%
...
%     
% \begin{myexercise}
% Verwende deine Python-Funktionen $\pythoninline{sieve_fast}$, $\pythoninline{Bin}$ und $\pythoninline{Nummer}$ um das Kommunikationsprotokoll in Python zu implementieren.
% \end{myexercise}
% \begin{myanswer}
%
% Die Funktionen \pythoninline{sieve_fast}, \pythoninline{Nummer} und \pythoninline{Bin} haben wir schon geschrieben. Diese können wir nun hier verwenden. Die Funktion \pythoninline{is_equal} wird nicht benötigt, kann aber verwendet werden um (deterministisch) zu prüfen, ob zwei Datensätze identisch sind oder nicht.
% \begin{minted}[linenos]{python}
% import numpy as np
%
% def sieve_fast(n):
