...
wobei $a = 121$, $b = 9$, $q = 13$ und $r = 4$.
    \item \phantom{1}
\begin{lstlisting}[language=Python,caption=Division mit Rest]
...
\begin{myanswer}
\begin{lstlisting}[language=Python,caption=Nummer]
def Nummer(x):
    x = list(str(x))
    n = len(x)
    dec = 0
...
\begin{lstlisting}[language=Python,caption=Sieb des Eratosthenes]
def sieb(n):
...
\begin{lstlisting}[language=Python,caption=Prim]
def prim(n):
    return len(sieve(n))
\end{lstlisting}
