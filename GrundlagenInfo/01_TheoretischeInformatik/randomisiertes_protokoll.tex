...
\subsection{Nummer}
Sei $x=x_1x_2\ldots x_n$, $x_i\in\lrc{0,1}$ für $i=1,2,\ldots, n$ eine endliche Folge von binären Ziffern. Dann bezeichnet 
\begin{align}
...
\begin{mychallenge}
Schreibe eine Python-Funktion \pythoninline{def Nummer(x)}, welche $\text{Nummer}(x)$ für eine natürliche Zahl $x$ berechnet.
\begin{myanswer}
...
\subsection{Primzahlsatz und die Funktion Prim}
...
\begin{myexample}
Es existieren genaud die vier Primzahlen $2, 3, 5$ und $7$, die kleiner oder gleich $9$ sind. Damit ist $\text{Prim}(9)$ = $4$ ebenso ist $\text{Prim}(10)$ = $4$.
...
Deshalb könnte man intuitiv annehmen, dass grosse Primzahlen äusserst selten sind, da sehr viele Zahlen \enquote{potentielle} echte Teiler der Zahl zu sein scheinen.
...
Der Algorithmus des \tib{Sieb des Eratosthenes}\index{Sieb des Eratosthenes} (englisch: Eratosthene's Sieve) ist bereits seit der Antike bekannt. Der Algorithmus bestimmt für eine gegebene natürliche Zahl $n$ alle Primzahlen, die kleiner oder gleich $n$. Man schreibt alle natürlichen Zahlen grösser gleich $2$ bis und mit $n$ auf:
\begin{align*}
...
Da $m$ eine Primzahl ist, besitzt sie keine echten Teiler. Damit ist sie nicht das Vielfache einer Zahl aus der Menge $\lrc{2,3,4, ..., m-1}$.
Deshalb hat das Sieb die Zahl $m$ bis zum Erreichen von $m$ nicht durchgestrichen und somit wird $m$ vom Sieb ausgegeben.

$\Leftarrow$:

Sei $n\geq m$ die Eingabe für den Algorithmus \textit{Sieb des Eratosthenes}. Nach Konstruktion des Algorithmus, wird $m$ genau dann ausgegeben, wenn $m$ kein Vielfaches einer Zahl aus der Menge $\lrc{2,3,4, ..., m-1}$ ist. Damit besitzt $m$ aber keinen echten Teiler und somit ist $m$ eine Primzahl.
...
def sieb(n):
    ...
    print(primzahlen)
\end{lstlisting}
\begin{myanswer}[myexercise:sieb]
\begin{lstlisting}[language=Python,caption=Sieb des Eratosthenes,numbers=none]
def sieb(n):
    # starte mit einer leeren Liste von
    # Primzahlen (wir haben noch keine gefunden)
...
Mit Hilfe des Sieb von Eratosthenes, ist es nun ganz einfach, eine Funktion zu definieren, welche $\text{Prim}(n)$ berechnet. Dazu ändern wir den Befehl \pythoninline{print(primzahlen)} in unserer Funktion \pythoninline{sieb} ab zu \pythoninline{return primzahlen} und können dann schreiben:
...
\begin{align*}
  ab \geq \lr{\floor{\green{\sqrt{n}}} + 1}^2 > \lr{\blue{\sqrt{n} - 1} + 1}^2 = n
\end{align*}
und somit insbesondere der Widerspruch $ab > n$ obwohl $ab = n$ gilt.
\end{description}
\end{myanswer}
\end{myexercise}
...
\begin{myanswer}[myexercise:implementVerbesserung1]
...
Angenommen, das Sieb des Eratosthenes hat soeben eine Primzahl $p > 2$ gefunden. Zeige, dass die Vielfachen $hp$ von $p$ mit $h\in\lrc{2,3, ..., p-1}$ bereits vom Algorithmus gestrichen wurden. Damit genügt es, die Vielfachen $\geq p^2$ zu streichen (solange sie kleiner oder gleich $n$ sind).
\begin{myanswer}[myexercise:Verbesserung2]
Sei $hp$ für ein $h\in\lrc{2,3, ..., p-1}$ ein Vielfaches von $p$. Angenommen $h$ ist eine Primzahl. Da $h<p$, wurde $h$ bereits als Primzahl vom Sieb (vor $p$) gefunden und die Vielfachen $\leq n$ von $h$ wurden gestrichen. Insbesondere also auch das Vielfache $hp$ von $h$. Falls $h$ keine Primzahl ist, dann besitzt $h$ eine Zerlegung $h = ab$ für eine Primzahl $a$ mit $a<h<n$. Damit wurde aber $hp = abp$ als $bp$-faches Vielfache von $a$ bereits gestrichen.
\end{myanswer}
\end{mychallenge}
...
\begin{myanswer}[myexercise:implementVerbesserung2]
...
\item Schreibe eine Python-Funktion \pythoninline{def prim_approx(n)}, welche $\text{Prim}(n)$ mit Hilfe der Abschätzung, welche von dem Primzahlsatz \ref{eq:primzahlsatz} geliefert wird, approximiert.
...
Sei $m\geq 1$ eine natürliche Zahl. Wir bezeichnen mit $\text{Bin}(m)$ die kürzeste binäre Darstellung von $m$, also $\text{Nummer}(\text{Bin}(m)) = m$.
Die binäre Darstellung ist offenbar genau dann am kürzesten, wenn die erste Ziffer der Darstellung eine $1$ und keine $0$ ist.
Beispielsweise sind $100$ und $00100$ beides binäre Darstellungen der Zahl $4$, doch die erste ist die kürzeste.
...
\begin{myanswer}
  Sei $k := \abs{\text{Bin}(m)}$. Wir wollen $k$ ermitteln. Die grösste Zahl, die sich in binär mit $k$-Stellen Darstellen lässt, ist
  \begin{align*}
    \underbrace{111\ldots 1}_{k-\text{Stellen}}\; =
    \underbrace{100\ldots 0}_{(k+1)-\text{Stellen}} -\; 1 = 2^k - 1.
  \end{align*}
Die kleinste Zahl (ohne führende Nullen), die sich in binär mit $k$-Stellen Darstellen lässt, ist
  \begin{align*}
    \underbrace{100\ldots 0}_{k-\text{Stellen}}\; =
    \underbrace{111\ldots 1}_{(k-1)-\text{Stellen}} +\; 1 = 2^{k-1} + 1.
  \end{align*}

  Offensichtlich gilt für $m$ nun also die \enquote{Sandwich-Beziehung}:
\begin{align*}
    & 2^k-1 \geq m > 2^{k-1} - 1 \iff \\
    & 2^k \geq m+1 > 2^{k-1} \iff \\
    & \log_2\lr{2^k} \geq \log_2\lr{m+1} > \log_2\lr{2^{k-1}} \iff \\
    & k \geq \log_2(m+1) > k-1.
\end{align*}
Die Beziehung $k \geq \log_2(m+1) > k-1$ sagt aus, dass $\log_2(m+1)\in (k-1,k]$ und damit folgt:
\begin{align*}
    k = \abs{\text{Bin}(m)} = \ceil{\log_2(m+1)}.
\end{align*}
\end{myanswer}
\end{myexercise}
...
\subsection{Formuluierung des Kommunikationsprotokolls}
...
\item Sei $x\neq y$. Wir bekommen eine falsche Antwort \enquote{gleich} nur dann, wenn $R_2$ eine zufällige Primzahl $p$ gewählt hat, die die Eigenschaft hat, dass
    ...
    \end{align*}
    Sei $w = p_1^{i_1}p_2^{i_2}...p_k^{i_k}$, wobei $p_1<p_2<\ldots<p_k$ Primzahlen  und $i_1, i_2,...,i_k$ positive ganze Zahlen sind. Wir wissen, dass jede Zahl eine solche eindeutige Faktorisierung besitzt. Unser Ziel ist zu beweisen, dass $k\leq n-1$ ist.
...
Die Funktionen \pythoninline{sieb}, \pythoninline{Nummer} und \pythoninline{Bin} haben wir schon geschrieben. Diese können wir nun hier verwenden.
