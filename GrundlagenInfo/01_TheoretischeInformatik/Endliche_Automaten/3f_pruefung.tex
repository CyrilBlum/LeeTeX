...
% name of the class
\newcommand{\myclass}{Klasse 3f}
% set date
\newcommand{\mydate}{08. Januar 2025}

\title{\textbf{Aus Daten lernen, endliche Automaten}}
\author{Thomas Graf\\
	\mycourse, \myclass}
\date{\mydate}
\maketitle
...
\begin{questions}
	...
	Es sei $\Sigma = \lrc{a, b, c}$ ein Alphabet. Bestimmen Sie die Anzahl der verschiedenen Wörter über $\Sigma$, welche das Teilwort $a$ enthalten.
	\begin{solution}
	...
	\begin{align*}
		L_3 := \setcm{w\in\lrc{a,b}^*}{\abs{w}_a < \abs{w}_b}.
	\end{align*}
	Dabei bezeichnet $\abs{w}_a$ die Anzahl der $a$'s im Wort $w$ und $\abs{w}_b$ die Anzahl der $b$'s in $w$. Beispiel: $\abs{w}_a = 3$ und $\abs{w}_b = 2$ für $w = abbaa$.
	\begin{parts}
