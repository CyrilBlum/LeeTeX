...
\subsection{Einfache Selektion: \texorpdfstring{\lstinline[style=sql]|SELECT|}{SELECT}}
...
\subsection{Selektion mit Filter: \texorpdfstring{\lstinline[style=sql]|WHERE|}{WHERE}}
...
\begin{table}[H]
  ...
  \begin{tabular}{c|l}
    ...
    \lstinline[style=sql]|BETWEEN x AND y|        & ein Wert zwischen \texttt{x} und \texttt{y} \\
    \lstinline[style=sql]|IN (wert1, wert2, ...)| & einer von mehreren möglichen Werten         \\
    \lstinline[style=sql]|IS NULL|                & Wert existiert nicht                        \\
  \end{tabular}
\end{table}
...
\begin{myachtung}
  Wie Sie anhand von \cref{ex:in} und \cref{ex:between} sehen, spielt es in SQL eine Rolle, ob  wir Zahlen oder Text filtern: Wenn nach einem Text gefiltert wird, muss dieser in Anführungszeichen stehen (wie z.B. \lstinline[style=sql]|"Berlin"| in \cref{ex:in}). Bei Zahlen ist dies nicht nötig (s. z.B. die Zahl \lstinline[style=sql]|172| in \cref{ex:between}).
\end{myachtung}

\lstinline[style=sql]|NULL| ist ein spezielles \gls{SQL}-Wort, welches bedeutet, dass ein Feld leer ist, bzw. keinen Wert hat. \lstinline[style=sql]|NULL| muss von ``0'' unterschieden werden: Beispielsweise könnte die Zahl 0 in einer Tabelle über Bankkonten bedeuten, dass jemand 0 Franken auf seinem Konto hat, während der Wert \lstinline[style=sql]|NULL| bedeuten würde, dass keine Informationen zum Kontostand vorhanden sind.
...
\subsection{Eindeutige Selektion: \texorpdfstring{\lstinline[style=sql]|SELECT DISTINCT|}{SELECT DISTINCT}}
...
\subsection{Ungefähre Treffer: \texorpdfstring{\lstinline[style=sql]|LIKE|}{LIKE}}
...
\begin{table}[ht]
  ...
  \begin{tabular}{c|l}
    ...
    \lstinline[style=sql]|"%"| & Stellt null oder mehrere, beliebige Zeichen dar \\ \hline
    \lstinline[style=sql]|"_"| & Stellt ein einzelnes, beliebiges Zeichen dar    \\
  \end{tabular}
...
\begin{mychallenge}
  Geben Sie die Grösse und den Wohnort von Juliette Amsel sowie Juliette Unger aus. Sie sollten das Zeichen \lstinline[style=sql]|"%"| nicht verwenden.
  \begin{myantwort}
    \begin{lstlisting}[style=sql]
...
\subsection{Sortieren: \texorpdfstring{\lstinline[style=sql]|ORDER BY|}{ORDER BY}}
...
\begin{myabgabe}
Lösen Sie Aufgaben 1-11 unter folgendem Link: \url{https://sql-tutorial.de/home/uebungen.php?lektion=1}. Geben Sie danach auf Moodle die Abgabe unter ``Übungen 1 (Einfache Selektion)'' ab.
\end{myabgabe}

\subsection{Aggregatsfunktionen}

\subsubsection{\texorpdfstring{\lstinline[style=sql]|COUNT|}{COUNT}}
...
\subsubsection{\texorpdfstring{\lstinline[style=sql]|MAX, MIN|}{MAX, MIN}}
...
\subsubsection{\texorpdfstring{\lstinline[style=sql]|SUM|}{SUM}}
...
\subsubsection{\texorpdfstring{\lstinline[style=sql]|AVG|}{AVG}}
...
\subsubsection{\texorpdfstring{\lstinline[style=sql]|LENGTH|}{LENGTH}}
...
\begin{myabgabe}
Lösen Sie Aufgaben 1-6 unter folgendem Link: \url{https://sql-tutorial.de/home/uebungen.php?lektion=2}. Geben Sie danach auf Moodle die Abgabe unter ``Übungen 2 (Aggregatsfunktionen)'' ab.
\end{myabgabe}

\subsection{Gruppieren: \texorpdfstring{\lstinline[style=sql]|GROUP BY|}{GROUP BY}}
...
\subsection{Filtern nach Gruppieren: \texorpdfstring{\lstinline[style=sql]|HAVING|}{HAVING}}
Falls nach einem \lstinline[style=sql]|GROUP BY| die Resultate noch weiter eingegrenzt werden soll, muss statt \lstinline[style=sql]|WHERE| der Befehl \lstinline[style=sql]|HAVING| verwendet werden. Der Befehl \lstinline[style=sql]|WHERE| wird verwendet, um die ursprünglichen Daten \textit{vor} einem \lstinline[style=sql]|GROUP BY| zu filtern, der Befehl \lstinline[style=sql]|HAVING| wird verwendet, um die Resultate \textit{nach} einem \lstinline[style=sql]|GROUP BY| zu filtern.
...
\subsection{Erste / Letzte Zeilen: \texorpdfstring{\lstinline[style=sql]|LIMIT|}{LIMIT}}
Mit dem Befehl \lstinline[style=sql]|LIMIT| kann das Resultat einer \gls{SQL}-Abfrage auf eine gewisse Anzahl Zeilen beschränkt werden.
...
\begin{mychallenge}
  ...
  \begin{itemize}
    ...
    \item Beschränken Sie danach Ihre Abfrage mit dem \lstinline[style=sql]|WHERE|-Befehl auf Benutzer, die ein ``b'' im Namen haben
...
\begin{myabgabe}
Lösen Sie Aufgaben 1-5 unter folgendem Link: \url{https://sql-tutorial.de/home/uebungen.php?lektion=4}. Geben Sie danach auf Moodle die Abgabe unter ``Übungen 3 (\lstinline[style=sql]|GROUP BY|)'' ab.
\end{myabgabe}

\subsection{Verzweigungen: \texorpdfstring{\lstinline[style=sql]|CASE WHEN|}{CASE WHEN}}
