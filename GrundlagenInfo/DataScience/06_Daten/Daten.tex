...
\begin{frame}{Moore's Law}{Es wird immer mehr und mehr...}
``Die Anzahl Transistoren verdoppelt sich ca. alle zwei Jahre''
...
\begin{frame}[fragile]{SQL-Befehle}{\lstinline[language=SQL]|SELECT [columns] FROM [tablename]|}
\ciascheme{.7}
...
\begin{frame}[fragile]{SQL-Befehle}{\lstinline[language=SQL]|SELECT [columns] FROM [tablename] WHERE [conditions]|}
\ciascheme{.7}
\begin{lstlisting}[language=SQL, basicstyle=\ttfamily\scriptsize]
SELECT Name, Region
FROM cia
