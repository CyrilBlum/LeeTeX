...
\begin{todolist}
    ...
    \item Die SuS können den Modulo-Operator (\lstinline|%|) korrekt verwenden, beispielsweise um alle geraden Zahlen in einer Schleife auszugeben  (s. DL 1 für einfache Schleifen).
...
Wie Sie bereits wissen, können Definitionen mit Kochrezepten verglichen werden: Sie beschreiben einen Ablauf, ohne jedoch diesen auszuführen. Damit der Ablauf ausgeführt wird, müssen Sie die Definitionen aufrufen. Definitionen können zudem Inputs entgegennehmen, sogenannte \textbf{Parameter}. Wir zwar bisher einen Wert einer Variable mittels \lstinline|drucken|, Bisher konnten Sie jedoch nichts an das Hauptprogramm zurückgeben. In diesem Kapitel lernen wir, dass dies möglich ist mit dem Befehl \lstinline|return|.
...
Stellen Sie sich vor, Sie sind SportlerIn und möchten Ihren Kalorienbedarf berechnen, um sich für auf einen kommenden Wettkampf optimal zu ernähren. Eventuell möchten Sie auch noch etwas ab- oder zunehmen, da Ihre Sportart dies erfordert. Hierzu wollen Sie Ihren täglichen Kalorienbedarf berechnen. Ihr Kalorienbedarf berechnet sich wie folgt:
\begin{enumerate}
...
Nun möchten wir im Code oben nach Zeile ... zum Grundumsatz auch noch unsere Aktivitäten hinzufügen, allerdings können wir nun nicht mehr auf die Variable bmr zurückgreifen. Wie können wir das ändern? Indem wir sie an das Programm zurückgeben und speichern (s. ...). Wichtige Änderungen gegebüber dem vorigen Code sind in \colorbox{CornflowerBlue}{blau} umrandet.
...
# berechne grundumsatz(*@\colorbox{CornflowerBlue}{, speichere das Resultat in einer Variable "bmr"}@*)
(*@\colorbox{CornflowerBlue}{bm\tikzmark{arrowTo}r = }@*) grundumsatz(gewicht, groesse, alter, geschlecht)
(*@\colorbox{CornflowerBlue}{print("Der Grundumsatz beträgt:", round(bmr, 2), "Kalorien")}@*)
\end{lstlisting}
\end{minipage}
...
Weshalb haben wir das gemacht? Nun können wir zum \gls{BMR} auch noch die Leistungsenergie hinzufügen, um den gesamten Energiebedarf zu berechnen.
...
\end{lstlisting}
Schreiben Sie eine Python-Funktion, um eine neue Liste zu produzieren, die für jede Aktivität die Anzahl verbrannter Kalorien berechnet. Das erwartete Resultat für das Beispiel hier wäre: \lstinline|[990, 120]|.
\begin{myanswer}
\begin{lstlisting}
def berechne_kalorien(l_zeit, l_kcal):
    """
    ...
    for i in range(len(l_zeit)):
        kalorien = l_zeit[i] * l_kcal[i]
        kalorienverbrauch.append(kalorien)
...
kalorienverbrauch = berechne_kalorien(l_zeit, l_kcal)
print(kalorienverbrauch)  # Erwartetes Ergebnis: [990, 120]
\end{lstlisting}
\end{myanswer}
\end{myexercise}
...
%\includesteps{16}
\begin{figure}[H]
	\include{GrundlagenInfo/00_Programmieren/Python/step_2.tex}	
\end{figure}
