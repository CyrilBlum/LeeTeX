...
\item Ich setze die \lstinline|repeat|-Schleife ein, um sich wiederholende Tätigkeiten auszuführen.
...
\item Ich kann \lstinline|repeat|-Schleifen innerhalb von \lstinline|repeat|-Schleifen einsetzen.
...
\item Ich setze die Operationen \lstinline|+|, \lstinline|*| und \lstinline|\n| ein, um komplizierte Strings (Zeichenketten) zu erzeugen.
...
\section{Lernziele Kapitel 3: Variablen, Speicherung und Verarbeitung von Daten}
\begin{todolist}
...
\item Ich kann in einem Programm Variablen, die keine Parameter sind, erstellen und verwenden.
...
\item Ich kann den Modulo-Operator \lstinline|%| verwenden, um zu prüfen, ob eine Zahl einen bestimmten Teiler besitzt.
...
\item Ich kann erklären, weshalb ein Code der nach einem ausgeführten \lstinline|repeat|-Ausdruck steht, nicht mehr ausgeführt wird.
...
\item ich kann die geeignetste Schleifenart auswählen, um ein bestimmtes Problem zu lösen (\lstinline|repeat| mit \lstinline|break| oder \lstinline|while|).
...
\item Ich kann die Elemente einer Liste in einer Schleife (\lstinline|repeat|, \lstinline|while| oder \lstinline|for...in|) durchlaufen.
