...
\begin{frame}[fragile]{Bubble-Sort-Algorithmus}{Python-Code}
	\begin{lstlisting}
		...
		# Äussere Schleife: Wiederholt Prozess für jede Position in der Liste
		for i in range(len(x) - 1):  
			...
			for j in range(len(x) - i - 1):  
				if x[index] > x[index + 1]:
					...
					x[index] = x[index + 1]
					x[index + 1] = z
				index += 1  # Erhöht den Index, um zum nächsten Element zu gehen
	\end{lstlisting}
\end{frame}
