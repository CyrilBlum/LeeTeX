...
\newcommand{\includesteps}[1]{%
	\foreach \step in {0,...,#1}{%
		 \pgfmathtruncatemacro{\stepplusone}{\step+1} % Increment step by 1 for the \only command
		\only<\stepplusone>{%
			\begin{figure}[H]
...
\end{itemize}
Heute: \textbf{Bubble-Sort-Algorithmus}
\end{frame}
...
\begin{frame}[fragile]{Bubble-Sort-Algorithmus}{Python-Code}
	\begin{lstlisting}
		...
		for i in range(len(x) - 1):  # Äussere Schleife: Wiederholt den Prozess für jede Position in der Liste
			index = 0
			for j in range(len(x) - i - 1):  # Innere Schleife: Vergleicht benachbarte Elemente
