...
\only<2->{
    \draw[red,-stealth] (arrow1From.north) to[bend right] node[right, xshift=.2cm, fill=CornflowerBlue,rounded corners] {\emoji{handshake} Übergabe} (arrowTo1);
    }
\only<3->{
...
\begin{figure}
\centering
\includegraphics[width=0.5\linewidth]{GrundlagenInfo/00_Programmieren/Figures/mehrere-parameter}
\end{figure}
\end{column}
\end{columns}
\only<2->{
...
\only<3->{\lstinline[style=TigerJython]|def meine_funktion(p1, p2, p3, ...)|}
\end{frame}
