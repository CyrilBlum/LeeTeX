\begin{frame}
    ...
    \emoji{abacus} Kapitel 03 (Teil 3):  \textbf{Zeit-Tabellen}
\end{frame}
...
def quadrat(seitenlaenge):
    repeat 4:
        !\only<1-4,6-8>{forward(seitenlaenge)}
          \only<5,9->{forward(\colorbox{CarnationPink}{seiten\tikzmark{arrowFrom3a}laenge})}!
        right(90)
...
\begin{figure}[H]
\centering
        \includegraphics[width=.75\linewidth]{GrundlagenInfo/00_Programmieren/Figures/prog-8-squares.png}
    \end{figure}}
    \only<4->{
\begin{table}[H]
    \centering
...
\item Parameter sind sogenannt ``\textbf{lokal}'' und existieren nur innerhalb einer Definition
\end{itemize}
}
\end{frame}
