...
\begin{enumerate}
	\item Erstellen Sie auf Ihrem eigenen Rechner einen neuen Ordner \emoji{file-folder} mit dem Namen
	      \begin{center}
		      {\tt BilderBearbeiten}.
	      \end{center}
	\item Laden Sie die Datei {\tt dateien.zip} von Moodle herunter, und zwar so, dass die Datei {\tt dateien.zip} in dem neu angelegten Ordner {\tt BilderBearbeiten} abgespeichert ist.
	\item Die Datei {\tt dateien.zip} ist komprimiert und muss nun zuerst entpackt werden. Entpacken geht ganz einfach so:
	      \begin{description}
		      \item[Windows:] \url{https://tinyurl.com/MSWindowsUnzip}
		      \item[macOS:] \url{https://tinyurl.com/macOSunzip}
	      \end{description}
...
\begin{questions}
	...
	Schreiben Sie eine Python-Funktion \pythoninline{def count_black_pixels(filename)}, welche das ein Schwarzweissbild mit dem Dateinamen \pythoninline{filename} als Argument erhält und die Anzahl der schwarzen Pixel in diesem Bild zählt und diese Anzahl mit \pythoninline{return} zurück gibt. Betrachten Sie dazu unbedingt die Vorlage.

	\question{\textbf{Schwarzweissbild invertieren}}

	Schreiben Sie eine Python-Funktion \pythoninline{def invert_black_white_image(filename)}, welche ein Schwarzweissbild mit dem Dateinamen \pythoninline{filename} als Argument erhält und dieses Bild invertiert: Alle weissen Pixel (Pixel mit Wert $1$) sollen schwarz werden und alle schwarzen Pixel (Pixel mit Wert $0$) sollen weiss werden.
	...
	Schreiben Sie eine Python-Funktion \pythoninline{def random_black_white_image(width, height)}, welche ein Schwarzweissbild mit den gegebenen Dimensionen (Breite / Höhe) erzeugt.
	Jeder Pixel soll dabei zufällig entweder schwarz (0) oder weiss (1) gewählt werden. Der Aufruf
	\begin{center}
		\pythoninline{random_black_white_image(400, 600)}
	\end{center}
	...
	Schreiben Sie ein Python-Funktion \pythoninline{def increase_green(filename)}, welche ein RGB-Bild mit dem Dateinamen \pythoninline{filename} als Argument erhält und den Grünanteil dieses RGB-Bilds um 20 Prozent verstärkt / erhöht.

	Stellen Sie das originale Bild und das transformierte Bild nebeneinander dar.

	(Bei Bildern mit Grünanteil von $> 212$ wird die Erhöhung natürlich weniger als 20 Prozent betragen.)
	...
	Schreiben Sie ein Python-Funktion \pythoninline{def sepia_filter(filename)}, welche ein RGB-Bild mit dem Dateinamen \pythoninline{filename} als Argument erhält und den Sepia-Effekt auf das Bild anwendet.
	...
	Schreiben Sie ein Python-Funktion \pythoninline{def vertical_stripes(filename)}, welche ein Graustufenbild mit dem Dateinamen \pythoninline{filename} als Argument erhält. Die Funktion soll \enquote{vertikale Streifen} in gleichmässigen Abständen durch das Bild legen. Ein Beipiel dieser Transformation ist in \cref{fig:streifen} gegeben. Hier haben wir vertikale Streifen mit einer Breite von 2 Pixeln gewählt. Der Abstand (Anfang zu Anfang) der Streifen ist 7 Pixel (also 5 unveränderte Pixel zwischen Ende eines Streifens und dem Anfang des nächsten Streifens).
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.5\textwidth]{GrundlagenInfo/00_Programmieren/BilderBearbeiten/Vertikale_Streifen.PNG}
	...
	Schreiben Sie eine Python-Funktion \pythoninline{def is_prime(number)}, welche für eine gegebene natürliche Zahl \pythoninline{number} entscheidet, ob \pythoninline{number} eine Primzahl ist (\pythoninline{return True}) oder nicht (\pythoninline{return False}).
