...
\begin{myexample}[Kleinste Zahl in einer Liste]\label{myexample:kleinste_zahl}
...
\begin{myexercise}
    Verändern Sie den Code aus \cref{myexample:kleinste_zahl} so, dass die Funktion \lstinline|finde_kleinste_zahl| nicht nur die kleinste Zahl in einer Liste von Zahlen ausgibt, sondern auch deren Position (Index) in der Liste.
    \begin{myanswer}
...
Bei solchen, etwas komplexeren Codes, kann es nützlich sein, sich die Entwicklung der Variablenwerte im Verlauf der Ausführung des Codes von Hand zu notieren (s. \cref{tab:binary_search}).
...
\begin{myexercise}
    ...
    \begin{enumerate}
        ...
        \item 6
    \end{enumerate}
    Zeichnen Sie eine Zeit-Tabelle wie in \cref{tab:binary_search} und überprüfen Sie Ihre Resultate, indem Sie zwischen den Zeilen Zeilen 9 und 10 \lstinline|print|-Befehle verwenden, um die Werte von \lstinline|links|, \lstinline|mitte| und \lstinline|rechts| auszugeben.
\end{myexercise}
