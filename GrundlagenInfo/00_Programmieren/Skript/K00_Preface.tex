...
\subsection{MacOS}
Um VSCodium unter MacOS zu installieren benötigen Sie zuerst den Paket-Manager \texttt{brew}. Was macht \texttt{brew}? Laut der \href{https://brew.sh/de/}{offiziellen Webseite}: ``Homebrew installiert Zeug, das du brauchst, das Apple aber nicht mitliefert.'' 

Falls Sie \texttt{brew} noch nicht auf Ihrem Mac installiert haben, tun Sie dies wie folgt:
\begin{enumerate}
\item Öffnen Sie ein neues Terminal-Fenster (Spotlight-Suche mit \keys{\cmd+\space}, Suchbegriff ``Terminal'')
\item Geben Sie folgende Code-Zeile ein und führen Sie diese aus:
...
\subsection{Windows}
Öffnen Sie das Programm ``Powershell'' (Sie können mit dem \keys{\faWindows}-Knopf danach suchen) und geben Sie folgende Zeile ein, um den Package-Manager \texttt{winget} zu installieren:
\begin{lstlisting}[language=bash, numbers=none]
...
Sie können nun mit dem \keys{\faWindows}-Knopf das Programm-Menu öffnen und nach ``VSCodium'' suchen, welches nun installiert sein sollte.