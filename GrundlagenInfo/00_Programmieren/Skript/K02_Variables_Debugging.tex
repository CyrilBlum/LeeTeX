...
In \cref{myexercise:Flaggen} haben Sie bereits die (quadratische) Flagge des Kantons Zürich mithilfe der Turtle gezeichnet. Ohne Beachtung der Farben könnte der Code dafür wie in \cref{listing:ohneVariable} aussehen.

\lstinputlisting[language=Python,caption=\texttt{ohneVariable.py},label=listing:ohneVariable]{GrundlagenInfo/00_Programmieren/Skript/Code/K02_Variables/ohneVariable.py}
...
Viel besser ist es, wenn wir der Seitenlänge des Quadrats einen Namen geben. Genau dies haben wir in \cref{listing:mitVariable} getan: Wir haben der Länge den Namen \pythoninline{laenge} gegeben.
\lstinputlisting[language=Python,caption=\texttt{mitVariable.py},label=listing:mitVariable]{GrundlagenInfo/00_Programmieren/Skript/Code/K02_Variables/mitVariable.py}
...
\begin{mysummary}[Variablennamen]
	Bei der Namensgebung für Variablen gibt es einige Punkte zu beachten:
	\begin{enumerate}
		...
		\item Die Variablennamen müssen in einem Wort geschrieben werden. Falls ein Name aus zwei oder mehr Worten gewählt wird, sollte die Variable durch ein \textit{Underscore} (\lstinline{_}) getrennt werden.
		\item Es sollte ein \textbf{sinnvoller} und \textbf{sprechender} (beschreibender) Name sein! Wenn Sie beispielsweise ihre Körperhöhe in einer Variable speichern, ist es sinnvoll, diese \lstinline|h| oder \lstinline|hoehe| zu nennen und nicht \lstinline|x|.
	\end{enumerate}
\end{mysummary}
...
\begin{myexercise}\label{ex:austausch-fix}
	...
	x = 5 
	y = 11
	# nun wollen wir die Werte austauschen, so dass y den Wert 5 
	# und x den Wert 11 hat!
...
\begin{itemize}
	\item Wir wollen zu Beginn 22 durch 5 berechnen. Die Zahl 22 ist nicht vollständig durch 5 dividierbar: Wir erhalten keine Ganzzahl als Resultat, sondern eine Zahl mit einer Nachkommastelle (4.4).
...
Diese Operation kann beispielsweise verwendet werden, um zu bestimmen, ob eine Zahl durch eine andere dividierbar ist, also ob eine Zahl ein Teiler ist. In diesem Fall erhalten wir nämlich keinen Rest.
...
\section{*Exkurs: Kongruenz modulo}
% \begin{myexample}\label{myexample:Wochen}
% Dieses Beispiel handelt von Kalenderrechung und Terminen und illustriert somit eine typische Überlegung aus dem Alltag.
%
% Zunächst einmal wird es sinnvoll sein, die sieben Wochentag zu nummerien.
% \begin{table}[H]
% 	\centering
% 	\begin{tabular}{c|c}
% 		Wochentag  & Nummer \\\midrule\midrule
% 		Montag     & 0      \\\midrule
% 		Dienstag   & 1      \\\midrule
% 		Mittwoch   & 2      \\\midrule
% 		Donnerstag & 3      \\\midrule
% 		Freitag    & 4      \\\midrule
% 		Samstag    & 5      \\\midrule
% 		Sonntag    & 6
% 	\end{tabular}
% \end{table}
%
% Der 01. Januar 2018 war ein Montag. Wir nennen den TODO
% \end{myexample}
%
% \begin{myexcursion}[Kongruenz]
% 	TODO
% \end{myexcursion}
...
\begin{myexercise}\label{ex:spirale-increment}
	Zeichnen Sie eine Spirale, indem Sie folgenden Code vervollständigen:
	\lstinputlisting[language=Python,caption=\texttt{spirale.py},label=listing:Arithmetische]{GrundlagenInfo/00_Programmieren/Skript/Code/K02_Variables/spirale_fill.py}

	Die Spirale sollte am Ende folgendermassen aussehen:
	\begin{figure}[H]
		\centering
	...
	\begin{myanswer}
	\lstinputlisting[language=Python,caption=\texttt{spirale.py},label=listing:Arithmetische]{GrundlagenInfo/00_Programmieren/Skript/Code/K02_Variables/spirale.py}
	\end{myanswer}
\end{myexercise}

\begin{mysubmission}
Geben Sie die Kubikzahlen von 1, 2, 3, $\cdots$ bis 10 aus, also 1, 8, 27 etc., indem Sie eine Variable \texttt{x} verwenden, die am Anfang den Wert 1 hat und bei jeder Schleife um 1 vergrössert wird.
\begin{myanswer}
...
\begin{mysubmission}\label{ex:name}
	Schreiben Sie ein Programm, das Sie nach Ihrem Namen fragt, indem der Hilfstext \lstinline|"Wie heissen Sie?"| auf der Konsole ausgegeben wird. Verwenden Sie dazu die Funktion \lstinline|name = input("Hilfstext hier...")|. Danach soll Sie das Programm neun Mal begrüssen, indem es auf 3 Zeilen je dreimal den Text ``\texttt{Hallo [Ihr Name]}'' druckt.
	\begin{myanswer}
		\begin{lstlisting}
...
\begin{mysubmission}\label{ex:str}
	Eine Person verrät uns die Vorwahl ihrer $10$-stelligen Mobiltelefonnummer. Zudem verrät Sie Ihnen auch, dass die letzten drei Ziffern eine \textit{gerade} Zahl zwischen 0 und 200 sind (entweder 000, 002, 004, ... oder 200).
	...
	\end{itemize}
	\begin{myanswer}
		\lstinputlisting[language=Python,caption=\texttt{telefonnummer.py},label=listing:ohneVariable]{GrundlagenInfo/00_Programmieren/Skript/Code/K02_Variables/telefonnummer.py}
	\end{myanswer}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.5\linewidth]{GrundlagenInfo/00_Programmieren/Skript/Figures/Fibonacci}
\caption{}
\label{fig:fibonacci}
\end{figure}
...
\begin{myanswer}
\lstinputlisting[language=Python,caption=\texttt{challenge\_fib.py},label=listing:ohneVariable]{GrundlagenInfo/00_Programmieren/Skript/Code/K02_Variables/challenge_fib.py}
\end{myanswer}
\end{mychallenge}