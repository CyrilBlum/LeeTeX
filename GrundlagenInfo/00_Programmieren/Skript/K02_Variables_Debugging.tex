...
\begin{myoverview}[Benennung von Variablen]
	...
	\begin{enumerate}
		...
		\item Variablennamen dürfen keine Leerzeichen enthalten. Falls ein Name aus zwei oder mehr Worten gewählt wird, sollte die Variable durch ein \textit{Underscore} (\lstinline{_}) getrennt werden.
		\item Der Name sollte ein \textbf{sinnvoller} und \textbf{sprechender} (beschreibender) Name sein. Wenn Sie beispielsweise die Höhe eines Hauses in einer Variable speichern, ist es sinnvoll, diese \lstinline|h| oder \lstinline|hoehe| zu nennen und nicht etwa \lstinline|x|.
...
\end{itemize}
Für mehr Details zur Division mit Rest siehe \cref{sec:DivisionMitRest} in \cref{ch:AppendixA}.
\end{mydefinition}
