...
Um mit der Turtle arbeiten zu können, müssen Sie das Turtle-Modul durch den Befehl \pythoninline{import turtle} zu Beginn des Programms (ganz oben) importieren. Mit dem ausdruck \pythoninline{import turtle as t} sagen wir, dass wir das Modul \pythoninline{turtle} in unser Programm importieren und dieses Modul innerhalb des Programms mit dem Ausdruck \pythoninline{t} benennen werden (wird könnten auch \pythoninline{import turtle as bliblablup} schreiben, dies wäre aber weniger praktisch).
...
\end{center}
\textit{Schleifenkopf}\index{Schleife!Kopf einer} der Schleife genannt. Die vier Zeilen (6 -- 9) werden \textit{Schleifenkörper}\index{Schleife!Körper einer} der Schleife genannt. Der Schleifenkörper ist (in Python) daran erkennbar, dass er (relativ zu dem Schleifenkopf) nach rechts eingerückt (Englisch: \textit{indented}) ist. Diese Einrückung erreichen wir in VSCodium durch das einfache Drucken der Tabulator-Taste (\keys{\tab}).


\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\textwidth]{GrundlagenInfo/00_Programmieren/Skript/Figures/approximation_kreis.pdf}
	\caption{Approximation (Annäherung) an einen Kreis durch regelmässige Polygone (links: 5-Eck, mittels: 6-Eck, rechts: 8-Eck)}
