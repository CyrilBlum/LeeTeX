...
\item Sobald Sie Ihren Browser aufgerufen haben, geben Sie eine Web-Adresse ein. Eine solche Web-Adresse wird im Allgemeinen \textbf{\acrfull{URL}} genannt und besteht aus folgenden Komponenten:
\[
...
\item Der zweite Teil (\texttt{www.beispiel.ch}) bezeichnet dabei den \textit{Server} $\mathcal{S}$, von dem wir die Datei anfordern. Die Endung (\texttt{.ch}) steht für eine sogenannte \acrfull{TLD}, in diesem Fall wird durch ``\texttt{.ch}'' eine Schweizer Webseite bezeichnet.
...
\subsubsection{TCP}
% TCP
\acrfull{TCP} = Übertragungssteuerungsprotokoll
\begin{figure}
\centering
...
\end{tabular}
\caption{Typischer \gls{TCP}-Paket-Aufbau}
\end{table}
...
\item Wird meist dezimal angegeben, z.B. \texttt{192.168.0.1}
...
\item Umrechner dezimal $\rightarrow$ binär: \url{https://www.rapidtables.com/convert/number/decimal-to-binary.html}
\end{itemize}
\begin{myanswer}
\begin{table}[H]
\centering
