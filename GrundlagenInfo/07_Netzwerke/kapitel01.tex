...
\subsubsection{DNS}
Wie wir bereits gesehen haben, besteht eine \gls{URL}, also eine Webseiten-Adresse, aus der Angabe eines Server-Namens und einem Dateipfad. Diese Datei fordert der Client von einem Server mittels dem \gls{HTTP}- oder \gls{HTTPS}-Protokoll an (s. \cref{fig:http-request}). Allerdings möchte man sich nicht für jeden Server die IP-Adresse merken müssen: ähnlich Ihrer Kontakteliste auf dem Mobiltelefon, welche dazu dient, dass Sie sich nicht alle Telefonnummern auswending merken müssen, ist eine \gls{DNS} essentiell eine Liste von merkbaren Webseiten-Namen (wie beispielsweise \texttt{sbb.ch}) und deren dazugehörigen \gls{IP}-Adressen, also den \gls{IP}-Adressen der Server, die die Webseiten-Datein speichern. Genauer gesagt ist ein \gls{DNS} ein Server, welcher den Clients eines Netzwerks eine Tabelle von Servern und damit verbundenen Namen zur Verfügung stellt. Eine DNS-Abfrage ist illustriert in \cref{fig:dns}. 
...
% Arrows
\draw[arrow] ([yshift=5mm]client) -- node[above] {\textbf{1.} \gls{DNS}-Anfrage\\\footnotesize``Wer ist \texttt{beispiel.ch}?''} ([yshift=5mm]dns);
\draw[arrow] ([yshift=-5mm]dns) -- node[below] {\textbf{2.} \gls{DNS}-Antwort\\\footnotesize``\texttt{192.168.0.11}''} ([yshift=-5mm]client);
