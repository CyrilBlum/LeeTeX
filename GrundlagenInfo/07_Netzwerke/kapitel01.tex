...
Beim Erstellen eines Netzwerks stellt sich als erstes die Frage, welche Computer man mit welchen anderen Computern verbinden soll. Die Strukture der Verbindungen in einem Netzwerk kann als umformuliert werden als Graphen-Aufgabe, bei der es zu bestimmen gibt, welche Knoten eines Graphen miteinander über eine Kanten verbunden werden. Die verschiedenen möglichen Strukturen eines Netzwerks werden auch als \textbf{Topologie} bezeichnet. Einige Beispiel-Topologien sind in \cref{fig:topologie} gezeigt.
\begin{figure}[H]
\centering
\centering
% Bus Topology
...
\begin{tikzpicture}
  ...
  \foreach \x in {1,2,3,4,5}{
    \foreach \y in {\x,...,5}{
      \ifnum\x=\y\relax % Skip self-connection
      \else
...
\begin{itemize}
\item \textbf{Ziel-IP und Netzmaske}: Diese beiden Spalten definieren, welche Subnetze dem Router bekannt sind (sowohl eigenes Subnetz wie andere Subnetze).
\item \textbf{Gateway}: Die Adresse, an die ein Paket geschickt werden muss, um sein Ziel zu erreichen. Das Gateway kann als eine Art ``Tor zur Aussenwelt'' angeschaut werden und ist häufig die \gls{IP}-Adresse des Routers.
\item \textbf{Interface}: Der Name der Netzwerkkarte, die mit der Ziel-Adresse (Gateway) verbunden ist. 
\end{itemize}
