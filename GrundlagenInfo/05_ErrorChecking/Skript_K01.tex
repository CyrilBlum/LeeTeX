...
$\rightarrow$ Jemand legt ein $n\cdot n$-Feld von Spiel-Karten aus, einige Karten mit dem Kopf nach oben, andere mit dem Kopf nach unten (hier: $n=4$):
\begin{figure}[H]
\centering
...
Anzahl Karten, die mit dem Bild nach oben liegen, spalten- und zeilenweise summieren:
\begin{itemize}
...
\centering
\begin{tabular}{c|c|c||c}
\underline{1} & \underline{1} & \underline{1} & \\\midrule\midrule
0 & 1 & 0 & \underline{1}\\\midrule
1 & 0 & 1 & \underline{0}\\
\end{tabular}
\end{table}

Die Kontrollbits sind so gewählt, dass jede Spalte und jede Zeile eine gerade Anzahl Einsen enthält. Wenn nun ein Bit geflippt wird, gibt es genau eine Spalte und eine Zeile, in welcher die Anzahl Einsen \textit{nicht} gerade ist, somit können wir den Ort des abgefälschten Bits lokalisieren.
