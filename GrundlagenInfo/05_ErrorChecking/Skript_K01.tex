...
Bisher haben wir gelernt, Kodierungen für gewissen Nachrichten zu finden, so dass ein gewisser Mindestabstand gewährleistet wird. Durch einen Mindestabstand von 2 (Bits) zwischen jedem Nachrichtenpaar wird gewährleistet, dass ein Fehler bestimmt erkannt wird, da es zwei Fehler (ungewollte Bit-Änderungen) bräuchte, damit aus einem gültigen Codewort ein anderes, gültiges Codewort wird. 
...
\begin{myexample}
Wenn wir die Kodierung von Xavier aus \cref{ex:fehler-kodierungen}
\end{myexample}
...
\begin{enumerate}
\item Wähle irgend ein Codewort als erstes Codewort aus, z.B., \texttt{0000}.
...
Dieses Vorgehen funktioniert auch für andere Code-Längen, beispielsweise 2 (\cref{fig:2d}), 3 2 (\cref{fig:3d}) oder 2 (\cref{fig:5d}).
\end{mydefinition}
...
\caption{Erster Schritt: Auslegen der Karten durch den Zauberer ($n=4$)}
\label{fig:zauberer-1}
\end{figure}


Eine im Voraus informierte Hilfs-Person legt nun noch eine weitere Zeile (oben) und eine weitere Spalte (rechts) dazu. Der Zauberer kann das Hinzurufen einer weiteren Person beispielsweise damit begründen, dass dies ``das Spiel noch etwas schwieriger machen soll'' (s. \cref{fig:zauberer-2}).
...
Anschliessend verlässt der Zauberer den Raum. Eine zuschauende Person darf nun genau eine Karte umdrehen (s. \cref{fig:zauberer-3}). 
\begin{figure}[H]
\centering
\AffCartesJeu[Hauteur=1.3,StyleJeu=v1]{Dos § 8C § Dos § VP § Dos}\\
\AffCartesJeu[Hauteur=1.3,StyleJeu=v1]{DK § Dos § 10T § Dos § Dos}\\
...
Der Zauberer betritt nun wieder den Raum. Um zu erraten, welche Karten umgedreht wurden, zählt der Zauberer nun, wie viele Karte mit dem Kopf nach oben in jeder Zeile und Spalte liegen.
\begin{itemize}
\item Zeilen: 2, 2, 4, \textbf{3}, 2
\item Spalten: 2, 2, \textbf{3}, 4, 2
\end{itemize}

Die umgedrehte Karte muss sich also genau auf der vierten Zeile und in der dritten Spalte befinden! 
...
Um das neue Codewort  mit Korrektur zu bilden, wird wie folgt vorgegangen: Fügen Sie dem ursprünglichen Code-Wort zuerst die erste Zeile, und danach die letzte Spalte (ohne erstes Bit) an.
\end{table}
...
\subsection{RAID 5: Paritätsbits}

RAID 5 und RAID 6 nutzen für die Datensicherheit statt mirroring einen effizienteren Ansatz, um Daten über mehrere Laufwerke zu verteilen, indem sie Paritätsbits verwenden. Dabei verwendet RAID 5 ein einzelnes Paritätsbit, welches es über verschiedene Festplatten verteilt, so dass jeder ``Striping-Block'' $A_1A_2A_3\cdots$ über genau ein Paritätsbit $A_p$ verfügt (\cref{fig:raid5}).
...
\begin{myexercise}
Marina gründet eine IT-Firma und kauft sich 8 Festplatten für die Speicherung seiner Daten. Da er viele Daten hat, möchte er möglichst viel nutzbaren Speicherplatz haben, gleichzeitig möchte er gegen den Ausfall von \textit{einer} Festplatte geschützt sein. Welche RAID-Konfiguration empfehlen Sie Marcel?
\begin{myanswer}
\begin{itemize}
    \item \textbf{Empfohlen: RAID 5} – Maximiert Speicherplatz, schützt vor \textbf{1 Plattenausfall}.
...
\begin{myexercise}
Bestimmen Sie für den Fall, dass sie 6 Festplatten gleicher Grösse besitzen, den verfügbaren Speicherplatz für die Konfigurationen RAID 0, RAID 1, RAID 5 und RAID 6. Geben Sie den verfügbaren Speicherplatz als Bruch an (z.B. 3/6). Geben Sie zudem die Ausfallsicherheit an (wie viele Festplatten können ausfallen ohne Datenverlust?).
