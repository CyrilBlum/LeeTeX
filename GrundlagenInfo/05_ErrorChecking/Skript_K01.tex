...
\begin{myexercise}
Bestimmen Sie für den EAN-Code 4030538657465 alle Paare von benachbarten Ziffern, deren Austausch in der Reihenfolge nicht erkannt werden kann.
\end{myexercise}

\begin{myexercise}
Nehmen wir an, dass an der vierten Position in diesem Code eine 8 statt der 0 getippt wurde. Finden Sie mindestens 5 Möglichkeiten, an anderen Stellen den Fehler so zu machen, dass die fehlerhafte Darstellung einem EAN-Code entspricht und somit dieser Doppelfehler unbemerkt bleibt.
\end{myexercise}
...
\begin{table}[H]
    ...
    \begin{tabular}{c|c|c|c|c}
        ...
        \midrule\midrule
        \textbf{Weiss} & 00000000 & 000000 & 000 & 001 \\
        \midrule
        \textbf{Gelb} & 01010101 & 011011 & 011 & 010 \\
        \midrule
        \textbf{Blau} & 10101010 & 101101 & 101 & 100 \\
        \midrule
        \textbf{Schwarz} & 11111111 & 110110 & 110 & 111 \\
    \end{tabular}
    \caption{Kodierungstabelle}
    \label{tab:kodierung}
\end{table}
...
\texttt{00000, 00110, 11001, 11111}. Bei welchen der folgenden Bitfolgen kann man eindeutig den Fehler korrigieren, falls man sicher ist, dass genau ein Fehler passiert ist?
...
\begin{myexercise}
Zeichnen Sie alle 1-Fehler-erkennenden Kodierungen im unten stehenden Hyperwürfel ein, ausgehend vom Code \texttt{011}. Wie viele Code-Wörter können Sie finden?
\begin{figure}[H]
\centering
\begin{tikzpicture}
\HyperCube{3}
...
\begin{myexercise}
Zeichnen Sie alle 1-Fehler-korrigierenden Kodierungen im unten stehenden Hyperwürfel ein, ausgehend vom Code \texttt{0000}. Wie viele 1-Fehler-korrigierenden Codewörter gibt es?
\begin{figure}[H]
\centering
\begin{tikzpicture}
\HyperCube{4}
...
Der \textbf{XOR-Operator} (exklusives Oder, $\oplus$) ist eine logische Verknüpfung, die zwei Eingangsbits vergleicht. Das Ergebnis ist genau dann 1, wenn die beiden Eingangsbits unterschiedlich sind. Die Wahrheitstabelle für XOR sieht wie folgt aus:
...
\subsection{Eigenschaften von XOR}
Der XOR-Operator hat einige wichtige Eigenschaften:

\begin{itemize}
    \item \textbf{Selbstinverse}: \( A \oplus A = 0 \) (ein Bit mit sich selbst XOR-verknüpft ergibt immer 0).
    \item \textbf{Identität}: \( A \oplus 0 = A \) (ein Bit XOR mit 0 bleibt unverändert).
    \item \textbf{Kommutativität}: \( A \oplus B = B \oplus A \).
    \item \textbf{Assoziativität}: \( A \oplus (B \oplus C) = (A \oplus B) \oplus C \).
\end{itemize}



\subsection{XOR in der RAID-Paritätsberechnung}
In RAID 5 wird XOR genutzt, um das Paritätsbit zu berechnen. Gegeben seien drei Datenblöcke:
...
\subsection{Anwendungsbereiche von XOR}
Der XOR-Operator wird in vielen Bereichen der Informatik verwendet, unter anderem:

\begin{itemize}
    \item \textbf{Fehlererkennung und -korrektur} (z. B. RAID, Hamming-Code).
    \item \textbf{Kryptographie} (z. B. One-Time Pad, Verschlüsselungsalgorithmen).
    \item \textbf{Bitmanipulation} in Algorithmen und Datenstrukturen.
\end{itemize}

\begin{myexercise}
Gegeben sind die folgenden binären Zahlen:
...
\begin{myexercise}
Gegeben sind vier binäre Zahlen:
...
\subsection{RAID 0: Striping ohne Redundanz}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.3\linewidth]{GrundlagenInfo/05_ErrorChecking/Figures/RAID_0}
...
\subsection{RAID 1: Spiegelung für hohe Ausfallsicherheit}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.3\linewidth]{GrundlagenInfo/05_ErrorChecking/Figures/RAID_1}
\caption{RAID-1-Architektur mit zwei physischen Festplatten}
\label{fig:raid1}
\end{figure}

Absicherung gegen Datenverlust kann mit lediglich 2 Festplatten nicht Speicherplatz-effizienter als mit RAID-1 erfolgen, d.h., ein Verlust von 50\% des physischen Speicherplatzes muss in Kauf genommen werden. Effizientere Lösungen bieten sich an, wenn weitere Speicherplatten vorhanden sind.
...
\subsection{RAID 5 und RAID 6: Redundanz mit Paritätsbits}

RAID 5 und RAID 6 nutzen Paritätsbits zur Fehlerkorrektur, wobei RAID 5 ein einzelnes Paritätsbit und RAID 6 zwei Paritätsbits verwendet.


\begin{figure}[H]
\centering
\includegraphics[width=0.5\linewidth]{GrundlagenInfo/05_ErrorChecking/Figures/RAID_5}
\caption{RAID-5-Architektur mit fünf physischen Festplatten und einem Paritätsbit}
\label{fig:raid5}
\end{figure}
...
\subsubsection{RAID 5: Berechnung eines Paritätsbits}
...
Nehmen wir an, die Datenblöcke sind in Binärform gegeben:
\begin{align*}
    D_1 &= 1101_2 \\
    D_2 &= 1011_2 \\
    D_3 &= 0110_2 \\
...
\subsubsection{RAID 6: Berechnung von zwei Paritätsbits}
...
- RAID 5 berechnet eine XOR-Parität und kann den Ausfall einer Festplatte tolerieren.
- RAID 6 nutzt zusätzlich eine zweite Parität über ein Galois-Feld, sodass zwei gleichzeitige Festplattenausfälle kompensiert werden können.
