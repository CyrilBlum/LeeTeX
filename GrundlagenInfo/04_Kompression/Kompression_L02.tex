...
% Draw the initial line
    ...
    \foreach \i in {1,...,4}{
	    \pgfmathsetmacro{\inc}{\increments[\i-1]}
	    \pgfmathsetmacro{\l}{\letters[\i-1]}
	    \pgfmathsetmacro{\xStart}{\xPosL[\i-1]}
	    \pgfmathsetmacro{\xEnd}{\xPosR[\i-1]}

		% draw vertical marks on main initial line
        \draw (\xStart*10,#1+0.2) to (\xStart*10,#1-0.2);
        \draw (\xEnd*10,#1+0.2) to (\xEnd*10,#1-0.2);
        %\node (\l-Perc) at (\xStart*10,.2) {\inc \%};
        % Increase y position for the next line 
        
        \draw[decoration={brace,mirror,raise=5pt},decorate]
                (\xStart*10,#1-.2) -- node[below=6pt] {\l{} (\pgfmathparse{int(\inc*100)}\pgfmathresult \%)} (\xEnd*10,#1-.2);       
        
    }
}
\begin{tikzpicture}
\arithmeticTikz{1}{{0.5,0.2,0.2,0.1}}{{0,0.5,0.7,0.9}}{{0.5,0.7,0.9,1}}{{"A","B","C","D"}}{{0,0.5,0.7,0.9}}
\arithmeticTikz{3}{{0.5,0.2,0.2,0.1}}{{"A","B","C","D"}}
