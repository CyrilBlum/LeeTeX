...
\begin{myexercise}\label{myexercise:0401}
	...
	\lstinline|print_binary_without_11(n, w = '')|.
...
\begin{myexercise}\label{myexercise:0403}
	...
	\begin{align*}
		w = xab,
	\end{align*}
	wobei $x\in L_{n-1}$ und $a,b\in\lrc{0,1}$. Unterscheiden Sie zwei Fälle $b = 0$ und $b = 1$ für das Symbol $b$. Drücken Sie $N(n+1)$ durch $N(n)$ und $N(n-1)$ aus.
\end{myexercise}
...
$a_{5000}$ nämlich alle Glieder $a_{1}, a_{2}, \ldots, a_{4999}$ zuerst bestimmt werden. Wie können wir späte Folgenglieder (mit hohen Indizes) effizienter berechnen? Die folgende \cref{myexercise:0404} wird sich mit dieser Frage beschäftigen.

\begin{myexercise}\label{myexercise:0404}
	Finden Sie eine \enquote{Formel}, welche $a_n$ mit lediglich einer Multiplikation und einer Addition berechnet, ohne zuerst die Vorgänger $a_{1}, a_{2}, \ldots, a_{n}$ bestimmen zu müssen. Berechnen Sie mithilfe dieser Formel das Folgenglied $a_{5000}$.
\end{myexercise}
\noindent
Die in \cref{myexercise:0404} gefundene (nicht rekursive) Darstellung von $a_n$ wird \tib{explizite Darstellung}\index{Folge!explizite Darstellung einer} von $a_n$ genannt.
...
\cref{eq:binet} enthält die irrationale Zahl $\sqrt{5}$. Ist es nicht erstaunlich, dass $F(n)$ für alle $n\in\N$ eine natürliche Zahl ist? In der anspruchsvollen \cref{myexercise:0477} haben Sie die Gelegenheit zu beweisen, dass die Formel von Moivre-Binet tatsächlich die Fibonacci-Zahlen berechnet (und somit ausschliesslich natürliche Zahlen generiert).

\begin{myexercise}\label{myexercise:0478}
	Berechnen Sie $F(0)$ und $F(1)$ mithilfe von \cref{eq:binet}. Überzeugen Sie sich, dass $F(0)$ und $F(1)$ die ersten beiden Fibonacci-Zahlen und somit insbesondere natürliche Zahlen sind.
\end{myexercise}

\clearpage
\begin{myexercise}\label{myexercise:0477}
...
\begin{myremark}[Moivre-Binet]\label{mybemerkung:moivre}
...
\begin{myexercise}\label{myexercise:0406}
	Verwenden Sie die Formel von Moivre-Binet, um eine explizite Darstellung von $N(n)$ zu finden und schreiben Sie eine Python-Funktion \lstinline|def N(n)|, welche $N(n)$ für gegebenes $n\in\N$ berechnet. Verwenden Sie dazu \cref{eq:moivrecomp}.
\end{myexercise}
...
\begin{myanswer}[myexercise:0478]
	Wir berechnen
	\begin{align*}
...
\begin{myanswer}[myexercise:0477]
	...
	\end{align*}
	In \cref{myexercise:0478} haben wir bereits die Korrektheit der Aussage für $n=0$ und $n=1$ gezeigt. Sei nun die Aussage für ein $n\in\Nunit$ wahr.
	\begin{align*}
