...
\begin{description}[<+->]
    \item[Lektion 1:]
        ...
        \end{itemize}
    \item[Lektion 2:] Arbeiten am Game in Gruppen
    \item[Lektion 3:] Backtracking und Sudoku
\end{description}
\end{frame}
...
\lstset{basicstyle=\ttfamily\scriptsize}
\begin{lstlisting}[language=Python,caption=rekursive Berechnung der Fibonacci-Folge mit Memoization,numbers=none]
...
\begin{frame}[fragile]{Aufgaben}
\aufgabebox{Lösen Sie \enquote{Aufgaben Rekursion 3} auf Moodle (ca. 10 Minuten)}
\end{frame}
...
\begin{frame}[fragile]{Aufgaben (falls schon fertig)}
\aufgabebox{Lösen Sie \enquote{Aufgaben Rekursion 2} auf Moodle}
\end{frame}