...
\begin{myexercise}\label{myexercise:0601}
...
\begin{myexercise}\label{myexercise:0677}
	Finden Sie die drei weiteren Lösungen des Sudokus in \cref{fig:Mini0}.
\end{myexercise}

\section{Darstellung von Sudokus in Python}
In Python können wir ein Sudoku durch eine \enquote{zweidimensionale} Liste (oder Array) angeben. Beispielsweise kann Sudoku Nr. 2 aus \cref{myexercise:0601} in der folgenden Form in Python gespeichert werden:
...
\end{lstlisting}
Um ein $n^2\times n^2$-Gitter ansehnlich in Python auszugeben (\enquote{pretty-print}), empfehlen wir, am Anfang des Programms die Bibliothek \lstinline|numpy| durch den Befehl \lstinline|import numpy as np| einzubinden. Danach kann das Gitter durch \lstinline|print(np.array(gitter))| übersichtlich ausgegeben werden.
...
\clearpage
\begin{myexercise}\label{myexercise:0688}
	Wir befassen uns in dieser Aufgabe nur mit $4\times 4$-Gittern. Entwickeln Sie eine Python-Funktion
	\begin{center}
		\lstinline|def erlaubt(zeile, spalte, ziffer, gitter)|,
	\end{center}
	welche für ein gegebenes noch \textbf{leeres} (markiert durch die Zahl 0) Feld mit Zeilenindex \lstinline|zeile| (von 0 bis 3) und Spaltenindex \lstinline|spalte| (von 0 bis 3) prüft, ob das Platzieren einer gegebenen Ziffer \lstinline|ziffer| (von 1 bis 4) in einem gegebenen Gitter erlaubt ist. Hier sind einige Testfälle:
...
# (bereits eine 1 in dem ersten 2x2 Block)
...
\begin{myexercise}\label{myexercise:0689}
...
Sudokus herantasten. Dazu beginnen wir mit der einführenden \cref{myexercise:0632}, in der Sie einige gedankliche Vorarbeiten leisten.

\begin{myexercise}\label{myexercise:0632}
...
\clearpage
Nun wollen wir die in \cref{myexercise:0632} gemachten Beobachtungen etwas vertiefen. Betrachten Sie dazu das neue Mini-Sudoku in \cref{fig:Mini1}.
...
Der Vorgang des \enquote{Zurückgehens} entlang der gegangenen Wege wird \tib{Backtracking}\index{Backtracking} genannt. Im nächsten Abschnitt werden wir einen eleganten Algorithmus vorstellen, welcher mithilfe von Backtracking auf rekursive Weise die Lösungen eines Sudokus findet.
...
\clearpage
\begin{myremark}[Backtracking in einem Labyrinth]\label{myremark:Metapher2}
...
Unser Lösungsalgorithmus verwendet die Funktion \lstinline|erlaubt| sowie Backtracking in Kombination mit Rekursion. Diese drei Komponenten sind Ihnen inzwischen bekannt. Der vollständige Algorithmus zum Lösen von Sudokus ist in \cref{listing:sudoku} gegeben. Mit der Definition
\begin{lstlisting}[language=Python,caption=Abspeichern von Sudoku-Gittern in Python,numbers=none]
  sudokuNo0 = [
    [0,0,7,0,0,3,9,0,2],
    [0,0,0,8,0,0,0,0,0],
...
\end{lstlisting}
wird Ihnen der Aufruf \lstinline|sudoku(sudokuNo0)| alle (es gibt hier nur eine) Lösungen von Sudoku Nr. 0 ausgeben. Die Funktion in \cref{listing:sudoku} verwendet die Funktion \lstinline|erlaubt|, welche Sie in \cref{myexercise:0689} geschrieben haben.

\lstset{style=BaselineStyle}
\begin{lstlisting}[language=Python,caption=Implementation der Funktion \lstinline|sudoku|,label=listing:sudoku]
def sudoku(gitter):
...
\begin{myexercise}\label{myexercise:0681}
	Betrachten Sie die \lstinline|sudoku|-Funktion in \cref{listing:sudoku}. Es gibt genau zwei verschiedene Möglichkeiten, die Zeile 24 in dieser Funktion zu erreichen. Nennen Sie diese beiden Möglichkeiten und erklären Sie jeweils die Bedeutung des Entfernens der gesetzten Ziffer in dem entsprechenden Fall.
\end{myexercise}

\begin{myexercise}\label{myexercise:0603}
	(!) Ändern Sie die \lstinline|sudoku|-Funktion in \cref{listing:sudoku} dahingehend ab, dass genau eine Lösung ausgegeben wird, falls das Sudoku mindestens eine Lösung besitzt. Falls das Sudoku keine Lösung besitzt, so soll auch nichts ausgegeben werden.
\end{myexercise}

%\lstset{basicstyle=\ttfamily\tiny}
\begin{myexercise}\label{myexercise:0604}
...
\begin{myanswer}[myexercise:0601]
	\begin{figure}[H]
		\centering
		\begin{tikzpicture}[scale=.5]
			\begin{scope}
...
\begin{myanswer}[myexercise:0677]
	\begin{figure}[H]
		\centering
		\begin{tikzpicture}[scale=.5]
...
\begin{myanswer}[myexercise:0688]
	\lstset{basicstyle=\ttfamily\footnotesize}
	\begin{lstlisting}[language=Python,caption=Implementation der Funktion \lstinline|erlaubt|]
...
\begin{myanswer}[myexercise:0689]
	\lstset{basicstyle=\ttfamily\footnotesize}
	\begin{lstlisting}[language=Python,caption=Funktion \lstinline|erlaubt| für allgemeine Sudokus]
def erlaubt(zeile, spalte, ziffer, gitter):
...
\begin{myanswer}[myexercise:0632]
	\begin{aenum}
...
\begin{myanswer}[myexercise:0681]
	Zeile 24 wird genau dann erreicht, wenn der rekursive Aufruf auf Zeile 21 abgeschlossen werden konnte. Dies ist genau dann der Fall, wenn der rekursive Aufruf einen \lstinline|return| lieferte. Dazu gibt es exakt zwei Möglichkeiten:
	\begin{description}
		\item[return auf Zeile 28:] Der \lstinline|return| auf Zeile 28 wird genau dann ausgelöst, wenn der rekursive Aufruf auf Zeile 21 in eine \enquote{Sackgasse} gelangt ist. Dies geschieht genau dann, wenn dieser Aufruf ein noch leeres Feld gefunden hat, in welches keine der 10 Ziffern gesetzt werden darf (Widerspruch). Die gesetzte Ziffer muss also wieder entfernt werden und wir versuchen es mit einer anderen Ziffer.
		\item[return auf Zeile 31:] Der \lstinline|return| auf Zeile 31 wird genau dann ausgelöst, wenn der rekursive Aufruf auf Zeile 21 in kein leeres Feld gefunden hat. Das bedeutet aber, dass dieser Aufruf eine gültige Lösung gefunden (und ausgegeben) hat. Durch das Entfernen der gesetzten Ziffer können rekursiv (falls vorhanden) weitere Lösungen gefunden werden (siehe auch \cref{myremark:Metapher2}).
	\end{description}
\end{myanswer}

\begin{myanswer}[myexercise:0603]
	Diese Funktion bricht ihre Berechnung ab, sobald eine Lösung gefunden wurde.
	\begin{lstlisting}[language=Python,caption=Implementation der Funktion \lstinline|sudoku|,label=listing:sudokuUnique]
def sudoku(gitter):
	for zeile in range(9):
		for spalte in range(9):
			if (gitter[zeile][spalte] == 0):
...
\begin{myanswer}[myexercise:0604]
	\lstset{basicstyle=\ttfamily\footnotesize}
	...
	\lstinputlisting[language=Python]{EF/Induktion_und_Rekursion/n_queen_problem.py}

	\textbf{Lösung in C++}
	\lstinputlisting[language=C++]{EF/Induktion_und_Rekursion/src/main.cpp}
	\lstinputlisting[language=C++]{EF/Induktion_und_Rekursion/src/n_queen_problem.h}
	\lstinputlisting[language=C++]{EF/Induktion_und_Rekursion/src/n_queen_problem.cpp}
\end{myanswer}
