...
\begin{itemize}
	\item Für \lstinline|bubble\_sort|: Überprüfe, ob das Array $L$ bereits sortiert ist.\footnote{Eine solche Überprüfung (pre-Check) kann meist in kurzer Zeit durchgeführt werden (z.B. in $\Theta(n)$, um die Ordnung in einem Array der Länge $n$ zu überprüfen bzw. in $O(n^2)$, um die Struktur einer Matrix zu überprüfen).}
...
\end{solution}
	...
	\begin{tabular}{|c|c|c|}
		...
		\hline
		$f \in Lmega(g)$ & $\liminf\limits_{n \to \infty} \abs{\frac{f(n)}{g(n)}} > 0$      & \makecell{$f$ wächst mindestens so schnell wie $g$, \\$g\in O(f)$}                          \\
		\hline
