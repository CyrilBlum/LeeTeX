...
% set date
\newcommand{\mydate}{09. April 2025}
...
\begin{questions}
	...
	\begin{parts}
		...
		Beweise oder widerlege die Behauptung oder finde ein Gegenbeispiel. Begründe deine Antwort!
		\begin{solution}
			richtig, $a_n\in O(b_n)$ impliziert, dass eine Konstante $C_0>0$ existiert mit $a_n / b_n < C_1$ und $b_n\in O(a_n)$ impliziert, dass eine Konstante $C_1>0$ existiert mit $b_n / a_n < C_1$. Wähle also $C:=\max{C_0, C_1}$.
		\end{solution}
		\droptotalpoints
	\end{parts}
	...
	\begin{parts}
		\part[1] $\lr{2n\sqrt{n}+10}\in\mathcal{O}\lr{n^2}$
		\begin{solution}
			Wegen
			\begin{align*}
		...
		\part[1] $\lr{5\cos\lr{m^2}m}\in\mathcal{O}\lr{m}$
		\begin{solution}
			Wegen
			\begin{align*}
		...
		\part[1] $2^{n+1}\in\mathcal{O}\lr{2^n}$
		\begin{solution}
			Wegen
			\begin{align*}
		...
		\part[1] $2^{2n}\in\mathcal{O}\lr{2^n}$
		\begin{solution}
			Es gilt
			\begin{align*}
	...
	\begin{parts}
		...
		\begin{checkboxes}
			\choice $\Omega\lr{n^3}$
			\choice $\Theta\lr{n^3}$
			\choice $\mathcal{O}\lr{n^3}$
			\correctchoice keine der obigen Antworten ist richtig
		\end{checkboxes}
		\begin{solution}
			Wir wissen, dass es Algorithmen gibt, welche eine gewünschte Matrix-Matrix-Multiplikation in $\mathcal{O}\lr{n^{2.38}}$ implementieren. Damit ist die erste Aussage sicherlich falsch.
			Der Algorithmus
