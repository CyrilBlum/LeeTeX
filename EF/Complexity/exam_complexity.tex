...
\begin{questions}
	...
	\begin{parts}
		...
		Beweise oder widerlege die Behauptung oder finde ein Gegenbeispiel. Begründe deine Antwort!
		\begin{solution}
			richtig, $a_n\in O(b_n)$ impliziert, dass eine Konstante $C_0>0$ existiert mit $a_n / b_n < C_0$ und $b_n\in O(a_n)$ impliziert, dass eine Konstante $C_1>0$ existiert mit $b_n / a_n < C_1$. Wähle also $C:=\max\lr{C_0, 1/C_1}$.
		\end{solution}
		\droptotalpoints
	\end{parts}
	...
	\begin{solution}
		Sei $A$ ein gegebener Algorithmus. Wir wollen $A$ modifizieren. Der modifizierte Algorithmus soll $B$ heissen. $B$ erstellt einen beliebigen Output im gewünschten Format und überprüft, ob dieser Output das Ziel von $A$ erfüllt.	Falls dies der Fall ist, dann ist $B$ mit der Arbeit fertig (best-case). Anderenfalls verwendet $B$ einfach, wie gewohnt, den Algorithmus $A$, um das Problem zu lösen. Die best-case Laufzeit reduziert sich also lediglich auf das Überprüfen einer Lösung, nicht jedoch auf das Produzieren einer solchen.

		Konkret bei \texttt{insertion\_sort}: $A$ ist der \texttt{insertion\_sort} Algorithmus. Da der Input (zu sortierende Liste) bereits das Format des Outputs hat, muss $B$ lediglich prüfen, ob die Liste bereits sortiert ist. Sollte dies der Fall sein, wird die Liste (der Input) direkt als Ouput ausgegeben (best-case). Ansonsten ruft $B$ einfach $A$, also \texttt{insertion\_sort}, auf.
	\end{solution}
