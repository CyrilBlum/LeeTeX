...
\begin{frame}{Bestimmung der Laufzeit \faStar\faStar[regular]\faStar[regular]\faStar[regular]\faStar[regular]}
    ...
    \pause % The slide stops here until the next click
    
    \textbf{Antwort:} Die Funktion wird $5\cdot 4 = 20$ mal aufgerufen.
\end{frame}
...
\begin{frame}{Bestimmung der Laufzeit \faStar\faStar[regular]\faStar[regular]\faStar[regular]\faStar[regular]}
	...
	\textbf{Antwort:} Die Funktion wird $nm$ mal aufgerufen.
\end{frame}
...
\begin{frame}{Bestimmung der Laufzeit \faStar\faStar\faStar[regular]\faStar[regular]\faStar[regular]}
	...
	Wie oft wird die Zeile 3 (\lstinline{if}-Konditional) ausgeführt? \emoji{thinking-face}
\end{frame}

\begin{frame}
	\textbf{Antwort:} Die genaue Anzahl hängt von der Probleminstanz --- in diesem Fall von der gegebenen Liste \lstinline{L} und dem gesuchten Element \lstinline{x} --- ab. Wir unterscheiden zwei extreme Fälle:
	\begin{description}[<+->]
	...
	\pause
	Natürlich können auch alle Fälle zwischen diesen beiden Extremfällen (1 Ausführungen und \lstinline{len(L)} Ausführungen) auftreten.
\end{frame}
...
\begin{frame}
	\textbf{Antwort:} Wir beginnen mit \lstinline{i = 1} und \lstinline{i} wird im jedem Durchlauf der \lstinline{while}-Schleife verdoppelt. Wie häufig können wir den Wert von \lstinline{i} verdopplen, bis die \lstinline{while}-Bedingung \lstinline{i <= n} nicht mehr erfüllt ist? Ungefähr also $\log_2(n)$ mal.

	\vspace{1cm}
	Genaue Analyse: Die Zeile \lstinline{i *= 2} wird $\ceil{\log_2(n)} + 1$ mal ausgeführt, wobei für jede reelle Zahl $x$ mit $m := \ceil{x}$ die grösste ganze Zahl $m \leq x$ bezeichnet wird (Abrundeklammern).
\end{frame}
