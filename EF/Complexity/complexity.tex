...
\title[Programmieren]{Komplexitätstheorie}

\subtitle{Komplexitätstheorie}
...
\begin{frame}{\texttt{insertion\_sort} implementieren}
	\begin{myexercise}[\texttt{insertion\_sort}]\label{myexercise:insertion_sort}
		...
		\lstinputlisting[language=Python,caption=\texttt{insertion\_sort\_template.py},label=listing:insertion_sort]{EF/Complexity/Code/insertion_sort_template.py}
	\end{myexercise}
\end{frame}

\begin{frame}
	\begin{myanswer}
		\lstinputlisting[language=Python,caption=\texttt{insertion\_sort.py}]{EF/Complexity/Code/insertion_sort.py}
	\end{myanswer}
\end{frame}
...
\begin{frame}{Analyse von \texttt{insertion\_sort} (Problemgrösse $n$)}
	\begin{itemize}[<+->]
		...
		\item Für $j = 1, 2, \ldots, n - 1$ sei $t_j$ ($t$ steht für $times$) die Anzahl der Überprüfungen der \lstinline|while|-Bedingung.
...
\begin{frame}{worst-case für \texttt{insertion\_sort}}
	 Wir bezeichnen mit $T(n)$, die \textbf{minimale} (Achtung: wir meinen wirklich die \textbf{minimale}) Laufzeit welche unser \texttt{insertion\_sort} Algorithmus benötigt, um jede (auch die schwierigste) Probleminstanz der Grösse $n$ (hier: zu sortierende Liste der Länge $n$) zu lösen.
\end{frame}
...
\begin{frame}{Motivation für die asymptotische Analyse}
	\begin{itemize}[<+->]
		...
		\begin{itemize}
			...
			\item In der Praxis stellte sich heraus, dass es am sinnvollsten ist, das wesentliche Wachstumsverhalten von Algorithmenzu betrachten.
...
\begin{frame}{asymptotische Betrachtung}
	Intuitiv gesprochen, möchten wir sagen, dass in einer asymptotischen Betrachtung, die exakten Wert der Konstanten $a, b, c$ mit $a\neq 0$ in einem Ausdruck $T(n) = an^2 + bn + c$ \enquote{nicht wirklich wichtig sind}. Diese Aussage basiert auf der Beobachtung, dass sich alle Terme dieser Form (asymptotisch) lediglich multiplikative und additive Konstanten unterscheiden:
\end{frame}
...
\begin{frame}{asymptotische Betrachtung (Landau-Symbole)}
	...
	Wir führen nun einige Mengen von Funktionen ein, die uns helfen, das asymptotische Verhalten von Algorithmen zu beschreiben.
\end{frame}
...
\begin{frame}{äquivalente Definitionen}
Man kann zeigen, dass die Definitionen von $\Theta$, $O$ und $\Omega$ (in den Gleichungen \ref{eq:Theta}, \ref{eq:grossO} und \ref{eq}) äquivalent sind zu den folgenden Grenzwertdefinitionen (wir haben noch weitere Symbole definiert):
\end{frame}
...
{\scriptsize\begin{mydefinition}\label{mydefinition:schranke}
