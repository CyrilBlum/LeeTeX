...
\begin{frame}{Effizienz (Laufzeit) ist eine wichtige Eigenschaft eines Algorithmus}
	\begin{itemize}[<+->]
		\item Kollisionserkennung: Unterschied in der Framerate
...
\begin{frame}{Lernziele}
	\begin{itemize}[<+->]
		\item Sie können die Laufzeit einfach Algorithmen bestimmen.
		\item Sie können die Begriffe \emph{worst-case} und \emph{best-case} erklären.
		\item Sie besitzen eine intuitve Vorstellung für die Komplexität alltäglicher algorithmischer Szenarien.
	\end{itemize}
\end{frame}
...
\begin{myanswer}
	...
	\pause
	\begin{description}[<+->]
		\item[günstigster Fall:] Das erste Element (\lstinline{L[0]}) ist bereits das gesuchte Element \lstinline{x} $\Rightarrow$ das \lstinline{if}-Statement wird nur einmal ausgeführt.
		\item[teuerster Fall:] Das gesuchte Element \lstinline{x} ist entweder das letzte Element der Liste oder kommt in der Liste gar nicht vor $\Rightarrow$ das \lstinline{if}-Statement wird \lstinline{n = len(L)} mal ausgeführt (jedes Element von \lstinline{L} muss überprüft werden).
	\end{description}
...
\begin{frame}{\emph{worst-case} und \emph{best-case}}
	\begin{itemize}[<+->]
		\item Die \emph{worst-case} Laufzeit $T_{\text{worst}}(n)$ ist die Anzahl der Operationen, welcher ein Algorithmus $A$ für die teuerste Eingabe (Probleminstanz) der Grösse $n$ benötigt.
		\item Die \emph{best-case} Laufzeit $T_{\text{best}}(n)$ ist die Anzahl der Operationen, welcher ein Algorithmus $A$ für die günstigste Eingabe (Probleminstanz) der Grösse $n$ benötigt.
	\end{itemize}
\end{frame}
...
\begin{myremark}
	\begin{itemize}[<+->]
		\item $T_{worst}(n)$ ist die \textbf{minimale} Anzahl der Rechenoperationen mit der Algorithmus $A$ jede (noch so schwierige) Probleminstanz der Grösse $n$ löst.
	\end{itemize}
\end{myremark}
\end{frame}
...
\begin{frame}
	\begin{myanswer}
	Wir beginnen mit \lstinline{i = 1} und \lstinline{i} wird im jedem Durchlauf der \lstinline{while}-Schleife verdoppelt. Wie häufig können wir den Wert von \lstinline{i} verdoppeln, bis die \lstinline{while}-Bedingung \lstinline{i <= n} nicht mehr erfüllt ist? Ungefähr also $\log_2(n)$ mal.
	% \pause
	% \vspace{1cm}
	% {\small
...
\begin{frame}{Aufgaben Teil 1 auf Moodle}
	...
	\pause
	\begin{itemize}
		\item Lösen Sie die Übungsaufgaben \emph{Laufzeitkomplexität 1} (Bearbeitungszeit: 15 Minuten).
		\item Bearbeiten Sie die Aufgaben alleine.
	\end{itemize}
\end{frame}

\begin{frame}{Besprechung der Aufgaben}
Welche Frage hat die grösste Mühe bereitet?
\end{frame}

\begin{frame}{Bestimmung der Laufzeit \faStar\faStar\faStar[regular]\faStar[regular]\faStar[regular]}
	\begin{myexercise}
    \lstinputlisting[basicstyle=\tiny\ttfamily,language=Python,caption=\texttt{anstossen.py},label=listing:anstossen]{EF/Complexity/Code/anstossen.py}
...
{\scriptsize
    ...
    \begin{itemize}[<+->]
        \item im ersten Durchlauf der äusseren Schleife (Fall \lstinline{i = 0}) genau {\color{orange} $n-1$} mal,
        \item im zweiten Durchlauf der äusseren Schleife (Fall \lstinline{i = 1}) genau {\color{cyan} $n-2$} mal,
        \item $\vdots$,
        \item im letzten Durchlauf der äusseren Schleife (Fall \lstinline{i = n - 2}) genau {\color{magenta} $1$} mal
    \end{itemize}
    \pause
    aufgerufen.
    ...
    \[ {\color{magenta} 1} + 2 + 3 + \ldots + {\color{cyan} (n-2)} + {\color{orange} (n-1)} \stackrel{\text{Gaußsche Summenformel}}{=} \frac{n(n-1)}{2} \]
...
\begin{frame}{Aufgaben Teil 2 auf Moodle}
	...
	\pause
	\begin{itemize}
		\item Lösen Sie die Übungsaufgaben \emph{Laufzeitkomplexität 2} (Bearbeitungszeit: 15 Minuten).
...
\begin{itemize}[<+->]
	\item Wir bezeichnen mit $w_k(n)$ die totale Anzahl der Ausführungen der $k$-ten Programmzeile in Abhängigkeit von $n$.
	\item Lassen Sie uns $w_k(n)$ für jede Zeile von \texttt{bubble\_sort} bestimmen.
\end{itemize}
...
def bubble_sort(A): !\tikzmark{c1end}! 									!\tikzmark{c1start}!
    ...
    for i in range(n): !\tikzmark{c3end}! 								!\tikzmark{c3start}!
        for j in range(0, n - i - 1): !\tikzmark{c4end}! 					!\tikzmark{c4start}!
...
\begin{tikzpicture}[overlay, remember picture,
    ...
    \only<4->{\drawArrow{c3end}{c3start}{\tiny $w_3(n) = n+1$}{green!35!white};}
    \only<8->{\drawArrow{c4end}{c4start}{\tiny $w_4(n) = w_5(n) + n$}{green!35!white};}
    \only<5->{\drawArrow{c5end}{c5start}{\tiny $w_5(n)$}{green!35!white};}
    \only<6->{\drawArrow{c6end}{c6start}{\tiny $w_6(n)$}{red!55!white};}
...
\begin{itemize}[<+->]
	...
	\item Für jeden der $n$ Durchläufe der äusseren Schleife muss im Kopf der inneren Schleife am Ende noch einmal die Abbruch-Bedingung überprüft werden $\Rightarrow w_4(n) = w_5(n) + n$.
\end{itemize}
\end{frame}

\begin{frame}[fragile]{Genaue Analyse, \textcolor{Red}{worst-case}}
...
\begin{itemize}[<+->]
	...
	\item $\vdots$
	\item Wenn \lstinline{i = n-2}, läuft \lstinline{j} von \lstinline{0} bis \lstinline{0} $\Rightarrow$ $1$ Mal
	\item Wenn \lstinline{i = n-1}, ist der Bereich \lstinline{range(0, 0)} $\Rightarrow$ $0$ Mal
\end{itemize}
\pause
Wir erhalten die bekannte Summe:
{\footnotesize
\begin{align*}
	w_5(n) = (n-1) + (n-2) + \ldots + 1 + 0 \stackrel{\text{Gaußsche Summenformel}}{=} \frac{n(n-1)}{2}
\end{align*}
}
\end{frame}
...
{\footnotesize
\begin{align*}
	T_{\text{worst}}(n) &= c_1w_1(n) + c_2w_2(n) + \ldots + c_7w_7(n) = \sum_{k=1}^{7}c_kw_k(n) =\\
	&= c_1 + c_2 + c_3(n+1) + c_4(w_5(n) + n) + c_5w_5(n) + c_6w_5(n) + c_7 = \\
	&= c_1 + c_2 + c_3n + c_4\lr{\frac{n(n-1)}{2}+n} + \lr{c_5 + c_6}\lr{\frac{n(n-1)}{2}} + c_7n
\end{align*}
}
\end{frame}

\begin{frame}[fragile]{Genaue Analyse}
	Wir vereinfachen die Terme und Gruppieren nach den Potenzen von $n$, um den folgenden Ausdruck zu erhalten:
	\pause
{\footnotesize
\begin{align*}
T_{\text{worst}}(n) &= \underbrace{\lr{\frac{c_4 + c_5 + c_6}{2}}}_{=:\textcolor{Red}{a}} n^2 + \underbrace{\lr{c_3 + c_7 + \frac{c_4 - c_5 - c_6}{2}}}_{=:\textcolor{RoyalBlue}{b}}n + \underbrace{(c_1 + c_2)}_{=:\textcolor{ForestGreen}{c}} = \\
...
\begin{tikzpicture}[overlay, remember picture,
    ...
    \only<7->{\drawArrow{c6end}{c6start}{\tiny $w_6(n) = w_7(n) + (n-1)$}{green!35!white};}
    \only<8->{\drawArrow{c7end}{c7start}{\tiny $w_7(n) = w_7(n)$}{green!35!white};}
    \only<9->{\drawArrow{c8end}{c8start}{\tiny $w_8(n) = w_7(n)$}{green!35!white};}
    \only<10->{\drawArrow{c9end}{c9start}{\tiny $w_9(n) = n-1$}{green!35!white};}
    \only<11->{\drawArrow{c10end}{c10start}{\tiny $w_{10}(n) = 1$}{green!35!white};}
\end{tikzpicture}
\end{myanswer}
\end{frame}
...
&+ \tilde{c}_6\lr{\frac{n(n-1)}{2}+n} + \tilde{c}_7\lr{\frac{n(n-1)}{2}} +\\
...
&+ \underbrace{\lr{\tilde{c}_1 + \tilde{c}_2 - \tilde{c}_4 - \tilde{c}_5 - \tilde{c}_9 + \tilde{c}_{10}}}_{=:\textcolor{ForestGreen}{\tilde{c}}}=\\
...
% \begin{frame}{Bestimmung der Laufzeit \faStar\faStar\faStar[regular]\faStar[regular]\faStar[regular]}
% 	\begin{myexercise}
%     \lstinputlisting[basicstyle=\scriptsize\ttfamily,language=Python,caption=\texttt{bubble\_sort.py},label=listing:bubble_sort]{EF/Complexity/Code/bubble_sort.py}
% 	\end{myexercise}
% \end{frame}
...
% 	\begin{tikzpicture}[overlay, remember picture,
% 			every node/.style={draw=green!50,fill=green!35!white,text=black,rounded corners}]
% 		\only<2->{\drawArrow{c1end}{c1start}{\tiny $\text{K:}\; c_1, \text{W:}\; 1$}{green!35!white};}
% 		\only<3->{\drawArrow{c2end}{c2start}{\tiny $\text{K:}\; c_2, \text{W:}\; n-1$}{green!35!white};}
% 		\only<4->{\drawArrow{c3end}{c3start}{\tiny $\text{K:}\; c_3, \text{W:}\; n-1$}{green!35!white};}
% 		\only<5->{\drawArrow{c4end}{c4start}{\tiny $\text{K:}\; c_4, \text{W:}\; n-1$}{green!35!white};}
% 		\only<6->{\drawArrow{c5end}{c5start}{\tiny $\text{K:}\; c_5, \text{W:}\; t_j$}{green!35!white};}
% 		\only<7->{\drawArrow{c6end}{c6start}{\tiny $\text{K:}\; c_6, \text{W:}\; t_j - 1$}{green!35!white};}
% 		\only<8->{\drawArrow{c7end}{c7start}{\tiny $\text{K:}\; c_7, \text{W:}\; t_j - 1$}{green!35!white};}
% 		\only<9->{\drawArrow{c8end}{c8start}{\tiny $\text{K:}\; c_8, \text{W:}\; n-1$}{green!35!white};}
% 		\only<10->{\drawArrow{c9end}{c9start}{\tiny $\text{K:}\; c_9, \text{W:}\; 1$}{green!35!white};}
% 	\end{tikzpicture}
% \end{frame}
...
% 	\begin{tikzpicture}[overlay, remember picture,
% 			every node/.style={draw=green!50,fill=green!35!white,text=black,rounded corners}]
% 		{\drawArrow{c1end}{c1start}{\tiny $\text{K:}\; c_1, \text{W:}\; 1$}{green!35!white};}
% 		{\drawArrow{c2end}{c2start}{\tiny $\text{K:}\; c_2, \text{W:}\; n-1$}{green!35!white};}
% 		{\drawArrow{c3end}{c3start}{\tiny $\text{K:}\; c_3, \text{W:}\; n-1$}{green!35!white};}
% 		{\drawArrow{c4end}{c4start}{\tiny $\text{K:}\; c_4, \text{W:}\; n-1$}{green!35!white};}
% 		{\drawArrow{c5end}{c5start}{\tiny $\text{K:}\; c_5, \text{W:}\; t_j$}{green!35!white};}
% 		{\drawArrow{c6end}{c6start}{\tiny $\text{K:}\; c_6, \text{W:}\; t_j - 1$}{green!35!white};}
% 		{\drawArrow{c7end}{c7start}{\tiny $\text{K:}\; c_7, \text{W:}\; t_j - 1$}{green!35!white};}
% 		{\drawArrow{c8end}{c8start}{\tiny $\text{K:}\; c_8, \text{W:}\; n-1$}{green!35!white};}
% 		{\drawArrow{c9end}{c9start}{\tiny $\text{K:}\; c_9, \text{W:}\; 1$}{green!35!white};}
% 	\end{tikzpicture}
% \end{frame}
...
% 	\begin{tikzpicture}[overlay, remember picture,
% 			every node/.style={draw=green!50,fill=green!35!white,text=black,rounded corners}]
% 		{\drawArrow{c1end}{c1start}{\tiny $\text{K:}\; \tilde{c}_1, \text{W:}\; 1$}{green!35!white};}
% 		{\drawArrow{c2end}{c2start}{\tiny $\text{K:}\; \tilde{c}_2, \text{W:}\; 1$}{green!35!white};}
% 		{\drawArrow{c3end}{c3start}{\tiny $\text{K:}\; \tilde{c}_3, \text{W:}\; n$}{green!35!white};}
% 		{\drawArrow{c4end}{c4start}{\tiny $\text{K:}\; \tilde{c}_4, \text{W:}\; n$}{green!35!white};}
% 		{\drawArrow{c5end}{c5start}{\tiny $\text{K:}\; \tilde{c}_5, \text{W:}\; i$}{green!35!white};}
% 		{\drawArrow{c6end}{c6start}{\tiny $\text{K:}\; \tilde{c}_6, \text{W:}\; i$}{green!35!white};}
% 		{\drawArrow{c7end}{c7start}{\tiny $\text{K:}\; \tilde{c}_7, \text{W:}\; i$}{green!35!white};}
% 		{\drawArrow{c8end}{c8start}{\tiny $\text{K:}\; \tilde{c}_8, \text{W:}\; i$}{green!35!white};}
% 		{\drawArrow{c9end}{c9start}{\tiny $\text{K:}\; \tilde{c}_9, \text{W:}\; n$}{green!35!white};}
% 		{\drawArrow{c10end}{c10start}{\tiny $\text{K:}\; \tilde{c}_{10}, \text{W:}\; 0$}{green!35!white};}
% 		{\drawArrow{c11end}{c11start}{\tiny $\text{K:}\; \tilde{c}_{11}, \text{W:}\; 1$}{green!35!white};}
% 	\end{tikzpicture}
% \end{frame}
