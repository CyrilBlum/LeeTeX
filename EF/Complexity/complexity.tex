...
\begin{frame}{Bestimmung der Laufzeit \faStar\faStar\faStar[regular]\faStar[regular]\faStar[regular]}
	\begin{myexercise}
    \lstinputlisting[basicstyle=\scriptsize\ttfamily,language=Python,caption=\texttt{anstossen.py},label=listing:anstossen]{EF/Complexity/Code/anstossen.py}
...
\begin{frame}[fragile]{Analyse von \texttt{insertion\_sort}}
	\begin{myexercise}
		{\small
		Führen Sie eine vollständige Laufzeitanalyse von \texttt{insertion\_sort} durch:
		}
...
\begin{frame}[fragile]{Analyse von \texttt{insertion\_sort} (Problemgrösse $n$)}
\begin{myanswer}
\begin{lstlisting}[escapechar=!,basicstyle=\footnotesize\ttfamily,numbers=left]
...
\begin{tikzpicture}[overlay, remember picture,
    ...
    \only<7->{\drawArrow{c6end}{c6start}{\tiny $w_6(n) = ?$}{green!35!white};}
    \only<8->{\drawArrow{c7end}{c7start}{\tiny $w_7(n) = ?$}{green!35!white};}
    \only<9->{\drawArrow{c8end}{c8start}{\tiny $w_8(n) = ?$}{green!35!white};}
    \only<10->{\drawArrow{c9end}{c9start}{\tiny $w_9(n) = ?$}{green!35!white};}
