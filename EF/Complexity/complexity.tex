...
\begin{frame}{Bestimmung der Laufzeit \faStar\faStar\faStar[regular]\faStar[regular]\faStar[regular]}
	...
	Wie oft wird die Zeile 4 (\lstinline{while}-Konditional) \textbf{ungefähr} ausgeführt?
\end{frame}

\begin{frame}
	\textbf{Antwort:} Wir beginnen mit \lstinline{i = 1} und \lstinline{i} wird im jedem Durchlauf der \lstinline{while}-Schleife verdoppelt. Wie häufig können wir den Wert von \lstinline{i} verdopplen, bis die \lstinline{while}-Bedingung \lstinline{i <= n} nicht mehr erfüllt ist? Ungefähr also $\log_2(n) + 1$ mal.
\end{frame}
