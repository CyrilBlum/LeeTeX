...
\begin{frame}[fragile]{Typisches Problem: \enquote{alle mit allen}}
	\begin{itemize}[<+->]
		...
		\item Jede Person stösst mit jeder anderen Person \textbf{genau einmal} an.
		\item Wie viele \enquote{Kling}-Geräusche (Begegnungen) gibt es insgesamt?
	\end{itemize}
	\pause
	\begin{figure}
		\centering
	...
	\end{figure}
	% \pause
	% Insgesamt haben wir also $(n-1) + (n-2) + (n-3) + \ldots + 1$ Paarungen.
\end{frame}
...
\begin{frame}{Beispiel einer exakten Analyse (Fortsetzung)}
    ...
    {\footnotesize
    \begin{align*}
        T(n) &= c_1w_1(n) + c_2w_2(n) + c_3w_3(n) + c_4w_4(n) + c_5w_5(n) \\
        &= c_1(1) + c_2(1) + c_3(n+1) + c_4(n) + c_5(1) \\
        &= (c_3 + c_4)n + (c_1 + c_2 + c_3 + c_5) \\
        &= \textcolor{RoyalBlue}{a}n + \textcolor{ForestGreen}{b}
    \end{align*}
    }
    \pause
    \begin{myremark}
...
\begin{itemize}[<+->]
	...
	\item $w_4(n)$ ist höher als $w_5(n)$, da die \lstinline{for}-Bedingung pro äusserem Durchlauf einmal öfter geprüft wird (Abbruchprüfung).
\end{itemize}
\end{frame}
