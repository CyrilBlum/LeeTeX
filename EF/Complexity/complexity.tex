...
\begin{frame}
	\begin{myanswer}
	Wir beginnen mit \lstinline{i = 1} und \lstinline{i} wird im jedem Durchlauf der \lstinline{while}-Schleife verdoppelt. Wie häufig können wir den Wert von \lstinline{i} verdoppeln, bis die \lstinline{while}-Bedingung \lstinline{i <= n} nicht mehr erfüllt ist? Ungefähr $\log_2(n)$ mal.
...
\begin{frame}{Aufgaben auf Moodle}
	...
	\pause
	\begin{itemize}
		\item Lösen Sie den Test \emph{Laufzeitkomplexität 1} (Bearbeitungszeit: 10 Minuten).
		\item Bearbeiten Sie diese Aufgabe alleine für sich.
	\end{itemize}
\end{frame}
...
\begin{frame}{Bestimmung der Laufzeit \faStar\faStar\faStar[regular]\faStar[regular]\faStar[regular]}
	\begin{myexercise}
    \lstinputlisting[basicstyle=\footnotesize\ttfamily,language=Python,caption=\texttt{anstossen.py},label=listing:anstossen]{EF/Complexity/Code/anstossen.py}
...
{\scriptsize
	...
	\begin{itemize}[<+->]
		\item im ersten Durchlauf der äusseren Schleife (Fall \lstinline{i = 0}) genau $n-1$ mal,
		\item im zweiten Durchlauf der äusseren Schleife (Fall \lstinline{i = 1}) genau $n-2$ mal,
		\item $\ldots$,
		\item im letzten Durchlauf der äusseren Schleife (Fall \lstinline{i = n - 2}) genau $1$ mal
	\end{itemize}
	aufgerufen.

	\pause
	Total haben wir also
	\[1 + 2 + 3 + \ldots + (n-2) + (n-1) \stackrel{\text{Gaußsche Summenformel}}{=} \frac{n(n-1)}{2}\]
...
\begin{frame}{Bestimmung der Laufzeit \faStar\faStar\faStar[regular]\faStar[regular]\faStar[regular]}
	\begin{myexercise}
    \lstinputlisting[basicstyle=\footnotesize\ttfamily,language=Python,caption=\texttt{bubble\_sort\_simple.py},label=listing:bubble_sort_simple]{EF/Complexity/Code/bubble_sort_simple.py}
	\end{myexercise}
\end{frame}
