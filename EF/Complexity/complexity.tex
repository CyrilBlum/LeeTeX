...
\begin{frame}{asymptotische Betrachtung}
	...
	\end{align*}
	Die asymptotischen Laufzeiten von \texttt{insertion\_sort} und \texttt{bubble\_sort} unterscheiden sich also lediglich um einen konstanten Faktor. Die analoge Betrachtung kann natürlich auch für \texttt{bubble\_sort}) durchgeführt werden.
\end{frame}
...
\begin{tikzpicture}
\begin{axis}[
    width=7.5cm, % Eine etwas größere Breite, da es nun ein Einzelplot ist
    height=7.5cm,
    axis lines=left,
    ticks=none,
    xmin=0, xmax=16,
...
\begin{frame}
Wir schreiben $f(n)\in\Theta(g(n))$, falls positive Konstanten $n_0, c_1$ und $c_2$ existieren, sodass für alle $n\geq n_0$ der Wert $f(n)$ zwischen $c_1g(n)$ und $c_2g(n)$ liegt.
\end{frame}
...
\begin{mydefinition}\label{mydefinition:schranke}
TODO
\end{mydefinition}
\end{frame}
