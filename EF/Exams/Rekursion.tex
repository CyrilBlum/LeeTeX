...
\begin{parts}
    ...
    \end{solution}
    \part[1] Schreibe eine Python-Funktion \pythoninline{def M(n)}, welche die rekursive Definition von McCarthy's 91 Funktion in Ausdruck \ref{eq:M} implementiert.
    \begin{solution}
...
\begin{solution}
  Die Funktion berechnet die Umkehrung des Strings \pythoninline{s} (Reversal von \pythoninline{s}). Die Ausgabe ist also \pythoninline{s} \enquote{rückwärts gelesen}.
\end{solution}
\end{parts}
\droptotalpoints
...
\end{solution}
\part[1]
Schreibe eine rekursive Python-Funktion \pythoninline{def f_rek(n)}, welche die oben gegebene rekursive Funktion $f$ implementiert.
...
\part[1]
Schreibe eine Python-Funktion \pythoninline{def f_it(n)}, welche den Wert $f(n)$ für eine gegebene ganze Zahl $n$ mit einem \pythoninline{for}-Schleife berechnet (iterativ), ohne sich rekursiv selbst aufzurufen.
Man darf davon ausgehen, dass der Input garantiert eine ganze Zahl ist.
\begin{solution}
\begin{lstlisting}[language=Python,numbers=none]
def f_it(n):
