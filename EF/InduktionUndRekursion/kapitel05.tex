...
\begin{myexercise}\label{myexercise:0502}
	Analysieren Sie den Algorithmus, welchen Sie in \cref{myexercise:0501} geschrieben haben. Welche Laufzeit hat dieser Algorithmus in Abhängigkeit von \pythoninline{n = len(L) + len(R)}?
\end{myexercise}
...
\begin{description}
	...
	\item[Combine:] Wie wir in \cref{myexercise:0502} bereits festgestellt haben, benötigt \textit{merge sort} die lineare Zeit $\Theta(n)$ für die Vereinigung zweier Listen mit summierter Länge \pythoninline{n = len(L) + len(R)}.
\end{description}
