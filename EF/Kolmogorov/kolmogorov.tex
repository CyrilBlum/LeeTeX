...
In der Theorie der algorithmischen Informationstheorie, ist die Kolmogorov-Komplexität eines Objekts, wie zum Beispiel einem gegebenen Text, die Länge eines kürzesten Computerprogramms (in einer festgelegten Programmiersprache), welches dieses Obejekt als Ausgabe (Output) produziert.
\par
...
\end{figure}
Wird die Illustration in \cref{fig:mandelbrot_16k} Pixel für Pixel abgespeichert, benötigt die PNG-Kompression dieser Illustration rund $100$ Millionen Bits an Speicherplatz, um die Illustration zu beschreiben. Nun kann aber ein recht kurzes Computerprogramm (siehe \cref{listing:mandelbrot}) diese $100$ Millionen Bits reproduzieren, indem es die Definition der Mandelbrotmenge verwendet. Dieses (kurze) Computerprogramm ist also eine deutlich kürzere Beschreibung der Illustration. Für die Reprodukton dieser $100$ Bits benötigt unser Computerprogramm zahlreiche (sehr viele) Rechenoperationen und viel Rechenzeit. Dies ändert aber nichts daran, dass das Programm eine kurze Beschreibung dieser $100$ Bits der Mandelbrotmenge ist.
...
Wir werden nun allmählich versuchen eine robuste Methode zur Messung des Informationsgehalts eines Wortes zu entwickeln. Wir werden immer wieder auf \cref{ch:Details} verweisen.
...
\section{Idee der Kolmogorov-Komplexität und Sprachinvarianz}
...
Zunächst möchten wir das kleine und etwas mühsames Detail der sogenannten \textit{Selbstbegrenzung} adressieren und in diesem Abschnitt aus dem Weg räumen. Betrachten wir dazu nochmals das folgende (standard) \texttt{C++}-Programm aus \cref{myexample:generateA}:
...
\end{mydefinition}
Für ein binäres Wort $w$ betrachten wir alle (unendlich vielen) Maschinencodes von \texttt{C'++} Programmen, die $w$ generieren. Die Länge eine kürzesten\footnote{Im Allgemeinen kann es mehrere verschiedene kürzeste Maschinencodes geben, die $w$ generieren.} solcher Maschinencodes ist dann $K(w)$.
\begin{myremark}
	Ist $K(w)$ nun ein geeignetes Mass für den Informationsgehalt des binären Wortes $w$? Ja, da jede Komprimierungsmethode als Programm formuliert werden kann, bezieht die Kolmogorov-Komplexität jede denkbare Komprimierungsmethode ein. Es sei $x$ ein binäres Wort und $f$ eine Komprimierungsmethode (Funktion), die zu $x$ eine komprimierte Darstellung $x' := f(x)$ generiert. Es sei $f^{-1}$ die Umkehrung der Komprimierung $f$, also $f^{-1}(f(x)) = f^{-1}(x') = x$. Dann können wir ein Programm schreiben, welches lediglich (das kurze Wort) $x'$ als Parameter beinhaltet und $x$ wieder aus $x'$ mithilfe von $f^{-1}$ erzeugt und schliesslich ausgibt. Dazu muss $f^{-1}$ natürlich auch im Programm kodiert sein. Doch die binäre Länge der Beschreibung dieser Umkehrfunktion ist unabhängig von $x$ und $x'$ und kann somit als konstant angesehen werden. Das so aufgebaute Programm würde dann $x$ generieren, ohne die Darstellung von $x$ speichern zu müssen.
\end{myremark}
...
\begin{mytheorem}[Sprachinvarianz]\label{mytheorem:Sprachinvarianz}
	...
	Es existiert eine Konstante $c_{A, B}$, welche nur von $A$ und $B$ abhängt, so dass
	\begin{align*}
	...
	\begin{myproof}
		...
		\begin{itemize}
			...
			\item Der Unterschied $\abs{K_A(w) - K_B(w)}$ ist also nicht grösser als $c_{A, B} := \max\lrc{c_{A\to B}, c_{B\to A}}$.
		\end{itemize}
	\end{myproof}
\end{mytheorem}
...
\end{lstlisting}
Im Programm \texttt{Cn} ist einzig die binäre Kodierung \colorbox{YellowGreen}{\texttt{n}} der Zahl $n$ abhängig von $n$ beziehungsweise $w_n$. Der Rest des Programms ist unabhängig von der konkreten Wahl von $n$ (für jede Wahl von $n$ gleich). Gemäss \cref{mytheorem:AnzahlZiffern} benötigen wir zur binären Darstellung von $n$ genau $\ceil{\log_2(n+1)}$ viele Bits. Der Rest des Programms ist unabhängig von $w_n$. Damit existiert Konstanten $c_0$ und $c_1$, so dass
\begin{align*}
	K(w_n) \leq c_0 + \ceil{\log_2(n+1)} \leq \log_2(n) + c_1 = \log_2(\abs{w_n}) + c_1
\end{align*}
...
void Dn() {
    ...
    \end{lstlisting}
Analog zu \cref{myexample:wn}, ist auch hier einzig die binäre Kodierung \colorbox{YellowGreen}{\texttt{n}} der Zahl $n$ abhängig von $n$ beziehungsweise $v_n$. Der Rest des Programms ist wieder unabhängig von der konkreten Wahl von $n$ (für jede Wahl von $n$ gleich). Zur binären Darstellung von $n$ benötigen wir genau $\ceil{\log_2(n+1)}$ viele Bits. Der Rest des Programms ist unabhängig von $v_n$. Damit existiert Konstanten $c_2$ und $c_3$, so dass
\begin{align*}
	K(v_n) \leq c_3 + \ceil{\log_2(n+1)} \leq \log_2(n) + c_4 = \log_2\lr{\sqrt{\abs{v_n}}} + c_4
\end{align*}
für beliebiges natürliches $n>0$.
...
\begin{itemize}
	...
	\item oder eine Berechnung wie \texttt{M * M} vielleicht sehr viel Rechenzeit in Anspruch nehmen könnte
\end{itemize}
...
void Fn() {
	...
	\begin{myanswer}
		...
		\begin{align*}
			K(v_n)\leq c + 2\ceil{\log_2\lr{n^2 + 1}} = c' + 2\log_2{n^2} = c' + 4\log_2\lr{\sqrt{\abs{v_n}}}
		\end{align*}
...
Wir haben die Kolmogorov-Komplexität nur für binäre Wörter definiert. Wir können die Kolmogorov-Komplexität aber auch ganz einfach für natürliche Zahlen definieren, indem wir die natürliche Zahl einfach in Basis 2 (binär) darstllen, um ein binäres Wort zu erhalten.
...
\begin{mytheorem}[Existenz nichtkomprimierbarer Wörter]\label{mytheorem:nichtkomprimierbar}
	...
	\begin{myproof}
		...
		\end{align*}
		gibt. Es gibt genau $2^1$ binäre Wörter der Länge $1$, $2^2$ binäre Wörter der Länge $2$ und allgemein $2^k$ binäre Wörter der Länge $k$. Insgesamt enthält die Menge $S$ also genau
		\begin{align*}
			\sum_{k = 1}^{n-1} 2^k \stackrel{\cref{mytheorem:verallgemeinertegeometrischeSumme}}{=} 2^n - 2
		\end{align*}
		Elemente. Doch es gibt genau $2^n$ binäre Wörter der Länge $n$. Dies sind aber mehr als es nichtleere verschiedene Maschinencodes der Länge $<n$ gibt. Damit muss ein binäres Wort $x$ der Länge $n$ existieren, für welches kein Maschinencode mit Länge $<n$ existiert und somit $K(w) \geq n = \abs{w}$.
	\end{myproof}
\end{mytheorem}

Ein nichtkomprimierbares Wort erlaubt also keine kürzere Beschreibung, als das Wort vollständig zu beschreiben. Es gibt also keinen anderen Plan zur seiner Generierung als einzig seine vollständige Beschreibung. Damit ist es plausible, dass \cref{mydefinition:zufall} unsere bislang beste bekannte Formalisierung des informellen Begriffes \enquote{zufällig} ist.
...
void seeking_alpha_n() {
    ...
    std::cout << x << std::endl;
		...
		\end{align*}
		können höchstens für endlich viele natürliche Zahlen $n>0$ gelten. Dies ist ein Widerspruch zur Annahme, dass ein Algorithmus $Q$ zur Berechung der Kolmogorov-Komplexität von allen binären Wörtern existiert.
	\end{myproof}
\end{mytheorem}
