...
\begin{myexercise}
	...
	\begin{myanswer}
		Die binären Darstellungen aller natürlichen Zahlen $n>0$ beginnen alle mit einer $1$. Das erste Bit in der binären Darstellung von $n$ ist also bereits bekannt. Es ist also sinnvoll die Zahl auch schon als zufällig anzuschauen, falls alle Bits ausser dem ersten genannt werden müssen.
	\end{myanswer}
\end{myexercise}
...
\begin{mytheorem}[$K(w)$ ist nicht für jedes binäre Wort berechenbar]\label{mytheorem:Knotcomputable}
	Das Problem, für jedes binäre Wort $w$ die Kolmogorov-Komplexität $K(w)$ von $w$ zu berechnen, ist algorithmisch unlösbar.
\end{mytheorem}
