	...
	\begin{myproof}
		...
		\begin{itemize}
			...
			\item Es sei $B_w$ ein Programm in Sprache $B$, welches $w$ generiert. Dann können wir $B_w$ dem Programm $T_{B\to A}$ als Parameter übergeben. Dann ist $T_{B\to A}\lr{B_w}$ ein Programm, in Sprache $A$, welches $w$ generiert.
			      Wir bezeichnen die binäre Länge des Programms $T_{B\to A}$ mit $c_{B\to A}$. Somit gilt $K_A(w) \leq K_B + c_{B\to A}$.
			\item Analog (von $A$ zu $B$) finden wir $K_B(w) \leq K_A + c_{A\to B}$.
...
\begin{mytheorem}[$K(w)$ ist auf keinen Fall wesentlich länger als $\abs{w}$]\label{mytheorem:upperbound}
	...
	\begin{myproof}
        ...
        !\colorbox{Yellow}{int main() \{}!
            !\colorbox{Yellow}{\hspace*{1.5em}A();}!
            !\colorbox{Yellow}{\hspace*{1.5em}return 0;}!
        !\colorbox{Yellow}{\texttt{\}}}!
        \end{lstlisting}
...
void Cn() {
    std::string wort(***!\colorbox{YellowGreen}{bin(n)}!+++, '1');
    std::cout << wort << std::endl;
}
...
int main() {
    ...
    \end{lstlisting}
Analog zu \cref{myexample:wn}, ist auch hier einzig die binäre Kodierung \colorbox{YellowGreen}{\texttt{bin(n)}} der Zahl $n$ abhängig von $n$ beziehungsweise $v_n$. Der Rest des Programms ist wieder unabhängig von der konkreten Wahl von $n$ (für jede Wahl von $n$ gleich). Zur binären Darstellung von $n$ benötigen wir genau $\ceil{\log_2(n+1)}$ viele Bits. Der Rest des Programms ist unabhängig von $v_n$. Damit existiert Konstanten $c_2$ und $c_3$, so dass
\begin{align*}
	K(v_n) \leq c_3 + \ceil{\log_2(n+1)} = \log_2(n) + c_4 = \log_2\lr{\sqrt{\abs{v_n}}} + c_4
\end{align*}
für beliebiges natürliches $n>0$.
...
int main() {
	...
	\begin{myanswer}
		Da \texttt{Fn} zweimal die binäre Kodierung von $n$ enthält, kann \texttt{Fn} sicherlich keine bessere obere Schranke liefern als $K(v_n)\leq c + \textcolor{Magenta}{2}\log_2\lr{\sqrt{\abs{v_n}}}$ für eine geeignete Konstante $c$.
	\end{myanswer}
\end{myexercise}

\section{Nichtkomprimierbare Wörter und Zufall}
Wir haben die Kolmogorov-Komplexität nur für binäre Wörter definiert. Wir können die Kolmogorov-Komplexität aber auch ganz einfach für natürliche Zahlen definieren, indem wir folgendes festlegen:
\begin{mydefinition}[Kolmogorov-Komplexität einer natürlichen Zahl]
Die Kolmogorov-Komplexität $K(n)$ einer natürlichen Zahl $n$ ist $K(n) := K(bin(n))$. Beachte, dass $bin(n)$ lediglich ein binäres Wort ist.
\end{mydefinition}
