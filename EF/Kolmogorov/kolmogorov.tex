...
\end{itemize}
tut hier nichts zur Sache! Uns interessiert einzig und alleine die binäre Länge der \textbf{Beschreibung} eines Wortes.
\end{myexample}
...
void Fn() {
	...
	\begin{myanswer}
		...
		\begin{align*}
			K(v_n)\leq c + 2\ceil{\log_2\lr{n^2 + 1}} \leq c' + 2\log_2{n^2} = c' + 4\log_2\lr{\sqrt{\abs{v_n}}}
		\end{align*}
		für geeignete Konstante $c, c'$.
	\end{myanswer}
\end{myexercise}
...
\begin{mytheorem}[$K(w)$ ist nicht für jedes binäre Wort berechenbar]\label{mytheorem:Knotcomputable}
	...
	\begin{myproof}
		...
		\par
		Angenommen es würde ein Algorithmus $Q$ existieren, der zu jedem binären Wort $w$ die Zahl $K(w)$ berechnet. Wir verwenden $Q$ um eine kurze Beschreibung von $\alpha$ anzugeben. Für jede natürliche Zahl $n>0$ sucht und generiert das folgende \texttt{C'++} Programm das Wort $\alpha$:
		\begin{lstlisting}[language=C++]
#include <iostream>
#include <string>
