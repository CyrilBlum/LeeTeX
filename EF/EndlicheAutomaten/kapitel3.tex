...
Wir geben einen endlichen Automaten $M = (Q, \Sigma, \delta, q0, F)$ mit $L(M) = L$ an.
\begin{itemize}
    \item $Q = \lr{q_0', q_t, q_0, q_1, \ldots, q_{10}, p_0, p_1, \ldots, p_{10}}$ (dies sind 24 Zustände)
    \item $\Sigma = \Sigma_{10} = \lrc{0, 1, \ldots, 9}$
    \item Der Anfangszustand $q0$ ist gegeben durch $q_0'$.
    \item $F = \lrc{q_0, p_0}$
    \item Die Übergangsfunktion $\delta$ ist vollständig beschrieben durch:
        \begin{align*}
            \delta(q_0', 0) &= q_t, \\
            \delta(q_t, j) &= q_t, \quad j\in\lrc{0, 1, \ldots, 9}, \\
            \delta(q_0', j) &= q_j, \quad j\in\lrc{1, 2, \ldots, 9}, \\
            \delta(q_i, j) &= p_{(i-j)\text{ mod } 11},\quad i,j\in\lrc{0, 1, \ldots, 9}, \\
            \delta(p_i, j) &= q_{(i+j)\text{ mod } 11},\quad i,j\in\lrc{0, 1, \ldots, 9}.
        \end{align*}
\end{itemize}
\end{myanswer}
