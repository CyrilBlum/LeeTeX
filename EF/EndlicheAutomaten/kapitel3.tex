...
betrachtet. In einer ersten Phase möchten wir eine gute intuitive Vorstellung der Arbeitsweise von endlichen Automaten gewinnen. Dazu werden wir in Unterabschnitt \cref{subsec:graphenDarstellung} endliche Automaten mit (leicht modifizierten) gerichteten Graphen identifizieren. In Unterabschnitt \cref{subsec:Programmdarstellung} werden wir sehen, dass endliche Automaten äquivalent zu einer Darstellung durch spezielle Programme sind. Diese speziellen Programme lösen gewisses Entscheidungsproblem, ohne bei ihrer Arbeit Variablen zu benutzen.
...
$A$ befindet sich zu Beginn in seinem Anfangszustand $q_0$. Nun beginnt er damit das Eingabewort von links nach recht zu lesen (Buchstabe um Buchstabe). Halten Sie sich im Folgenden \cref{fig:EA1} vor Augen.
\begin{enumerate}
...
\clearpage
\noindent
Betrachten wir nochmals den endlichen Automaten in \cref{fig:EA2}. Diesen Automaten wollen wir nun \textbf{formal} beschreiben. Dazu genügt es, die $5$ Komponenten des $5$-Tupels $(Q,\Sigma, \delta, q_0, F)$ gemäss Definition \cref{mydefinition:endlicherAutomat}, anzugeben. Der zur Graphendarstellung in \cref{fig:EA2} äquivalente endliche Automat  ist $M = (Q,\Sigma, \delta, q_0, F)$, wobei:
\begin{align*}
