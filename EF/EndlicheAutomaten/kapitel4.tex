...
betrachtet. In einer ersten Phase möchten wir eine gute intuitive Vorstellung der Arbeitsweise von endlichen Automaten gewinnen. Dazu werden wir in Unterabschnitt \cref{subsec:graphenDarstellung} endliche Automaten mit (leicht modifizierten) gerichteten Graphen identifizieren. In Unterabschnitt \cref{subsec:Programmdarstellung} werden wir sehen, dass endliche Automaten äquivalent zu einer Darstellung durch spezielle Programme sind. Diese speziellen Programme lösen gewisse Entscheidungsproblem ohne bei ihrer Arbeit Variablen zu benutzen.\\
Mithilfe dieser Intuition wird uns die Formale Definition endlicher Automaten in Unterabschnitt \cref{subsec:formaleDefinitionEA} elegant und natürlich erscheinen.\\
...
Um konkret über endliche Automaten sprechen zu können, betrachten wir den einfachen endlichen Automaten $A$ in Abbildung \cref{fig:EA1}. Wir werden den Aufbau und die Funktionsweise endlicher Automaten anhand dieses konkreten endlichen Automaten $A$ erklären.
\begin{figure}[H]
	\centering
...
$A$ befindet sich zu Beginn in seinem Anfangszustand $q_0$. Nun beginnt er damit das Eingabewort von links nach recht zu lesen (Buchstabe um Buchstabe). Halten Sie sich im Folgenden Abbildung \cref{fig:EA1} vor Augen.
\begin{enumerate}
...
\begin{example}{example:Aabb}
	...
	\noindent
	\textbf{Erklärung}:\\
	Sei $w\in\lrc{a,b,c}^*$ ein Eingabewort für den endlichen Automaten $A$ in Abbildung \cref{fig:Aabb}. $A$ beginnt seine Arbeit im Zustand $q_0$ und liest den ersten Buchstaben von $w$.
	\begin{itemize}
		...
		\item Die Wörter, welche den endlichen Automaten $A$ in den Zustand $q_{ab}$ führen, beginnen mit $a$, enden mit $b$, haben aber an ihrer zweitletzten Stelle nicht das Symbol $b$ (siehe dazu Übung \cref{myexercise:4.3}).
...
Bei der Arbeit auf dem Wort $w = abbc$ zum Beispiel, durchläuft der endliche Automat in Abbildung \cref{fig:Aabb} zwar den akzeptierenden Zustand $q_{abb}$, beendet seine Arbeit aber im nicht akzeptierenden Zustand $q_a$. Dies ist auch richtig, denn $w = abbc$ liegt nicht in der Sprache $L_{abb}$.\\
...
\begin{myexercise}%[difficulty={1}]
	\noindent
	Beschreiben Sie die Sprache $L_2$, der von dem endlichen Automaten in Abbildung \cref{fig:L2} akzeptierten Wörter.
\end{myexercise}
\begin{myanswer}
	\noindent
	Der endliche Automat in Abbildung \cref{fig:L2} akzeptiert genau die Eingabewörter, für die der endliche Automat seine Arbeit in einem der beiden akzeptierenden Zuständen $p_0$ \textbf{oder} $p_3$ beendet.\\
	...
	\begin{align*}
		L_2 = \setcm{w\in\lrc{a,b}^*}{\text{Das Wort $w$ hat eine gerade Länge ($|w|$ ist eine gerade Zahl).}}
	\end{align*}
\end{myanswer}
...
\begin{myexercise}\label{myexercise:4.3}%[difficulty={1}]
	\noindent
	Betrachten sie den endlichen Automaten in Abbildung \cref{fig:Aabb}. Beschreiben Sie mathematisch präzise die Teilmenge $M_{ab}$ aller Wörter über $\lrc{a,b,c}$, welche in den Zustand $q_{ab}$ führen.
\end{myexercise}
\begin{myanswer}
	\noindent
	Intuitiv gesprochen, ist $M_{ab}$ die Menge aller Wörter aus $\lrc{a,b,c}^*$ welche zwar auf $b$ enden, aber nicht auf $bb$. Wir erhalten $M_{ab}$ durch das Abziehen der Menge $L_{abb}$ von der Menge aller Wörter $M_{axb}$, die mit $a$ beginnen und auf $b$ enden:
	\begin{align*}
...
Um die Äquivalenz einzusehen, werden wir eine Methode demonstrieren, welche jede Graphendarstellung in ein äquivalentes Programm übersetzt (und umgekehrt). Bevor wir die Methode allgemein beschreiben, möchten wir das Vorgehen anhand eines Beispiels entwickeln. Betrachten Sie dazu den endlichen Automaten in Abbildung \cref{fig:EA2}.
\begin{figure}[H]
	\centering
	\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2.8cm,
			semithick]
...
\end{figure}
\noindent
In einem ersten Schritt möchten wir die Zustände $q,p,r$ durch die Nummern $0,1,2$ ersetzen. Die Vergabe der Nummern kann beliebig gewählt werden\footnote{Die zugehörigen Programme werden die gleichen Entscheidungen treffen und bis auf einfache Vertauschungen von Nummern identisch sein. Siehe dazu auch Übung \cref{myexercise:equivalentProgramms}.}, aber dem Anfangszustand muss die Nummer $0$ zugewiesen werden. Wir wählen für $p$ die Nummer $1$ und für $r$ die Nummer $2$. Damit erhalten wir den \quotes{nummerierten Graphen} in Abbildung \cref{fig:EA3}.
\begin{figure}[H]
	\centering
	\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2.8cm,
			semithick]
		...
		(C) edge [bend right, left] node {$\strut b$} (A);
	\end{tikzpicture}
	\caption{Nummerierte Version des endlichen Automaten in Abbildung \cref{fig:EA2} mit Zustandsmenge $\lrc{0,1,2}$.}
	\label{fig:EA3}
\end{figure}
\noindent
Wir möchten nun ein Programm entwerfen, welches den endlichen Automaten in Abbildung \cref{fig:EA3} beschreibt, jedoch ohne Variablen auskommt. Das Programm soll genau so viele Zeilen haben, wie der endliche Automat Zustände (Knoten) hat. In diesem Fall wird das Programm entsprechend genau drei Zeilen haben. Die Nummerierung der Zustände wird dabei genau der Nummerierung der Programmzeilen entsprechen. Deshalb ist es wichtig, dass der Anfangszustand die Nummer $0$ erhält, da das Programm von oben nach unten abgearbeitet wird und somit bei Zeile $0$ seine Arbeit beginnt. Das Programm erhält das Eingabewort des endlichen Automaten als Input.\\
Falls das erste Symbol des Input $a$ oder $b$ ist, dann soll das Programm zu Zeile $1$ springen. Falls das erste Symbol ein $c$ ist, soll das Programm zu Zeile $2$ springen. Nach dem Lesen eines Symbols des Eingabebandes wird dieses automatisch gelöscht und das nächste wird gelesen. Analog ist das Vorgehen für die anderen beiden Zeilen $1$ und $2$ des Programms. Insgesamt wird die Programmdarstellung des endlichen Automaten in Abbildung \cref{fig:EA3} durch folgendes Programm beschrieben:
...
\end{minted}
Falls der endliche Automat in Abbildung \cref{fig:EA3} im Zustand $0$ das Symbol $a$ oder $b$ liest, dann wechselt er in den Zustand $1$. Falls er das Symbol $c$ liest, dann wechselt er in den Zustand $2$. Genau so geht auch unser obiges Programm vor:\\
...
\clearpage
\begin{myexercise}\label{myexercise:equivalentProgramms}%[difficulty={1}]
	\noindent
	Wie müsste obiges Programm angepasst werden, falls wir für die Zustände in Abbildung \cref{fig:EA2} die Nummerierung $q \leftrightarrow 0$, $r \leftrightarrow 1$ und $p \leftrightarrow 2$ gewählt hätten?\\
\end{myexercise}
\begin{myanswer}
	\begin{minted}[linenos, firstnumber = 0]{cpp}
...
\begin{align}\label{eq:erlauberBefehl}
	...
	\text{ \textbf{\darkgreenone{if}} input }= s_2 \text{ \textbf{\darkgreenone{goto}} } i_2,\text{ \ldots  },
...
Nach den Betrachtungen in den Unterabschnitten \cref{subsec:graphenDarstellung} und \cref{subsec:Programmdarstellung} wollen wir nun eine formale Definition von endlichen Automaten angeben. Diese Definition wird uns sehr natürlich erscheinen.
\begin{mydefinition}[endlicher Automat]{mydefinition:endlicherAutomat}
...
\begin{itemize}
	...
	\item Falls $C_n\in ((Q-F)\times \lrc{\lambda})$, dann ist $C$ eine \textbf{verwerfende Berechnung} von $M$ auf $x$. Man sagt dann, dass \textbf{$M$ das Wort $x$ verwirft} (oder \textbf{nicht akzeptiert}).
...
\begin{mydefinition}[akzeptiere Sprache eines endlichen Automaten]{mydefinition:akzeptiereSpracheEA}
...
\begin{mydefinition}[Klasse der regulären Sprachen]{mydefinition:regulaereSprachen}
...
\clearpage
\noindent
Betrachten wir nochmals den endlichen Automaten in Abbildung \cref{fig:EA2}. Diesen Automaten wollen wir nun \textbf{formal} beschreiben. Dazu genügt es, die $5$ Komponenten des $5$-Tupels $(Q,\Sigma, \delta, q_0, F)$ gemäss Definition \cref{mydefinition:endlicherAutomat}, anzugeben. Der zur Graphendarstellung in Abbildung \cref{fig:EA2} äquivalente endliche Automat  ist $M = (Q,\Sigma, \delta, q_0, F)$, wobei:
\begin{align*}
...
\begin{myexercise}%[difficulty={1}]
	\noindent
	Geben Sie die formale Definition des in Abbildung \cref{fig:L2} dargestellten endlichen Automaten an.\\
\end{myexercise}
\begin{myanswer}
	\noindent
	Der zur Graphendarstellung in Abbildung \cref{fig:L2} äquivalente endliche Automat  ist $M = (Q,\Sigma, \delta, q_0, F)$, wobei:
	\begin{align*}
...
\begin{mydefinition}[Stern-Operation (transitive Hülle der Schrittrelation)]{mydefinition:transitive_hull}
...
\begin{align*}
	(q,w) \hull{M} (p,u) \Longleftrightarrow (q = p\text{ und } w = u)\text{ oder es existiert ein } k\in\mathbb{N}-\lrc{0},
\end{align*}
so dass
\begin{enumerate}[label=(\roman*)]
	\item $w = a_1a_2\ldots a_ku$, $a_i\in\Sigma$, fü $i = 1,2,\ldots ,k$ und
	\item es existieren $(k-1)$ Zustände $r_1, r_2, \ldots , r_{k-1}\in Q$, so dass\\
...
\begin{mydefinition}[Dach-Operation]{mydefinition:dach_operation}
...
\begin{example}{example:hat}
	Wir führen hier nochmals den endlichen Automaten aus Abbildung \cref{fig:EA2} auf:
	\begin{figure}[H]
		\centering
		\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2.8cm,
				semithick]
	...
	\begin{align*}
		\deltah{r}{bcac} = p.
	\end{align*}
	Dies sehen wir ein, indem wir Punkt $(ii)$ der Definition \cref{mydefinition:dach_operation} wiederholt anwenden:
	\begin{align*}
	...
	\end{align*}
	Gemäss Punkt $(i)$ der Definition \cref{mydefinition:dach_operation} ist $\blue{\deltah{r}{\lambda}} = r$  und somit verschwinden alle Dach-Operationen aus dem Ausdruck (wir haben den Basis-Fall der Rekusion erreicht). Die Übergangsfunktionen $\delta$ lassen sich nun schrittweise von innen nach aussen auflösen:
	\begin{align*}
		 & \deltah{r}{bcac} = \delta(\deltah{r}{bca},c) = \delta(\delta(\deltah{r}{bc},a),c) = \delta(\delta(\delta(\deltah{r}{bc},c),a),c) = \\
...
In Definition \cref{mydefinition:regulaereSprachen} haben wir die Klasse $\mathcal{L}_{EA}$ der regulären Sprachen kennengelernt. Eine Sprache $L$ ist regulär ($L\in\mathcal{L}_{EA}$), genau dann, wenn ein endlicher Automat $A$ existiert, welcher die Sprache $L$ akzeptiert ($L(A) = L$).\\
...
\noindent
\underline{Beweis:}\\
Wir beweisen die Behauptung durch Widerspruch. Wir nehmen also an, $L_{count}$ sei regulär. Dann existiert ein endlicher Automat $A = (Q,\lrc{0,1}, \delta, q_0, F)$ mit $L(A) = L_{count}$. Die Anzahl der Zustände $Q$ von $A$ ist $|Q|$. Wir betrachten nun die $|Q|+1$ Wörter (ein Wort mehr als $A$ Zustände hat):
\begin{align}\label{eq:list}
	0^1, 0^2, 0^3, \ldots , 0^{|Q|}, 0^{|Q|+1}.
\end{align}
Weil in der Auflistung \cref{eq:list} mehr Wörter stehen, als $A$ Zustände hat, müssen sich in der Liste mindesten zwei unterschiedliche Wörter finden lassen, welche den Automaten in denselben Zustand führen. Damit existieren also zwei Zahlen $i,j\in\lrc{1,2,\ldots ,|Q|, |Q|+1}$ mit $i<j$, so dass
\begin{align*}
	\deltah{q_0}{0^i} = \deltah{q_0}{0^j}.
\end{align*}
Gemäss Satz \cref{theo:pfad_invarianz} muss für jedes Wort $z\in\lrc{0,1}^*$ gelten, dass
\begin{align*}
...
\begin{myexercise}%[difficulty={1}]
	\noindent
	Beweisen Sie mithilfe des Satzes \cref{theo:pfad_invarianz}, dass die Sprache
