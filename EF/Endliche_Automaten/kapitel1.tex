...
\begin{align}\label{eq:dec}
	\text{dec}(x):=\displaystyle\sum_{k=1}^n x_k\cdot 2^{n-k}
\end{align}
betrachtet werden.

\begin{myexercise}\label{myexercise:0298}
	Sei $x = 1011$. Berechnen Sie $\text{dec}(x)$.
\end{myexercise}
Für eine Zahl $m\in\N\setminus\lrc{0}$ wird mit $\text{bin}(m)\in \lr{\Sigma_{\text{bool}}}^*$ die kürzeste binäre Darstellung von $m$ bezeichnet, also gilt $\text{dec}\lr{\text{bin}(m)} = m$. Man setzt $\text{bin}(0) = 0$.
...
%
% 	Die kleinste binäre Zahl der Länge $k+1$ ist $1\underbrace{00\ldots 0}_{k\;\text{mal}}$. Diese hat gemäss der Gleichung \cref{eq:dec} den dezimalen Wert (dec) $2^{k}$. Dann kann der Dezimalwert der grössten binäre Zahl der Länge $k$ höchstens $2^{k-1}-1$ sein ($1$ weniger). Damit ist $2^k-1 \geq m$ begründet.
%
...
\begin{myexample}
Sei $\Sigma_1 = \lrc{g,h}$ und $\Sigma_2 = \lrc{x,y}$, dann ist das kartesische Produkt
\[\Sigma_1\times \Sigma_2 = \lrc{(g,x),(g,y),(h,x),(h,y)}\].
\end{myexample}
...
<<<<<<< Updated upstream
	\item $\emptyset$
	\item $\lrc{\lambda}$
	\item $\Sigma^* = \lrc{\lambda, a,b,aa,\ldots }$
	\item $\Sigma^+ = \lrc{a,b,aa,\ldots }$
	\item $\Sigma$
	\item $\setcm{a^p}{p\;\text{ist eine Primzahl}}$
	\item $\lrc{a}^* = \lrc{\lambda, a, aa, aaa, aaaa, \ldots }$
...
=======
	...
	\item $L_7 = \lrc{a}^* = \lrc{\lambda, a, aa, aaa, aaaa, \ldots }$
	\item $L_8 = \Sigma^2 = \lrc{aa,ab,ba,bb}$
>>>>>>> Stashed changes
\end{itemize}
\end{myexample}
...
\begin{myanswer}[myexercise:0298]
	\begin{align*}
		\text{dec}(x) = \text{dec}(\red{1}\blue{0}\green{1}\orange{1}) = \displaystyle\sum_{k=1}^4 x_k\cdot 2^{4-k} = \red{1}\cdot 2^3+\blue{0}\cdot 2^2 + \green{1}\cdot 2^1+\orange{1}\cdot 2^0 = 8 + 2 + 1 = 11
	\end{align*}
\end{myanswer}
...
\begin{myexercise}\label{myexercise:kp01}
	...
	\begin{aenum}
		...
		\item das Teilwort $00$ nicht enthalten (schwierig, ohne Musterlösungen).
	\end{aenum}
\end{myexercise}
