...
\begin{myexercise}[Kosten eines Zootags]
	\begin{enumerate}
		\item Auf der Webseite des Zoos Zürich finden Sie Angaben zu den Kosten eines Zootags:
		      \begin{center}
			      \url{https://www.zoo.ch/de/der-zoo-zuerich/wie-viel-kostet-ein-zootag}.
		      \end{center}
		      Erstellen Sie ein Python-File \texttt{zootag.py}, welches mithilfe des Pakets \texttt{Matplotlib} ein Kuchendiagramm der Kosten eines Zootags erstellt und mit dem Namen \texttt{ausgaben.pdf} abgespeichert.
		\item Erstelle in demselben Verzeichnis wie \texttt{zootag.py} ein File \texttt{main.tex}, welches den Output \texttt{ausgaben.pdf} mithilfe von \lstinline|\includegraphics{}| einbindet.
		\item Erstelle ein Bash-Script \texttt{run.sh} (Linux / macOS) beziehungsweise ein PowerShell-Script \texttt{run.ps1} (Windows), welches zuerst das Python-File \texttt{zootag.py} ausführt und danach \texttt{main.tex} neu kompiliert. Die Ausführung des Skripts soll also den Plot neu erzeugen und direkt das Dokument \texttt{main.pdf} neu erzeugen.
	\end{enumerate}
\end{myexercise}
...
\begin{myexercise}
	In dieser Aufgabe erstellst du ein Probekapitel für deine Maturitätsarbeit sowie eine kurze Beamer-Präsentation, um die Inhalte vorzustellen.

	\textbf{Teil 1: Probekapitel in \LaTeX}

	Erstelle ein neues \LaTeX-Dokument (\lstinline|kapitel.tex|), das als Entwurf für ein Kapitel deiner Maturitätsarbeit dient. Das Thema kannst du frei wählen, idealerweise orientiert es sich an deinem tatsächlichen Maturitätsarbeitsthema.

	\textbf{Anforderungen an das Dokument:}
	\begin{enumerate}
		\item \textbf{Dokumentklasse:} Verwende die \lstinline|article|-Klasse.
		\item \textbf{Titelei:} Dein Dokument soll einen Titel, deinen Namen als Autor und das aktuelle Datum enthalten.
		\item \textbf{Verzeichnisse:} Füge ein Inhaltsverzeichnis (\lstinline|\tableofcontents|) und ein Abbildungsverzeichnis (\lstinline|\listoffigures|) am Anfang des Dokuments ein.
		\item \textbf{Struktur:} Gliedere dein Kapitel mit Abschnitten (\lstinline|\section|) und Unterabschnitten\\ (\lstinline|\subsection|). Der Text kann aus Platzhaltern (z.B. mit dem \lstinline|lipsum|-Paket) oder selbst verfassten Abschnitten bestehen.
		\item \textbf{Abbildung:} Binde mindestens eine Abbildung (\lstinline|figure|-Umgebung) mit einer Bildunterschrift (\lstinline|\caption|) ein.
		\item \textbf{Fussnote:} Füge mindestens eine Fussnote (\lstinline|\footnote|) mit einer Anmerkung oder Quellenangabe hinzu.
		\item \textbf{Literaturverzeichnis:}
		      \begin{itemize}
			      \item Erstelle eine separate \lstinline|.bib|-Datei (z.B. \lstinline|literatur.bib|) mit mindestens zwei Literatureinträgen (z.B. ein Buch und ein Online-Artikel).
			      \item Binde das \lstinline|biblatex|-Paket ein und zitiere deine Quellen mindestens einmal im Text mit \lstinline|\cite{...}|.
			      \item Gib am Ende des Dokuments das Literaturverzeichnis mit \lstinline|\printbibliography| aus.
		      \end{itemize}
	\end{enumerate}

	\textbf{Teil 2: Beamer-Präsentation}

	Erstelle eine kurze Beamer-Präsentation (ca. 5 Minuten), die die wichtigsten Punkte deines Probekapitels zusammenfasst.

	\textbf{Anforderungen an die Präsentation:}
	\begin{enumerate}
		\item \textbf{Dokumentklasse:} Verwende die \lstinline|beamer|-Klasse.
		\item \textbf{Titel-Folie:} Erstelle eine Titelfolie mit Titel, Autor und Datum.
		\item \textbf{Inhalt:} Erstelle 3--4 Inhaltsfolien, die dein Thema zusammenfassen.
		      \begin{itemize}
			      \item Nutze Aufzählungen (\lstinline|itemize| oder \lstinline|enumerate|), um die Inhalte übersichtlich darzustellen.
			      \item Binde mindestens eine Abbildung aus deinem Probekapitel in die Präsentation ein.
		      \end{itemize}
	\end{enumerate}
\end{myexercise}
	...
	\begin{myanswer}[myexercise:kaufpreis_haus]
		\begin{enumerate}
			...
			\item
			      ...
			      \lstinputlisting{Grundlagen_Info/00_Programmieren/Skript/Code/K05_Functions/max_kaufpreis.py}

			      \begin{figure}[H]
				      \centering
				      \includegraphics[width=0.9\textwidth]{Grundlagen_Info/00_Programmieren/Skript/Code/K05_Functions/max_kaufpreis_3d_plot.pdf}
