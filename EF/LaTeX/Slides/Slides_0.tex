...
\title[Textverarbeitung in \LaTeX\ Teil 1]{Textverarbeitung in \LaTeX}
...
\begin{itemize}[<+->]
    ...
    \item ein geeigneter \TeX-Editor (z.B. VS Code, TeXstudio, TEXMAKER)
\end{itemize}
\end{frame}
...
\begin{frame}[fragile]
	\frametitle{Installation und Konfiguration einer \LaTeX-Extension für VS Code}
	Installiere die VS Code-Extension \texttt{Latex Workshop} (von James Yu).
\end{frame}

\begin{frame}[fragile]
	\frametitle{Latex Workshop Rezepte erstellen}
	Öffne in VS Code die Kommando-Palette durch:
\begin{itemize}[<+->]
    \item MacOS:
\begin{lstlisting}[language=bash, numbers=none]
Cmd + Shift + P
...
\textbf{Füge} den Code in \texttt{latex\_recipies.json} (siehe Moodle) zu deinem \texttt{settings.json} hinzu.
\end{frame}

\begin{frame}[fragile]
\frametitle{erstes Dokument}
	    Erstellen Sie in VS Code ein neues File \texttt{minimal.tex} mit dem folgenden Inhalt:
	    ...
	    %     \end{document}
	    % \end{lstlisting}
		und kompiliere dieses mit der Extension \texttt{Latex Workshop}.
\end{frame}
...
\begin{myexercise}\label{myexercise:hitchens-razor}
    Erstelle ein PDF-Dokument mit exakt folgendem Inhalt (recherchiere):
...
Speichere ein Bild deiner Wahl in demselben Ordner wie dein \LaTeX-Dokument ab. Skaliere das Bild angemessen und gib ihm eine kurze Beschreibung in blauer Farbe.
\begin{figure}
    \centering
...
\begin{frame}[fragile]
\frametitle{Historische Entwicklung}
Nehmen wir an, wir möchten nun eine Präsentation wie diese hier schreiben. Konzentrieren wir uns einmal auf die Überschrift \enquote{Historische Entwicklung} dieses Abschnitts (Section). Wie ist diese wohl entstanden? 
...
\begin{frame}[fragile]{Lernziele}
	\begin{itemize}
		\item Begriffe \textit{WYSIWYG} und \textit{WYSIWYM} erklären können
		\item den Aufbau und die Struktur eines \LaTeX-Dokuments kennen
		\item Aufbau und Struktur von Befehlen in \LaTeX\ verstehen und einige grundlegende Befehle wie \verb|\textit, \textbf, \sqrt| kennen
		\item die Idee hinter Macros und ihren Aufbau kennen
		\item typische Fehler in gegebenen Code-Beispielen erkennen und erklären können
	\end{itemize}
\end{frame}