...
class Pendulum:
    ...
    def draw(self, surface):
        bob_pos = (self.fixation_point.x + self.length * math.sin(self.angle),
                   self.fixation_point.y + self.length * math.cos(self.angle))
        pygame.draw.line(surface, (100, 100, 100), self.fixation_point, bob_pos, 2)
        pygame.draw.circle(surface, (50, 50, 200), (int(bob_pos[0]), int(bob_pos[1])), 20)
...
def main():
    ...
    while running:
        ...
        img = font.render(f"Methode: {['Euler', 'Symp. Euler', 'Verlet'][METHOD-1]}", True, (0,0,0))
    ...
    plt.title(f"Energieverlauf: {['Expliziter Euler', 'Symplektischer Euler', 'Velocity-Verlet'][METHOD-1]}")
