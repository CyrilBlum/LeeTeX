...
%     \ddot{x}(t) = a(x(t), t)
% \end{equation}
% Hierbei hängt die Beschleunigung $a$ nur von der Position $x$ und der Zeit $t$ ab, jedoch \textbf{nicht} von der Geschwindigkeit $v$. Die Anfangswerte zum Zeitpunkt $t_n$ sind gegeben als $x(t_n) = x_n$ und $v(t_n) = v_n$. Gesucht sind die Werte $x_{n+1}$ und $v_{n+1}$ zum Zeitpunkt $t_{n+1} = t_n + \Delta t$.
...
% \begin{enumerate}
%     \item \textbf{Positions-Update:} 
%     \[ x_{n+1} = x_n + v_n \Delta t + \frac{1}{2} a_n \Delta t^2 \]
%     \item \textbf{Berechnung der neuen Beschleunigung:} 
%     \[ a_{n+1} = a(x_{n+1}, t_{n+1}) \]
%     \item \textbf{Geschwindigkeits-Update:} 
%     \[ v_{n+1} = v_n + \frac{1}{2} (a_n + a_{n+1}) \Delta t \]
% \end{enumerate}
