    ...
    private:
        ...
        _interpreter() {
            ...
            s_python_function_contour =
                PyObject_GetAttrString(pymod, "contour");
            s_python_function_axhline =
                PyObject_GetAttrString(pymod, "axhline");
            s_python_function_axvline =
                PyObject_GetAttrString(pymod, "axvline");
            s_python_function_semilogx =
                PyObject_GetAttrString(pymod, "semilogx");
            s_python_function_semilogy =
                PyObject_GetAttrString(pymod, "semilogy");
            ...
            s_python_function_scatter =
                PyObject_GetAttrString(pymod, "scatter");
            s_python_function_spy = PyObject_GetAttrString(pymod, "spy");
            s_python_function_subplot =
                PyObject_GetAttrString(pymod, "subplot");
            ...
            s_python_function_save =
                PyObject_GetAttrString(pylabmod, "savefig");
            s_python_function_annotate =
                PyObject_GetAttrString(pymod, "annotate");
            s_python_function_clf = PyObject_GetAttrString(pymod, "clf");
            s_python_function_errorbar =
                PyObject_GetAttrString(pymod, "errorbar");
            ...
            s_python_function_suptitle =
                PyObject_GetAttrString(pymod, "suptitle");
...
template <typename Vector> PyObject *get_array(const Vector &v) {
    ...
    if (type == NPY_NOTYPE) {
        ...
        // Eigen Vectors do not support begin/end() in the currently stable
        // version this can be changed once Eigen 3.4. is released data()
        // returns a pointer to the storage of the first element. If the vector
        // is modified afterwards, it may be rendedered invalid. Note, that this
        // is not an issue since get_array() is called by a plot command using
        // the instantaneous state of the vector. The pointer is not reused. If
        // the vector is plotted anew, data() is called again and again get's
        // the current, valid storage location.
        std::copy(v.data(), v.data() + v.size(), vd.begin());
        PyObject *varray = PyArray_SimpleNewFromData(1, &vsize, NPY_DOUBLE,
                                                     (void *)(vd.data()));
        return varray;
    }
...
bool loglog(const VectorX &x, const VectorY &y, const std::string &s = "",
    ...
    return detail::plot_base(
        detail::_interpreter::get().s_python_function_loglog, x, y, s,
        keywords);
}
...
void imshow(const Matrix &X,
    ...
    PyObject *res =
        PyObject_Call(detail::_interpreter::get().s_python_function_imshow,
                      plot_args, kwargs);
...
inline void xkcd() {
    ...
    res =
        PyObject_Call(detail::_interpreter::get().s_python_function_xkcd,
                      detail::_interpreter::get().s_python_empty_tuple, kwargs);
