...
Pendulum::Pendulum(double tstart, double tend, unsigned int steps, double v0,
                   double y0, double L, double g, double m)
    : tstart(tstart),
      tend(tend),
      steps(steps),
      v0(v0),
...
void Pendulum::write_vectors_to_file(const std::string &filename,
                                     const std::vector<T> &vec,
                                     const std::vector<Args> &...args) const {
    // Open the file in write mode
    std::string baseDir = "gnuplot_files/";
    std::ofstream outFile(baseDir + filename);
    // Get the size of the vectors (all vectors are assumed to have the same
    // length)
    ...
    for (std::size_t i = 0; i < size; ++i) {
        ...
        outFile << vec[i];
        ((outFile << "\t" << args[i]),
         ...); // Fold expression to write the rest of the vectors
        outFile << std::endl;
    }
}

void Pendulum::evolve_pendulum() {
    // Copy private data members to local variables
    const double g_local = g;
    const double L_local = L;

    VelocityVerletSolver VerletEvolution(
        tstart, tend, steps, v0, y0, [g_local, L_local](double y) {
            return -(g_local / L_local) * std::sin(y);
        });
...
void Pendulum::compute_energies() {
    // compute energies
...
void Pendulum::write_data_to_files() const {
