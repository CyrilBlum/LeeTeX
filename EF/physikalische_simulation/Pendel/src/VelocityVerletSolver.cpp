...
VelocityVerletSolver::VelocityVerletSolver(double tstart, double tend,
                                           unsigned int steps, double v0,
                                           double y0,
...
void VelocityVerletSolver::single_velocity_verlet_step() {
    // step 1: compute y_{k+1}
    y[current_step + 1] = y[current_step] + h * v[current_step] +
                          h * h * f(y[current_step]) / 2.0;
    // step 2: compute v_{k+1}
    v[current_step + 1] =
        v[current_step] +
        h / (2.0) * (f(y[current_step]) + f(y[current_step + 1]));
    ++current_step;
}

void VelocityVerletSolver::solve() {
