...
Solver::Solver (sf::Vector2f constraint_center, float constraint_radius, sf::Vector2f a_gravity, unsigned int n_sub_steps, float frame_dt)
    : constraint_center (constraint_center), constraint_radius (constraint_radius), a_gravity (a_gravity), n_sub_steps (n_sub_steps), frame_dt (frame_dt)
{
}

Particle &
Solver::add_particle (sf::Vector2f y0, sf::Vector2f v0, sf::Vector2f a0, float radius, sf::Color color)
{
  return particles.emplace_back (y0, v0, a0, radius, color);
}

sf::Vector3f
Solver::get_constraints () const
{
      ...
      for (unsigned int j = i + 1; j < particle_count; ++j)
        {
          ...
          const sf::Vector2f vec_p1p2 = particle_1.y_particle - particle_2.y_particle;
          const float squared_dist = vec_p1p2.x * vec_p1p2.x + vec_p1p2.y * vec_p1p2.y;
          ...
          if (squared_dist < min_dist * min_dist)
            {
              ...
              particle_1.y_particle -= vec_p1p2_normalized * mass_ratio_2 * kappa;
              particle_2.y_particle -= vec_p1p2_normalized * mass_ratio_1 * kappa;
            }
        }
    }
}
      ...
      if (dist > constraint_radius - particle.radius)
        {
          ...
          particle.y_particle = constraint_center - vec_normalized * (constraint_radius - particle.radius);
        }
    }
}

void
Solver::verlet_updates (float h)
{
  for (auto &particle : particles)
    {
